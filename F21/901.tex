...
\begin{rem} The notation above follows \DEF{cohmological indexing}, in which we write a complex \[ \cdots \to M_2 \xra{d_2} M_1 \xra{d_1} \to M_0 \xra{d_0}  M_{-1} \xra{d_{-1}} M_{-2} \to\cdots\] 
as 
\[ \cdots \to N^{-2} \xra{d^{-2}} N^{-1} \xra{d_{-1}} \to N^0 \xra{d^0}  N^{1} \xra{d^1} N^{2} \to\cdots,\]
...
Suppose we have consructed maps $\tf_0, \dots, \tf_n$ for some $n \geq 0$  so that
 $d^Q_i \circ \tf_i = \tf_{i-1} \circ d^P_i$ for $1 \leq i \leq n$. (When $n = 0$, the condition is vacuous.) Then $d_n^Q \tf_n d_{n+1}^P = \tf_{n-1} d_n^P d_{n+1}^P = 0$, so $\im(\tf_n d_{n+1}^P )\in Z_{n}(Q_\bullet)$.  Use the definition of projective again with the diagram
$$
\xymatrix{
...
$$
 where the bottom row is exact by exactness of $Q_\bullet$, to construct $\tf_{n+1}$ such that  $d^Q_{n+1} \circ \tf_{n+1} = \tf_{n} \circ d^P_n$ holds too.  This proves existence. 
...
to $N$. Thus, it suffices to prove that if $g: P_\bullet \to Q_\bullet$ is a chain map such that $q g_0 = 0$, then $g$ is null-homotopic. That is, we need to show there are maps
$h_i: P_i \to Q_{i+1}$ for $i \geq 0$ such that $d^Q_{i+1} \circ h_i + h_{i-1} \circ d^P_i = g_i$ for all $i \geq 0$. (In the latter equation,
when $i = 0$ we have $h_{-1} = 0$.)

Since $q \circ g_0 = 0$, the image of $g_0$ is contained in $\ker(q) = \im(d^Q_1)=B_0(Q_\bullet)$ and so since $P_0$ is projective, considering the diagram
$$
\xymatrix{
& P_0 \ar[d]^{g_0}   \ar@{-->}[dl]_{\exists h_0} \\
Q_1 \ar[r]^{d_1} & B_0(Q_\bullet) \ar[r] & 0. \\
}
$$ there is a map $h_0: P_0 \to Q_1$ such 
that $d_1^Q \circ h_0 = g_0$ as needed. 


Suppose maps $h_0, \dots, h_n$ have been constructed for some $n \geq 0$ with $g_n = d^Q_{n+1} \circ h_n + h_{n-1} \circ d^P_n$, and that $\im(g_{n+1} - h_n d^P_{n+1}) \subseteq B_{n+1}(Q_\bullet)$. 

Since $P_{n+1}$ is projective, considering the diagram
$$
\xymatrix{
& P_{n+1} \ar[d]^{g_{n+1}-h_n d^P_{n+1}}   \ar@{-->}[dl]_{\exists h_{n+1}} \\
Q_{n+2} \ar[r]^{d^Q_{n+2}} & B_{n+1}(Q_\bullet) \ar[r] & 0, \\
}
$$ there is a map $h_{n+1}: P_{n+1} \to Q_{n+2}$ such 
that $d_{n+2}^Q \circ h_{n+1} = g_{n+1}-h_n d^P_{n+1}$. Then
\[ d^Q_{n+2}(g_{n+2} - h_{n+1} d^P_{n+2}) = g_{n+1} d^P_{n+2} - d^Q_{n+2} h_{n+1} d^P_{n+2} = (g_{n+1} - d^Q_{n+2} h_{n+1}) d^P_{n+2} = h_n d^P_{n+1} d^P_{n+2} = 0.\]
Again using exactness of $Q_\bullet$, we see that $\im(g_{n+2} - h_{n+1} d^P_{n+2}) \subseteq B_{n+2}(Q_\bullet)$. Thus, by induction, we can construct such a map $h$.
\end{proof}
...
\begin{exer} If $g$ is a homotopy equivalence, and $F$ is a functor, then $F(g)$ is a homotopy equivalence.
\end{exer}
...
\Cat{$\Z$-modules}$. So, for each $j$, $\Tor^R_j(N, M)$ is an abelian group. When $R$ is commutative, $N \otimes_R -$ can be viewed as taking values in $\Mod{R}$ and hence $\Tor^R_j(N, M)$ is an $R$-module.
\end{defn}
...
Explicitly, 
$$
\Tor_R^i(N, M) = H_j( \cdots \xra{\id_N \otimes d_3} N \otimes_R P_2 \xra{\id_N \otimes d_2} N \otimes_R P_1
\xra{\id_N \otimes d_1} N \otimes_R P_0 \to 0 \to \cdots)
$$
where $P_\bullet \xra{\sim} M$ is a projective resolution of $M$. 

\begin{ex} 
Let's compute $\Tor^\Z_j(N, \Z/n)$ for any $\Z$-module $N$ and integers $n \geq 1$, and $j$. 

We have the projective resolution $\cdots \to 0 \to \Z \xra{n} \Z \xra{\can} \Z/n \to 0$ of $\Z/n$ and so $\Tor^R_j(N,\Z/n)$ is the homology of the complex
$$
...
and hence
$$
\Tor_0^R(N, \Z/n) \cong N/nN \cong N \otimes_\Z \Z/n
$$
(as the Proposition below tells us), 
$$
\Tor_1^R(N, \Z/n) \cong \ker(N \xra{n} N) = \{x \in N \mid n \cdot x = 0\},
$$
and
$$
\Tor_j^R(N, \Z/n) = \bL_j F(\Z/n) = 0
$$
for all $j \notin \{0,1\}$.

Note that $\Tor_1^R(N, \Z/n)$ is the $n$-torsion submodule of $N$ --- this explains the notation $\Tor$.
\end{ex}
