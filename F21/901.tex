...
We claim that $f$ cannot be extended to $g:R\to Q/R$. Indeed, given an extension $g$, we have $x g(1) = g(x) = f(x) = [\frac{1}{y}]$, so $g(1) = [\frac{1}{xy}]$, but then we would have $f(y) = g(y) = y g(1) = [\frac{1}{x}]$, but $f(y)=0$.
\end{ex}
