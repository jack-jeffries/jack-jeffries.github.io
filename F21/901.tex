...
\sssec{Natural transformations}

\begin{defn}
	Let $F$ and $G$ be covariant functors $\mathscr{C} \longrightarrow \mathscr{D}$. A \DEF{natural transformation} between $F$ and $G$ is a mapping that to each object $A$ in $\mathscr{C}$ assigns a morphism $\eta_A \in \Hom_\mathscr{D}(F(A),G(A))$ such that for all $f \in \Hom_\mathscr{C}(A,B)$, the diagram
	...
	A {\bf natural isomorphism} is a natural transformation $\eta$ where each $\eta_A$ is an isomorphism. 
\end{defn}

In short, a natural transformation is a rule to turn $F$ of whatever into $G$ of whatever in a reasonable way.

\begin{ex} Let $F,G: \sC \to \sD$ be covariant functors. Show that a natural transformation $\eta: F\implies G$ is a natural isomorphism if and only if there is another natural transformation $\mu:G\implies F$ such that $\mu \circ \eta=\eta \circ \mu = 1_{\sC}$.
\end{ex}
...
\begin{ex} 
Let's describe a natural transformation of functors $(-)^{\times 2} \implies 1_{\Set}$, namely we take 
\[p_{1,S}:S\times S \to S \qquad (s_1,s_2) \mapsto s_1. \]
(In the terminology of the definition this is the map $\eta_S$.) We need to check for every map $f:S\to T$ the commutativity of a diagram:
$$\xymatrix{S\times S \ar[d]_-{(f,f)} \ar[r]^-{p_{1,S}} & S \ar[d]^-{f} \\ T\times T \ar[r]_-{p_{1,T}} & T.}$$
Going either down then left or right then down, $(s_1,s_2)$ maps to $f(s_1)$, so this does commute, and we indeed have a natural transformation. This is not a natural isomorphism, since the map $p_{1,S}$ is not always (almost never) an isomorphism of sets. 
\end{ex}

\begin{ex}
Recall the contravariant functor $(-)^*:\vs{K}\to \vs{K}$. Composing this functor with itself twice we get the covariant double-dual functor $(-)^{**}:\vs{K}\to\vs{K}$. We will show that there is a natural isomorphism $1_{\vs{K}}\implies (-)^{**}$.
...
\begin{fact} A pair of covariant functors $F:\cC\to \cD$ and $G:\cD\to \cC$ is an equivalnce of categories if and only
~if
\begin{itemize}
...
\begin{eg} Recall the category $\MaT{K}$ of matrices over a field: the objects are positive natural numbers, and the morphisms from $m \to n$ are $n\times m$ matrices over $K$. For convenience, let $\vsp{K}$ be the category of finite-dimensional $K$-vector spaces with the zero-dimensional ones and all morphisms to and from them removed.

I claim that $\MaT{K}$ is equivalent to $\vsp{K}$. Let's construct functors that realize this. Let $F:\MaT{K} \to \vsp{K}$ be the functor that on objects sends $n$ to $K^n$, the vector space of $n$-tuples, and on morphisms sends the matrix $A$ to the map of left multiplication by $A$. This is a functor because composition of linear transformations agrees corresponds to multiplication of matrices. To construct $G:\vsp{K} \to \MaT{K}$, choose a basis $\cB_{V}$ for every $V\in \Ob(\vsp{K})$ (this requires a pretty hefty version of the axiom of choice!); among these, choose the standard bases for the vector spaces $K^n$. Then, for every $V\in \Ob(\vsp{K})$, we set $G(V)=\dim(V)$, and for every linear transformation $V\xrightarrow{\alpha}W$ we set $G(\alpha)$ to be the matrix of $\alpha$ in the bases $\cB_{V}, \cB_{W}$. It is not hard to see that this is a functor.

We need to show that $G\circ F$ is naturally isomorphic to the identity functor on $\MaT{K}$. In fact, it is the identity functor: on objects $n\mapsto K^n \mapsto n$, and on morphisms, $m \xrightarrow{A}  n \mapsto K^n \xrightarrow{A\cdot} K^m \mapsto m\xrightarrow{A}n$, since we chose the standard bases for $K^n$ and $K^m$.

We also need to show that $F\circ G$ is naturally isomorphic to the identity functor on $\vsp{K}$. On objects $(F\circ G)(V)=K^{\dim V}$, and on morphisms, $(F\circ G)(V\xrightarrow{\alpha} W) = K^{\dim(V)} \xrightarrow{A\cdot} K^{\dim(W)}$ where $A$ is the matrix of $\alpha$ in $(\cB_V,\cB_W)$-coordinates. We construct a natural isomorphism $1_{\vsp{K}} \implies F\circ G$ by taking $V \xrightarrow{\sim} K^{\dim(V)}$ to be the $\cB_V$ coordinate map\dots
