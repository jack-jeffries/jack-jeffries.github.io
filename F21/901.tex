...
\begin{defn} For a pair of left $R$-modules $M$ and $N$, we write
  ...
  $$
  where $F$ in Definition \ref{def:rightderived1} is taken to be the left exact covariant functor $F := \Hom_R(M, -)$. 
  
We define
  $$
  \Ext^j_R(M, N)^{II} = \R^j  \Hom_R(-,N)(M)
  $$
  where $G$ in Definition \ref{def:rightderived2} is taken to be the left exact contravariant functor $G := \Hom_R(-,N)$. 
