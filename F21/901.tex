...
\begin{ex} Let $K$ be a field, and $R=K[x,y]/(xy)$. Take $M=R/(x)$. There is a surjection $\pi:R\to M$. The kernel of this surjection is the ideal generated by $x$, and since the annihilator of $x$ is $(y)$, we can take $d_1: R\xra{y} R$. Now, the kernel of this is the ideal generated by $y$, which has annihilator $(x)$, so we can take $d_2:R\xra{x} R$. This keeps going back and forth, so we have a free resolution
...
Let $N = \coker(j) = E^1/\im(j)$ and apply this result again to obtain a injective $R$-module map $N \to E^1$ with $E^1$ injective. 
...
to $N$. Thus, it suffices to prove that if $g: P_\bullet \to Q_\bullet$ is a chain map such that $q g = 0$, then $g$ is null-homotopic. That is, we need to show there are maps
...
Thus, by induction, we can construct such a map $h$.
\end{proof}
