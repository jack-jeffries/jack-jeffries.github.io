...
\begin{ex} Let $M_\bullet$ be a chain complex. We can make chain complexes $Z_\bullet(M_\bullet), B_\bullet(M_\bullet)$ with differential zero. Then we obtain a short exact sequence of complexes
\[ 0 \to Z_\bullet(M_\bullet) \xra{j'} M_\bullet \xra{d} B_\bullet[-1](M_\bullet) \to 0,\]
where $d$ is given by the differential on $M_\bullet$:
\[ \xymatrix{ & \vdots \ar[d] & \vdots \ar[d] & \vdots \ar[d] & \\ 
0 \ar[r] & Z_n(M_\bullet) \ar[r] \ar[d]_0 & M_n \ar[d]_{d_n} \ar[r]^-{d_n} & B_{n-1}(M_\bullet) \ar[r] \ar[d]^0 & 0\\
0 \ar[r] & Z_{n-1}(M_\bullet) \ar[r] \ar[d] & M_{n-1} \ar[d] \ar[r]^-{d_{n-1}} & B_{n-2}(M_\bullet) \ar[r] \ar[d] & 0 \\
& \vdots & \vdots & \vdots &}\]
(The $[-1]$ after $B_\bullet$ is to keep track of the fact that the homological indices are off by 1.)
We compute the long exact sequence of homology:
\[ \cdots \to H_n(Z_\bullet(M_\bullet)) \xra{H_n(j')} H_i(M_\bullet) \xra{H_n(d)} H_n(B_\bullet[-1](M_\bullet)) \xra{\partial} H_{n-1}(Z_\bullet(M_\bullet)) \to \cdots.\]
Since $Z_\bullet(M_\bullet)$ and $B_\bullet[-1](M_\bullet)$ have zero differential, the homology modules are just the modules of those complexes. The map $H_i(j')$ maps a cycle to its homology class in $H_i(M_\bullet)$. The map $H_i(d)$ takes a homology class, and applies the differential to a representative, which yields zero since a representative is a cycle, so this is the zero map. The connecting map is just the inclusion map of the boundaries into the cycles. Thus the long exact sequence is
\[ \cdots \to H_{n+1}(M_\bullet) \xra{0} B_n(M_\bullet)  \to Z_n(M_\bullet)  \to   H_{n}(M_\bullet) \xra{0} B_{n-1}(M_\bullet) \to \cdots,\]
which breaks into short exact sequences
\[ 0\to B_n(M_\bullet) \to Z_n(M_\bullet) \to H_n(M_\bullet) \to 0.\]
\end{ex}

Here is an application.

\begin{prop}
Let $F$ be an exact functor and $M_\bullet$ be a chain complex. There are isomorphisms $H_n(F(M_\bullet)) \cong F(H_n(M_\bullet))$ for all $n\in \Z$.
\end{prop}

\begin{proof}
Write $j'_n :B_n(M_\bullet) \to Z_n(M_\bullet)$, $j''_n:Z_n(M_\bullet) \to M_n$, and $j''_n \circ j'_n = j_n: B_n(M_\bullet) \to M_n$ for the inclusion maps.
We have a short exact sequence
\[ 0 \to B_n(M_\bullet) \xra{j'_n} Z_n(M_\bullet) \to H_n(M_\bullet) \to 0,\] and hence also
\[ 0 \to F(B_n(M_\bullet)) \xra{F(j'_n)} F(Z_n(M_\bullet)) \to F(H_n(M_\bullet)) \to 0.\]

Observe that the differential $d_n: M_n \to M_{n-1}$ can be written as $d_n=j_n \circ d'_n$, with $d'_n:M_n \to B_{n-1}(M_\bullet)$.
Apply $F$ to the short exact sequence of complexes
\[ 0 \to Z_\bullet(M_\bullet) \xra{j''} M_\bullet \xra{d'} B_\bullet[-1](M_\bullet) \to 0,\]
to get
\[ 0 \to F(Z_\bullet(M_\bullet)) \xra{F(j'')} F(M_\bullet) \xra{F(d')} F(B_\bullet[-1](M_\bullet)) \to 0,\]
i.e.,
\[ \xymatrix{ & \vdots \ar[d] & \vdots \ar[d] & \vdots \ar[d] & \\ 
0 \ar[r] & F(Z_n(M_\bullet)) \ar[r]^-{F(j''_n)} \ar[d]_0 & F(M_n) \ar[d]_{F(d_n)} \ar[r]^-{F(d'_n)} & F(B_{n-1}(M_\bullet)) \ar[r] \ar[d]^0 & 0\\
0 \ar[r] & F(Z_{n-1}(M_\bullet)) \ar[r]^-{F(j''_{n-1})} \ar[d] & F(M_{n-1}) \ar[d] \ar[r]^-{F(d'_{n-1})} & F(B_{n-2}(M_\bullet)) \ar[r] \ar[d] & 0 \\
& \vdots & \vdots & \vdots &}\]
Take the long exact sequence of homology:
\[ \cdots \to F(Z_n(M_\bullet)) \xra{H_n(j'')} H_n(F(M_\bullet)) \xra{H_n(F(d'))} F(B_{n-1}(M_\bullet)) \xra{\partial} F(Z_{n-1}(M_\bullet)) \to \cdots.\]
As $F(d)$ is the differential on $F(M_\bullet)$, the map $H_n(F(d'))$ is zero. We claim that the connecting morphism is $F(j'_{n-1})$. Indeed, for $b\in F(B_{n-1}(M_\bullet))$, take $m\in F(M_n)$ with $F(d'_n)(m)=b$. Then \[F(d_n)(m) = F(j_{n-1})(b) = F(j''_{n-1}) ( F(j'_{n-1})(b)),\] so $\partial(b) = F(j'_{n-1})(b)$. Thus, the long each sequence in homology reads
\[ \cdots \to H_{n+1}(F(M_\bullet)) \xra{0} F(B_{n}(M_\bullet)) \xra{F(j'_{n})} F(Z_{n}(M_\bullet)) \to H_{n}(F(M_\bullet)) \xra{0}  F(B_{n-1}(M_\bullet)) \to \cdots,\]
which yields short exact sequences
\[ 0\to F(B_{n}(M_\bullet)) \xra{F(j'_{n}} F(Z_{n}(M_\bullet)) \to H_{n}(F(M_\bullet))  \to 0.\]
Thus, we obtain isomorphisms $F(H_n(M_\bullet)) \cong H_{n}(F(M_\bullet))$.
\end{proof}
...
\ssec{Balancing Tor and Ext}

Our last goal is to balance Tor and Ext. We'll just deal with Ext, but Tor is similar.

\begin{defn} Given a module $M$ and a projective resolution $P_\bullet \to M$, we denote by \Def{$\Omega_i(M)$} the $i-1$ module of cycles of the exact complex (augmented resolution)
\[ \cdots \to P_2 \to P_1 \to P_0 \to M \to 0.\]
We call this the $i$th \DEF{syzygy} module of $M$.
\end{defn}
...
\[ 0 \to \Omega_{i+1}(M) \to P_i \to \Omega_i(M) \to 0.\]

Similarly, given an injective resolution $M\to E^\bullet$, we define the \DEF{cosyzygy} modules \Def{$\Omega^i(M)$}, and have $\Omega^0(M)=M$ and
\[0 \to \Omega^{i}(M) \to E_i \to \Omega^{i+1}(M) \to 0.\]


\begin{lem} 
\begin{enumerate}
\item Let $F$ be a right exact covariant functor. Then for all $i \geq 0$ and all $j > 0$ there are isomorphisms
\[ \bL_j F(\Omega_{i+1}(M)) \cong \bL_{j+1} F(\Omega_i(M)).\]
Hence, $\bL_k F(M) \cong \bL_1F (\Omega_{k-1}(M))$ for all $k>0$.
\item Let $F$ be a left exact covariant functor. Then for all $i\geq 0$ and $j>0$ there are isomorphisms
\[ \bR^j F(\Omega^{i+1}(M)) \cong \bR^{j+1} F(\Omega^i(M)).\]
Hence $\bR^k F(M) \cong \bR^1 F(\Omega^{k-1} M)$ for all $k>0$.
\item Let $G$ be a left exact contravariant functor. Then for all $i\geq 0$ and $j>0$ there are isomorphisms
\[ \bR^j G(\Omega_{i+1}(M)) \cong \bR^{j+1} G(\Omega_i(M)).\]
Hence $\bR^k G(M) \cong \bR^1 G(\Omega_{k-1} M)$ for all $k>0$.
\end{enumerate}
\end{lem}
\begin{proof} Take the long exact sequence of $\bL_j F$ from the short exact sequence above. We get 
\[ \cdots \bL_{j+1} F (P_i) \to \bL_{j+1} \Omega_i(M) \to \bL_{j} \Omega_{i+1}(M) \to \bL_j F(P_i) \to \cdots.\]
For $j>0$, we have  $\bL_j F(P_i) = 0$ since $P_i$ is projective (so it is a projective resolution of itself). The isomorphisms follow.

The other cases are similar.
\end{proof}

\begin{comment}
As a consequence, we have $\Tor_i^R(N,M) \cong \Tor_1^R(N,\Omega_{i-1}(M))$ and $\mathrm{tor}_i^R(N,M) \cong \mathrm{tor}_1^R(\Omega_{i-1}(N),M)$ for all $i$.

\begin{lem} Given a commutative diagram with each row and column exact of the form
\[ \xymatrix{ & \ker(a) \ar[r] \ar[d] & 0 \ar[d]& \ker(c) \ar[d] & \\
\ker(b) \ar[r] \ar[d] & L' \ar[r]^b \ar[d]_a & M' \ar[r] \ar[d] & N' \ar[r] \ar[d]^c & 0 \\
0 \ar[r]  & L \ar[r] \ar[d] & M \ar[r] \ar[d] & N \ar[r] \ar[d] & 0 \\
\ker(d) \ar[r]  & L'' \ar[r]^d \ar[d] & M'' \ar[r] \ar[d] & N'' \ar[r] \ar[d] & 0 \\
& 0 & 0 & 0 &}\]
we have $\ker(c) \cong \ker(d)$ and $\ker(a)=\ker(b)$.
\end{lem}
\begin{proof} We can apply the Snake Lemma to the top two rows to get
\[ 0 \to \ker(c) \xra{\partial} L'' \xra{d} M'' \]
exact, so $\ker(c) \cong \ker (d)$. Since $b(ker(a))=0$, we have $\ker(a) \subseteq \ker(b)$, and likewise $a(\ker(b))=0$ implies $\ker(b) \subseteq \ker(a)$, so $\ker(a)=\ker(b)$.
\end{proof}


\begin{thm} Let $N$ be a right $R$-module and $M$ be a left $R$-module. Then $\Tor_n^R(N,M)\cong \mathrm{tor}_R(N,M)$. (I.e., $\bL_n(N \otimes_R - )(M) \cong \bL_n(- \otimes_R M)(N)$).
\end{thm}

\begin{proof} For $n=0$, we have $N\otimes_R M$ for both sides.

Fix projective resolutions $P_\bullet \to M$ and $Q_\bullet \to N$. We have the short exact sequences
\[ 0 \to \Omega_{i+1}(N) \to Q_i \to \Omega_i(N) \to 0\] for all $i\geq 0$. Applying $- \otimes_R P_j$ yields a short exact sequence. From the functor $- \otimes_R  \Omega_{j}(M)$ we get an exact sequence
\[ \cdots \to 0 \to \mathrm{tor}_1^R(\Omega_i(N),\Omega_j(M)) \to \Omega_{i+1}(N) \otimes_R \Omega_j(M) \to Q_i \otimes_R \Omega_j(M) \to \Omega_i(N) \otimes_R \Omega_j(M) \to 0.\]  Combining this with the short exact sequences of syzygies of $M$, we get a commutative diagram
\[ \xymatrix{ & \ker(f) \ar[r] \ar[d] & 0 \ar[d]& \ker(h) \ar[d] & \\
\ker(a) \ar[r] \ar[d] & \Omega_{i+1}(N) \otimes_R \Omega_{j+1}(M) \ar[r]^-a \ar[d]^-f & Q_i \otimes_R \Omega_{j+1}(M)  \ar[r] \ar[d] & \Omega_{i}(N) \otimes_R \Omega_{j+1}(M) \ar[r] \ar[d]^-h & 0 \\
0 \ar[r]  &  \Omega_{i+1}(N) \otimes_R P_i \ar[r] \ar[d] & Q_i \otimes_R P_i \ar[r] \ar[d] & \Omega_{i}(N) \otimes_R P_i \ar[r] \ar[d] & 0 \\
\ker(b) \ar[r]  & \Omega_{i+1}(N) \otimes_R \Omega_{j}(M) \ar[r]^-b \ar[d] & Q_i \otimes_R \Omega_{j}(M) \ar[r] \ar[d] & \Omega_{i}(N) \otimes_R \Omega_{j}(M) \ar[r] \ar[d] & 0 \\
& 0 & 0 & 0. &}\]
(One has to check the commutativity, but it is straightforward.)
For the sequence above, we see that $\ker(a) \cong \mathrm{tor}_1^R(\Omega_i(N),\Omega_{j+1}(M))$ and $\ker(b) \cong \mathrm{tor}_1^R(\Omega_i(N),\Omega_{j}(M))$. Similarly, $\ker(f) \cong \mathrm{Tor}_1^R(\Omega_{i+1}(N),\Omega_{j}(M))$ and $\ker(h) \cong \mathrm{Tor}_1^R(\Omega_{i}(N),\Omega_{j}(M))$. Thus, by the Lemma, 
\[ \mathrm{tor}_1^R(\Omega_i(N),\Omega_{j}(M)) \cong \mathrm{Tor}_1^R(\Omega_{i}(N),\Omega_{j}(M))\]
for all $i,j\geq 0$, and in particular, when $i=j=0$,
\[ \mathrm{tor}_1^R(N,M) \cong \mathrm{Tor}_1^R(N,M).\]

From the Lemma, we also have 
\[ \mathrm{tor}_1^R(\Omega_i(N),\Omega_{j+1}(M)) \cong \mathrm{Tor}_1^R(\Omega_{i+1}(N),\Omega_{j}(M))\]
for all $i,j\geq 0$. Combined with the isomorphism above, we get $\mathrm{tor}_1^R(\Omega_i(N),\Omega_{j+1}(M) \cong \mathrm{tor}_1^R(\Omega_{i+1}(N),\Omega_{j}(M)$ for all $i$.
Thus, for $n \geq1$ we have
\[\begin{aligned} \mathrm{tor}_n^R( N,M) &\cong \mathrm{tor}_1^R( \Omega_{n-1}(N), M) \cong  \mathrm{tor}_1^R( \Omega_{n-1}(N), \Omega_{0}(M)) \\  &\cong  \mathrm{tor}_1^R( \Omega_{0}(N), \Omega_{n-1}(M))) \cong  \mathrm{Tor}_1^R( \Omega_{0}(N), \Omega_{n-1}(M))) \cong  \mathrm{Tor}_n^R( N, M)).\qedhere \end{aligned}\]
\end{proof}
\end{comment}

\begin{lem} Given a commutative diagram with each row and column exact of the form
\[ \xymatrix{ & 0  \ar[d] & 0 \ar[d]& 0 \ar[d] & \\
0 \ar[r] & L' \ar[r] \ar[d] & M' \ar[r]^a \ar[d] & N' \ar[r] \ar[d] & \coker(a)\\
0 \ar[r]  & L \ar[r] \ar[d]^b & M \ar[r] \ar[d] & N \ar[r] \ar[d]^d & 0 \\
0 \ar[r]  & L'' \ar[r] \ar[d] & M'' \ar[r]^c \ar[d] & N'' \ar[r] \ar[d] & \coker(c)  \\
& \coker(b)  & 0 & \coker(d) &}\]
we have $\coker(a) \cong \coker(b)$ and $\coker(c)=\coker(d)$.
\end{lem}
\begin{proof} Since $M$ surjects onto $N$ and onto $M''$, we have \[\im(c) = \im(M \to M'' \to N'') = \im(M \to N \to N'') = \im(d),\] so $\coker(c) = \coker(d)$.

We can apply the Snake Lemma to the bottom two rows to get
\[ M' \xra{a} N'  \to \coker(b) \to 0 \]
exact, so $\coker(c) \cong \coker (b)$.
\end{proof}

\begin{thm} Let $M,N$ be $R$-modules. Then $\Ext^n_R(M,N)^I \cong \Ext^n_R(M,N)^{II}$. (I.e., $\bR^n \Hom_R(M,-)(N) \cong \bR^n(\Hom_R(-,N)(M)$).
\end{thm}

\begin{proof} For $n=0$, we have $\Hom_R(M,N)$ for both sides.

Fix a projective resolutions $P_\bullet \to M$ and  injective resolution $Q_\bullet \to N$. We have the short exact sequences
\[ 0 \to \Omega_{i+1}(M) \xra{\alpha} P_i \xra{\beta} \Omega_i(M) \to 0\] for all $i\geq 0$. Apply $\Hom_R(-,E^j)$ yields a short exact sequence
\[ 0 \to \Hom_R(\Omega_i(M),E^j) \xra{\beta^*} \Hom_R(P_i,E^j) \xra{\alpha^*} \Hom_R(\Omega_{i+1}(M),E^j) \to 0.\]
 From the functor $\Hom_R(-, \Omega^{j}(N))$ we get the long exact sequence
\[ \begin{aligned} 0 \to \Hom_R(\Omega_i(M),&\Omega^{j}(N)) \xra{\beta^*} \Hom_R(P_i,\Omega^{j}(N)) \xra{\alpha^*} \Hom_R(\Omega_{i+1}(M),\Omega^{j}(N)) \\ &\to \Ext^1_R(\Omega_i(M),\Omega^{j}(N)) \to \Ext^1_R(P_i,\Omega^{j}(N)) \to \cdots. \end{aligned}\]  
Note that the last term is zero, since $P_i$ is projective.

Now, we also have the short exact sequences
\[ 0 \to \Omega^j(N) \xra{\gamma} E^j \xra{\zeta} \Omega^{j+1}(N) \to 0,\] which yield short exact sequences
\[ 0 \to \Hom_R(P_i, \Omega^j(N)) \xra{\gamma_*} \Hom_R(P_i,E^j) \xra{\zeta_*} \Hom_R(P_i,\Omega^{j+1}(N) )\to 0\] and long exact sequences
\[ \begin{aligned} 0 \to \Hom_R(\Omega_i(M), &\Omega^j(N)) \xra{\gamma_*} \Hom_R(\Omega_i(M),E^j) \xra{\zeta_*} \Hom_R(\Omega_i(M),\Omega^{j+1}(N) ) \\&\to \Ext^1_R(\Omega_i(M),\Omega^{j}(N)) \to \Ext^1_R(\Omega_i(M),E^j)\to \cdots.\end{aligned} \]
Again, the last term here is zero.

Thus, there is a diagram with exact rows and columns: 

\[ \small \xymatrix@R=4mm@C=4mm{ & 0  \ar[d] & 0 \ar[d]& 0 \ar[d] &  &\\
0 \ar[r]  & \Hom_R(\Omega_i(M),\Omega^{j}(N)) \ar[r]^-{\beta^*} \ar[d]^-{\gamma_*} & \Hom_R(P_i,\Omega^{j}(N))  \ar[r]^-{\alpha^*}  \ar[d]^-{\gamma_*} &  \Hom_R(\Omega_{i+1}(M),\Omega^{j}(N)) \ar[r] \ar[d]^-{\gamma_*} &  \Ext^1_R(\Omega_i(M),\Omega^{j}(N))^{II} \ar[r] &0 \\
0 \ar[r]  &  \Hom_R(\Omega_i(M),E^j) \ar[r]^-{\beta^*} \ar[d]^-{\zeta_*} & \Hom_R(P_i,E^j) \ar[r]^-{\alpha^*} \ar[d]^-{\zeta_*} &  \Hom_R(\Omega_{i+1}(M),E^j) \ar[r] \ar[d]^-{\zeta_*} & 0  &\\
0\ar[r]  &  \Hom_R(\Omega_i(M),\Omega^{j+1}(N)) \ar[r]^-{\beta^*} \ar[d] & \Hom_R(P_i,\Omega^{j+1}(N))  \ar[r]^-{\alpha^*} \ar[d] & \Hom_R(\Omega_{i+1}(M),\Omega^{j+1}(N)) \ar[r] \ar[d] &  \Ext^1_R(\Omega_i(M),\Omega^{j+1}(N))^{II} \ar[r]   & 0 \\
& \Ext^1_R(\Omega_i(M),\Omega^{j}(N))^{I}\ar[d] & 0 & \Ext^1_R(\Omega_{i+1}(M),\Omega^{j}(N))^{I}\ar[d] & &\\
 & 0 & & 0 & &}\]
...
From the lemma, we see immediately that \[\Ext^1_R(\Omega_i(M),\Omega^{j}(N))^{I} \cong \Ext^1_R(\Omega_i(M),\Omega^{j}(N))^{II},\] so taking $i=j=0$, we have \[\Ext^1_R(M,N)^{I} \cong \Ext^1_R(M,N)^{II}.\]

We also have \[ \Ext^1_R(\Omega_{i+1}(M),\Omega^{j}(N))^{I} \cong \Ext^1_R(\Omega_i(M),\Omega^{j+1}(N))^{II},\]
so combined with the previous isomorphism, \[ \Ext^1_R(\Omega_{i+1}(M),\Omega^{j}(N))^{I} \cong \Ext^1_R(\Omega_i(M),\Omega^{j+1}(N))^{I},\]
and inductively
\[ \Ext^1_R(\Omega_{n-1}(M),\Omega^{0}(N))^{I} \cong \Ext^1_R(\Omega_0(M),\Omega^{n-1}(N))^{I}\]
for all $n$. Thus,
\[ \begin{aligned} \Ext^n_R(M,N)^{I} &\cong \Ext^1_R(\Omega_{n-1}(M),\Omega^{0}(N))^{I} \cong \Ext^1_R(\Omega_0(M),\Omega^{n-1}(N))^{I} & \hfill \null\\ & \cong \Ext^1_R(\Omega_0(M),\Omega^{n-1}(N))^{II} \cong \Ext^n_R(M,N)^{II}. \end{aligned}\]
