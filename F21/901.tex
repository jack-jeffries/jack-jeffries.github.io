...
\begin{ex} Suppose $R = k$ is a field. Then every short exact sequence of $R$-modules splits. We could verify any of the four equivalent conditions
  directly; I'll do (3). Given a surjection $p: V \onto V''$ of $k$-vector spaces, pick a basis $B$ of $V''$. For each $w \in B$ pick any element
  $\tilde{w}$ of $V$ such that $p(\tilde{w}) = w$. The  function $w \mapsto \tilde{w}$ from $B$ to $V$ 
extends uniquely to a $k$-linear map $j: V'' \to V$ such that $j(w) = \tilde{w}$ for all $w \in B$. The composition
  $p \circ j$ is the identity on $V''$ since it is the identity on the basis $B$ by construction.
\end{ex}
...
Given $g,h\in \Hom_{R}(A,B)$, we have \[f_*(g)(x+y)=f(g(x+y)) = f(g(x)) + f(g(y)) = f_*(g)(x) + f_*(g)(y).\]
If $R$ is commutative,
\[ f_*(g)(rx) = f(g(rx)) = r f(g(x)) = r f_*(g)(x).\]
...
Given $g,h\in \Hom_{R}(A,B)$, we have \[f^*(g)(x+y)=g(f(x+y)) = g(f(x)) + g(f(y)) = f^*(g)(x) + f^*(g)(y).\]
If $R$ is commutative,
\[ f^*(g)(rx) = g(f(rx)) = r g(f(x)) = r f^*(g)(x).\]
...
\begin{ex}
Similarly, if $F=R^{\Lambda}$ is a free module, then $\Hom_R(R^{\Lambda},M) \cong M^{\Lambda}$ by the map that sends a morphism to its tuple of values on the standard basis: as abelian groups, and as $R$-modules in the commutative case. 

We can interpret the right-hand side as the values of a functor: set $F(M) = M^{\Lambda}$, and for $f:M\to N$, set $F(f)$ to be the map given by $f$ on each coordinate. Interpreted like so, the isomorphisms form a natural isomorphism.
\end{ex}

\begin{ex} If $R^{\Gamma}$ and $R^{\Lambda}$ are free modules, then every $R$-linear homomorphism $\alpha:R^{\Gamma} \to R^{\Lambda}$ is given by left multiplication by the (possibly infinite) $\Lambda \times \Gamma$ matrix where the $\gamma$ column is the $\Lambda$-tuple $(\alpha(e_\gamma))_\lambda$.
\end{ex}


\begin{exer} Show that when $R$ is not necessarily commutative, if we give $\Hom_R(R^\Lambda,M)$ the $R$-module structure via the $(R,R)$-bimodule structure on $R^{\Lambda}$, the isomorphisms $\Hom_R(R^{\Lambda},M)\cong M^{\Lambda}$ are natrual isomorphisms of $R$-modules.
\end{exer}

\begin{ex} Let $R$ be a commutative ring, and consider the module $R/I$ for some ideal $I$. For every module $M$, there is an isomorphism $\Hom_R(R/I,M) \cong (0 :_M I)$, where $(0 :_M I)$ is the set of elements $m\in M$ such that $Im=0$.
\end{ex}

\begin{ex} For a field $K$, the functor $\Hom_K(-,K)$ is exactly the ``dual'' functor $(-)^*$.
\end{ex}
...
\sec{Exact functors and left exactness of Hom}
...
whenever
$$
M' \xra{g} M \xra{p} M'' \to 0
$$
is exact, then so is
$$
F(M') \xra{F(g)}  F(M) \xra{F(p)} F(M'') \to 0.
$$
...
whenever
$$
0 \to M' \xra{g} M \xra{p} M'' 
$$
is exact, then so is
$$
0 \to F(M') \xra{F(g)}  F(M) \xra{F(p)} F(M'').
$$

\item $F$ is {\em exact} if it is both left and right exact.
\end{itemize}
\end{defn}
...
is exact, then so is
$$
G(M') \xra{G(g)}  G(M) \xra{F(p)} G(M'') \to 0.
$$
...
is exact, then so is
$$
0 \to G(M') \xra{G(g)}  G(M) \xra{G(p)} G(M'').
$$
...
\ssec{Projective and injective modules}
