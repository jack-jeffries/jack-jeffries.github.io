...
\begin{defn}
\label{def:pushout}
A {\em pushout} of a diagram 
 $$
  \xymatrix{
    A \ar[r]^i \ar[d]^j & B \\
    C & }
  $$in a category $\cC$ is a triple $(D,f,g)$ so that the diagram below commutes
   $$
  \xymatrix{
    ...
    and satisfies the following universal property: for any other commutative diagram as above with $D$ replaced by $Y$ there is a unique dotted map  that makes the big diagram below commute
      $$
  \xymatrix{
    A \ar[r]^i \ar[d]^j & B \ar[d]_f\ar[ddr]^{f'} &\\
    C\ar[r]^g \ar[drr]_{g'}& D \ar@{-->}[dr]& \\
   & & Y.  }
   $$
\end{defn}
\begin{exer}
Show that in the category $\Mod{R}$ the pushout exists and is given (in the notation of Definition \ref{def:pushout}) as an object by \[ D=\frac{ B\bigoplus C}{\{(i(a), -j(a)) \mid a\in A \}}\] with maps $f, g$ given by the inclusions of the
two summands into $B\oplus C$ followed by the quotient map $B\oplus C\to D$.
\end{exer}
\end{comment}
...
By assumption (i.e. statement (4)), there is a map $q: M \onto  E$ such that $q \circ j = \id_M$. Define $g: N \to E$ as $g := q \circ h$.
...
$$
\xymatrix{
0 \ar[r] & J \ar[d]^f \ar[r]^\iota & R \ar@{-->}[dl]^{\exists g} \\ 
& E \\
}
$$
