...
It follows that the composition series $M_\bullet$ and $N_\bullet$ are equivalent.
...
We say a poset $(P,\leq)$ satisfies ACC if for every totally ordered subset has a maximum element.

A module has ACC or is Noetherian if every ascending chain of submodules stabilizes.

A module has DCC or is Artinian if every descending chain of submodules stabilizes.
...
\begin{defn}
Let $R$ be a ring. $R$ is left Noetherian if it is Noetherian as a left $R$-module. $R$ is left Artinian if it is Artinian as a left $R$-module.
\end{defn}

\begin{prop} Let $R$ be left Noetherian. Then a module is finitely generated if and only if it is left Noetherian. In particular, every submodule of a finitely generated module is finitely generated.
\end{prop}
