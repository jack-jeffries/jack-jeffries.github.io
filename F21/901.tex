...
\begin{enumerate}
		...
		\item If $L\subseteq M$ is a nonzero submodule and $N=M/L$, then $\ell(N)< \ell(M)$.
...
\begin{proof}
If $n := \ell(M)$, consider a strict composition series of $M$ of length $n$, say
	\[\tag{$\alpha$}  0 = M_0 \subsetneqq M_1 \subsetneqq M_2 \subsetneqq \cdots \subsetneqq M_n = M.\]
	
\begin{enumerate}
	\item Consider the filtration
	$$ 0 = M_0 \cap L \subseteq M_1 \cap L \subseteq M_2 \cap L \subseteq \cdots \subseteq M_n \cap L = L.$$
	By the Second Isomorphism Theorem, its composition factors satisfy
	$$\frac{M_{i+1} \cap L}{M_i \cap L} \cong \frac{M_{i+1} \cap L + M_i} {M_i}.$$
	This is a submodule of $M_{i+1}/M_i$, which by assumption is simple. Then either 
	$$(M_{i+1} \cap L + M_i) / M_i = 0 \quad \textrm{ or } \quad (M_{i+1} \cap L + M_i) / M_i = M_{i+1} / M_i.$$
	The quotients that are zero correspond to terms that we can delete; the remaining ones are simple modules. The resulting filtration is a strict composition series for $N$, so this shows that $\ell(L) \leqslant n$. Moreover, if there are no zero coefficients to delete, then 
	$$(M_{i+1} \cap L + M_i) / M_i = M_{i+1} / M_i$$
	for all $i$, and in particular when we take $i + 1 = n$ we obtain
	$$L + M_{n-1} = M_n \cap L + M_{n-1} = M_n = M,$$ 
	Since $M/M_{n-1}$ is simple by assumption, we must have $L = M$. This contradicts that $L$ is a proper submodule, so $\ell(L) < n$.
	
	\item Consider the filtration
	\[ 0 = \frac{M_0 + L }{L} \subseteq \frac{M_1 + L }{L} \subseteq \cdots \subseteq \frac{M_n + L }{L} = N.\]
	The factors satisfy
	\[ \frac{(M_{i+1} + L )/ L }{(M_i + L) / L} \cong \frac{M_{i+1} + L}{M_{i} + L} \cong \frac{M_{i+1} + (M_i+L)}{M_i+L}\cong \frac{M_{i+1} }{M_{i+1} \cap( M_i + L)},\] and since $M_i \subseteq M_{i+1} \cap (M_i+L)$, these are quotient modules of the simple module $M_{i+1}/M_i$, so this is a composition series. Again we discard the zero factors to get a strict series. If there are no zero factors, then $M_{i+1} \cap (M_i+L) = M_{i}$ for all $i$, so \[ \frac{M_{i+1} + L}{M_{i+1}} \cong \frac{L+M_i}{(L+M_i) \cap M_{i+1}} =\frac{L+M_i}{M_i}\]
	for all $i$, and hence $L \cong (L+M_0)/M_0 \cong (L+M_n)/M_n \cong 0$, contradicting that $L\neq 0$.
		 
	\item We proceed by induction on length again. Given a filtration of $M$, we can suppose that there is some nonzero proper submodule $L$ in the filtration, since otherwise we could just take any composition series. Then $L$ and $M/L$ has length less than $M$. The filtration up to $L$ can be refined to a strict composition series by the induction hypothesis, and the filtration from $L$ to $M$ taken mod $L$ can be refined to a strict composition series for $M/L$; pulling back we get the strict composition series we want.
	
	
	\item We show by induction on $n$ that for any module of length $n$, all of its strict composition series are equivalent.
	Assume that $\ell(M)=n$. If $n=1$, the claim is clear.
	Suppose that
	$$0 = N_0 \subsetneqq N_1 \subsetneqq \cdots \subsetneqq N_k = M$$
	is another strict composition series for $M$, so $k\geq n$. If $N_k=M_{n-1}$, then the two composition series we have for $M_{n-1}$ are equivalent by induction, so the two given series are equivalent. If $N_k \neq M_{n-1}$, set $K=N_k \cap M_{n-1}$. Since $M/M_{n-1}$ is simple, we must have $N_{k-1}+M_{n-1} = M$. By the second isomorphism theorem, we then have
	\[\frac{M}{M_{n-1}} = \frac{M_{n-1}+N_{k-1}}{M_{n-1}} \cong \frac{N_{k-1}}{K}\]
	and similarly $M/N_{k-1} \cong M_{n-1} / K$, and both of these modules are simple.
	Fix a strict composition series for $K$:
$$0 = K_0 \subsetneqq K_1 \subsetneqq \cdots \subsetneqq K_m = K$$
and extend to a strict series for $M_{n-1}$:
$$0 = K_0 \subsetneqq K_1 \subsetneqq \cdots \subsetneqq K_m = K \subsetneqq M_{n-1}.$$
Since we also have the composition series 
$$0 = M_0 \subsetneqq M_1 \subsetneqq M_2 \subsetneqq \cdots \subsetneqq M_{n-1}$$
of length $n-1$, we must have that $m=n-2$ and these two series are equivalent. Now, 
$$0 = K_0 \subsetneqq K_1 \subsetneqq \cdots \subsetneqq K_{n-2} = K \subsetneqq N_{k-1}$$
is a strict composition series, so $k=n$, and this is equivalent to 
	$$0 = N_0 \subsetneqq N_1 \subsetneqq \cdots \subsetneqq N_{k-1}.$$
	Thus, altogether, the factors in the two series agree.
	\qedhere
	\end{enumerate}
\end{proof}
