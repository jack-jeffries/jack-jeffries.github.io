...
\begin{ex} As an important special case of the previous example, if $R^{\Gamma}$ and $R^{\Lambda}$ are free modules, then every $R$-linear homomorphism $\alpha:R^{\Gamma} \to R^{\Lambda}$ is given by left multiplication by the (possibly infinite) $\Lambda \times \Gamma$ matrix where the $\gamma$ column is the $\Lambda$-tuple $(\alpha(e_\gamma))_\lambda$.
\end{ex}
...
Then if the top row is exact, $G(i)=\theta^{-1} F(i) \theta'$ is injective, $G(p)=(\theta'') ^{-1} F(p) \theta$ is surjective, and \[x\in \ker G(p) \iff \theta^{-1} (x) \in \ker F(p) \iff \theta^{-1} (x) \in \im F(i) \iff x\in \im G(i).\]
...
\begin{ex} Let $M$ be a finitely presented $R$-module with presentation
...
We have $\Hom_R(R^n,R)$ is free with free basis given by the coordinate functions; likewise for $\Hom_R(R^m,R)$. In these bases, the map $\Hom_R(A\cdot, R)$ is given by $A^T$, so we get a left exact sequence
