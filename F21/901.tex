...
%
 ...
 \begin{prop}
 \label{prop:homotopy}
  Homotopic chain maps induced the same map on homology: If $f$ and $g$ are chain maps from $(M_\bullet, d^M)$ to $(N_\bullet, d^N)$ and they are homotopic,
...
If $f \htpy g$, then $f- g$ is null-homotopic, so that $H_i(f-g) = 0$, by what we just proved. Since $H_i$ is additive, 
  we have $0 = H_i(f-g) = H_i(f) - H_i(g)$. 
   ...
   $$
   \xymatrix{
     \cdots \ar[r] & 0 \ar[r]\ar[d] &   0 \ar[d] \ar[r] &  \langle 2 \rangle  \Z/4 \ar[r] \ar[d]^{inc} & 0 \ar[r] \ar[d]& \cdots \\
     \cdots \ar[r] & 0 \ar[r] &   \Z/4 \ar[r]^{2} &  \Z/4 \ar[r] & 0 \ar[r] & \cdots \\
   }
   $$
   induces the $0$ map on all homology groups, but it  is not null homotopic. Indeed, the only possible homotopy would
   be $0$ in all degrees except one, in which it would be a map $h_0: \langle 2  \rangle \to \Z/4$. The only possibilities for $h_0$
   are the $0$ map and the inclusion map. Neither works.
 \end{ex}
 ...
 \begin{rem}
 Homotopy is an equivalence relation on chain maps. Moreover, if $f \htpy g$, then $f\circ h \htpy g \circ h$ and $h\circ f \htpy h \circ g$, whenever these make sense.
 \end{rem}
