...
$$
\cdots \to 0 \to R \xra{\begin{bmatrix} -y \\ x \end{bmatrix}} R^2 \xra{(x,y)} R \to R/(x,y) \to 0.
$$
It follows that $\Tor_*^R(M, R/(x,y))$ is the homology of the complex
$$
\cdots \to 0 \to M \xra{\begin{bmatrix} -y \\ x \end{bmatrix}} M^{\oplus 2} \xra{(x,y)} M \to 0 \to \cdots.
$$
...
\begin{defn} For a pair of left $R$-modules $M$ and $N$, we write
  $$
  \Ext^j_R(M, N)^I = \R^j  \Hom_R(M, -)(N)
  $$
  where $F$ in Definition \ref{def:rderI} is taken to be the left-exact covariant functor $F := \Hom_R(M, -)$. 
  
We define
  $$
  \Ext^j_R(M, N)^{II} = \R^j  \Hom_R(-,N)(M)
  $$
  where $G$ in Definition \ref{def:rderII} is taken to be the left exact contravariant functor $G := \Hom_R(-,N)$. 
  
 Both $\Ext^j_R(M, N)^I$ and $\Ext^j_R(M, N)^{II}$ are abelian groups in general and $R$-modules when $R$ is commutative. 
\end{defn}

\begin{defn}
By the balancedness of $\Ext$, which I will formally state later, there is a canonical isomorphism
$\Ext_R^j(M,N)^{I} \cong \Ext_R^j(M,N)^{II}$. Then one just writes 
...
\begin{ex} Let's compute $\Ext^*_\Z(\Z/m, \Z/n)^I$ and $\Ext^*_\Z(\Z/m, \Z/n)^{II}$.

    For the latter, we start with the free resolution $\cdots 0 \to \Z \xra{m} \Z \to \Z/m \to 0$ of $\Z/m$ and apply $\Hom_\Z(-, \Z/n)$ to obtain
    $$
    \cdots \to 0 \to \Hom_\Z(\Z, \Z/n) \xra{m}\Hom_\Z(\Z, \Z/n) \to 0
    $$
    which is isomorphic to
    $$
    \cdots \to 0 \to \Z/n \xra{m}  \Z/n \to 0.
    $$
    The two non-zero homology modules are both isomorphic to $\Z/g$ where $g = gcd(m,n)$. So
    \[
\Ext^i_\Z(\Z/m, \Z/n)^{II} \cong  
  \begin{cases}
\Z/g & i=0,1\\
0 & i\geq 2.
\end{cases}
\]
    ...
    $$
    This holds since $\Q/\Z$ is divisible and the kernel of multiplicaition by $j$ is $\{\overline{\frac{i}{j}} \mid 0 \leq i \leq j-1\}$,
...
It follows that 
$$
\Hom_\Z(\Z/m, \Q/\Z) \cong \Hom_\Z(\Z/m\Z, \Z/m\Z) \cong \Z/m\Z
$$
and that the previous complex is isomorphic to
$$
\cdots \to    0 \to \Z/m \xra{n} \Z/m \to 0 \to \cdots
$$
This gives
\[
\Ext^i_\Z(\Z/m, \Z/n)^I \cong  
  \begin{cases}
\Z/g & i=0,1\\
0 & i\geq 2.
\end{cases}
