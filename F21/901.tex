...
In the proof, we will use the following observations.
...
\begin{proof}
  ...
  &=& \End_R(M_i^{\oplus n_i})\cong \Mat_{n_i}(\End_R(M_i)).
\end{eqnarray*}
...
Say we are given an isomorphism of rings $R \cong \prod_i \Mat_{t_i}(Q_i)$. Then since $\Mat_{t_i}(Q_i)$ decomposes as a direct sum of $t_i$ copies of $N_i:=Q_i^{t_i}$, and $N_i$ is a simple $\Mat_{t_i}(Q_i)$-module, hence also a simple $R$-module, we have a semisimple $R$-module decomposition of $R$ as
...
\[  \End_R(N_i)^{\op} = \End_{\Mat_{n_i}(Q_i)} (Q_i^{n_i}) ^{\op}= Q_i,\]
