...
\[ R^{\Gamma} \to R^{\Lambda} \to M \to 0\] is a right exact sequence, where the standard basis of $R^{\Lambda}$ maps to $\{m_\lambda\}$ and the standard basis of $R^{\Gamma}$ maps to $\{ (r_\lambda)_\gamma\}$. Conversely, a right exact sequence of the form
\[ R^{\Gamma} \to R^{\Lambda} \to M \to 0\] 
...
\begin{ex}
Similarly, if $F=R^{\Lambda}$ is a free module, then $\Hom_R(R^{\Lambda},M) \cong M^{\oplus \Lambda}$, where $M^{\oplus \Lambda}= \bigoplus_{\lambda\in \Lambda} M$ by the map that sends a morphism to its tuple of values on the standard basis: as abelian groups, and as $R$-modules in the commutative case. 

We can interpret the right-hand side as the values of a functor: set $F(M) = M^{\oplus \Lambda}$, and for $f:M\to N$, set $F(f)$ to be the map given by $f$ on each coordinate. Interpreted like so, the isomorphisms again form a natural isomorphism.
\end{ex}

\begin{ex} As an important special case of the previous example, if $R$ is commutative, and $R^{\Gamma}$ and $R^{\Lambda}$ are free modules, then every $R$-linear homomorphism $\alpha:R^{\Gamma} \to R^{\Lambda}$ is given by left multiplication by the (possibly infinite) $\Lambda \times \Gamma$ matrix where the $\gamma$ column is the $\Lambda$-tuple $(\alpha(e_\gamma))_\lambda$.
\end{ex}


\begin{exer} Show that when $R$ is not necessarily commutative, if we give $\Hom_R(R^\Lambda,M)$ the $R$-module structure via the $(R,R)$-bimodule structure on $R^{\Lambda}$, the isomorphisms $\Hom_R(R^{\Lambda},M)\cong M^{\Lambda}$ are natural isomorphisms of $R$-modules.
\end{exer}
...
\item $\ker(i^*)\subseteq\im(p^*)$: Let $g\in \Hom_R(B,M)$ be in the kernel of $i^*$, so $g\circ i=0$. Thus, as $g|_{\im(i)} = 0$, we obtain a well-defined map $\overline{g}:B/\im(i) \to M$. Note that, since $p$ is surjective, it induces an isomorphism $\overline{p}:B/\im(i) = B/\ker(p) \to C$, so there is a map $j:C\to B/\ker(p)$ such that $ j \circ p$ is the quotient map $B\to B/\im(i)$. Set $f=\overline{g} \circ j$. We then have $p^*(f)(b) = \overline{g} \circ j \circ p(b) = \overline{g}(\overline{b})=g(b)$, so $p^*(f) = g$. Thus $g\in \im(p^*)$.\qedhere\end{itemize}
\end{enumerate}
\end{proof}

We can use these exactness statements to compute various Hom modules. 
...
\[
\xymatrix{ M\times N \ar[r]^f \ar[d]^h& A \\
M\otimes_R N  \ar@{-->}[ur]_{\exists ! g}
}
\]
\end{defn}
...
\begin{ex} I claim $\Z/m \otimes_\Z \Z/n \cong \Z/g$ where $g = gcd(m,n)$.
\end{ex}
...
Define a function
$$
b: \Z/m \times \Z/n \to \Z/g
$$ 
by $b(\overline{i}, \overline{j}) = \overline{ij}$. It is not hard to see that $b$ is well-defined (exercise!) 
and $\Z$-biadditive.
By the Theorem, it therefore induces a homomorphism of abelian groups
$$
\a: \Z/m \otimes_\Z \Z/n \to \Z/g
$$ 
such that $\a(\overline{i} \otimes \overline{j}) = \overline{ij}$. 

Now define a homomorphism $\phi: \Z \to \Z/m \otimes_\Z \Z/n$ by sending $1$ to $1 \otimes 1$. Notice that 
$$
...
$$
\beta = \overline{\phi}: \Z/g \to \Z/m \otimes_\Z \Z/n
$$
...
A typical element of  $\Z/m \times \Z/n$ has the form $\sum_t \overline{i_t} \otimes \overline{j_t}$. We have 
$$
...
We can also realize tensor as a covariant functor.

\begin{defn} Let $R$ be a ring and $M$ be a right $R$-module. There is a covariant functor \index{$M\otimes_R -$}
\[M\otimes_R -: \Mod{R} \to \Ab\] that on objects sends $N$ to $M\otimes_R N$, and on morphisms sends $f:N\to N'$ to the map \index{$1_M\otimes f$}\[1_M \otimes f : M\otimes_R N \to M\otimes_R N'\] that is given by the rule\[ (1_M \otimes_R f )(\sum_i m_i \otimes n_i) = \sum_i m_i \otimes f(n_i).\]
...
\end{defn}
\begin{proof}
We check first that $1_M\otimes f$ is well-defined. There is an $R$-balanced bilinear map $a:M\times N \to M\otimes_R N'$ given by $a(m,n)=m\otimes f(n)$ (easy to check), which induces a well-defined map of the stated form. It is straightforward to check that this map has the desired linearity properties, and behaves well with compositions/identities.
\end{proof}

Of course, there is an equally valid functor $-\otimes_R N: \Mod{R^{\op}} \to \Ab$.
