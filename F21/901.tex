...
\[H_i(-):\Cx{R} \to \Mod{R}.\]
Recall that this means $H_i(f\circ g) = H_i(f) \circ H_i(g)$ and $H_i(\id) =
  \id$.
    \end{lem}
 \begin{proof}
...
\begin{proof} One needs to show:

$\bullet$ well-definedness of $i|$ and $p|$, specifically the fact that the images of these maps land in $\ker(f)$ and $\ker(f'')$ respectively.

To show this for $i|$, consider $u\in \ker(f')$. Then $i|(u)=i(u)$ and $f(i(u))=j(f'(u))=j(0)=0$ by the commutativity of the given diagram. Thus $i|(u)\in \ker(f)$ as desired.

$\bullet$ well-definedness of $\ov{j}$ and $\ov{q}$, specifically independence of coset representative.

To show this for $\ov{j}$, consider $n-\tilde{n}\in \im(f)$. Then we have $q(n)-q(\tilde{n})=q(n-\tilde{n})\in q(\im(f))=f''(\im(p))\subseteq \im(f'')$ yields that $\ov{q}(n + \im(f))=q(n) + \im(f'')=q(\tilde(n))+  \im(f'')=\ov{q}(\tilde{n} + \im(f))$.

$\bullet$ well-definedness of $\partial$
...
So, we have proven $\partial$ is a well-defined function. The fact that it is $R$-linear is proven similarly. 

$\bullet$ $\im(p|) = \ker(\partial)$:
...
If $\partial(m'')  = 0$, then in the construction $n' = f'(m')$ for some $m'$. Then
$p(m - i(m')) = p(m) = m''$ and $f(m - i(m')) = f(m) - jf'(m') = 0$, which proves that $m'' \in \im(p|)$. This proves the other containment.

$\bullet$ $\im(\partial)=\ker\bar{j}$: 

For $m''\in M''$, write $\partial(m'')=n'+\im(f')$; then $j(n')=n\in \im(f)$, so $\im(\partial)\subseteq \ker\bar{j}$. If $\bar{j}(n' + \im(f') )=0$, then $j(n')\in \im(f)$, so write $n=j(n') = f(m)$. Then $f''(p(m))=q(f(m))=q(j(n'))=0$, so $p(m)\in \ker(f'')$, and $\partial(p(m))=n'+\im(f)$ by definition, which proves that $n'+\im(f)\in \im(\partial)$.

The ``moreover" part of the statement is clear since $i|$ and $\ov{q}$ are obtained from $i$ and $q$ respectively.
\end{proof}
