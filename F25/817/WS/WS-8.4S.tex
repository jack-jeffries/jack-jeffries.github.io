...
\item If $I$ is an ideal, then $I[x] \colonequals \{ a_0 + a_1 x + \cdots + a_n x^n \ | \ a_i \in I\}$ is an ideal of $R[x]$.
\end{enumerate}
\end{framed}
\footnotetext[1]{This means that for all $\alpha,\beta\in A$, either $I_\alpha\subseteq I_\beta$ or $I_\beta\subseteq I_\alpha$.}
	...
	\solution{Since $I\neq \varnothing$, take $a\in I$, and we have $0=0 \cdot a \in I$. To complete the two-step test, we just need to check that $I$ is closed under additive inverses: give $a\in I$, we have $-a = (-1)a\in I$.}
	\itema Very quickly explain why  $\{0\}$ and $R$ are ideals of $R$. We say that an ideal is \textbf{nontrivial} if $I\neq 0$ and proper if $I\neq R$.
	\solution{$\{0\}$ is closed under addition since $0+0=0$ and absorbs multiplication since $r \cdot 0 = 0 = 0 \cdot r$ for all $r\in R$. $R$ is closed under addition and absorbs multiplication since addition and multiplication are operations on $R$.}
	\itema Explain why an ideal $I\subseteq R$ is proper if and only if $1\notin I$.
	\solution{If $1\notin I$, then clearly $I\neq R$. If $1\in I$, then for any $r\in R$, $r=1\cdot r\in I$.}
	\itema Quickly explain why ``ideal,'' ``left ideal,'' and ``right ideal'' are identical notions in a commutative ring.
		\solution{Absorbs left multiplication is the same as absorbs multiplication (or right multiplication).}
	\end{enumerate} 
	...
	\solution{It is a left ideal since 
	\[ \begin{bmatrix} c & d \\ e & f\end{bmatrix} \begin{bmatrix} a & 0 \\ b & 0 \end{bmatrix} = \begin{bmatrix} ca+db & 0 \\ ea+fb & 0 \end{bmatrix}\] is in the subset, but not an ideal since, for example,
		\[ \begin{bmatrix} 1 & 0 \\ 0 & 0 \end{bmatrix} \begin{bmatrix} 0 & 1 \\ 0 & 0\end{bmatrix} =\begin{bmatrix} 0 & 1 \\ 0 & 0\end{bmatrix}\] is not.}
	
	\itemB Prove parts (i) and (v) of Lemma 1.
...
\
	...
	\itemB Prove parts (ii)--(iv) of Lemma 1.
...
\solution{We have $(2) = \{ 2 (a_0 + a_1 x + \cdots + a_n x^n)\} = \{ 2 a_0 + 2 a_1 + \cdots + 2 a_n x^n\}$.}
\itema $(x)$ is the set of all integer polynomials with zero constant term.
\solution{We have $(x) = \{ x (a_0 + a_1 x + \cdots + a_n x^n)\} = \{ a_0 x  + \cdots + a_n x^{n+1} \} = \{ b_1 x  + \cdots + b_{n} x^{n} \}$.}
\itema $(2,x)$ is the set of all integer polynomials with even constant term.
\solution{For $f\in (2,x)$ we can write $f=2g+xh$; since $2g$ has even constant term and $xh$ has constant term zero, $f$ has even constant term. Conversely, given $f= 2a_0 + a_1 x + \cdots + a_n x^n$, we can write $f= 2 a_0 + x (a_1 + \cdots + a_n x^{n-1}) \in (2,x)$.}
\end{enumerate}
