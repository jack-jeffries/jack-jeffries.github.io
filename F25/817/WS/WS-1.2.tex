...
\solution{ $1\mapsto 4$, $2\mapsto 2$, $3\mapsto 1$, $4\mapsto 3$, $5\mapsto 5$.}
\itema Write out the product of cycles $(1 \, 3 \, 5) (2 \,5)$ explicitly as a function by listing the input and output values.
\solution{ $1\mapsto 3$, $2\mapsto 1$, $3\mapsto 5$, $4\mapsto 4$, $5\mapsto 2$.}
...
\item $(3 \, 4 \, 1 \, 5)$
\end{itemize}
\solution{The first and the last.}
\itema What is the inverse of $(1 \, 5 \, 3 \, 4)$? How would you find the inverse of a cycle in general?
\solution{$(1\, 4 \, 3\, 5)$. The inverse of a cycle is the reverse cycle.}
\itema What is the \emph{order}\footnote{Recall that the \textbf{order} of an element $g$ in a group $G$ is the least integer $n>0$ such that $g^n=e$ if some such $n$ exists, else~$\infty$.} of $(1 \, 5 \, 3 \, 4)$? How would you find the order of a cycle in general?
\solution{The order of $(1\, 5\, 3\, 4)$ is four. The order of an $n$-cycle is $n$.}
...
\itemB Show\footnote{Hint: To show that two functions are the same, show they have the same values. Compute what each side does to $i_j$, and what it does to an element of $[n]$ that is not an $i_j$.} the following \textsc{Lemma:} For any distinct $i_1,\dots,i_p\in [n]$,
...
\item Look at the smallest element of $i\in [n]$ that hasn't appeared, and repeat with $i$ in place of $1$.
...
\itemb Prove part (1) of Theorem 1.
\itemb Prove\footnote{Hint: Let $\sigma = \tau_1 \cdots\tau_m$ with $\tau_i$ disjoint cycles, and $j\in [n]$. Then $j$ appears in at most one $\tau_i$. Show that, for such $i$,  $\sigma^k(j) = \tau_i^k(j)$ and use this to solve for $\tau_i$.} part (3) of Theorem 1.
\end{enumerate}
...
\solution{A transposition has sign $-1$ by definition. A $3$-cycle can be written as a product of two transpositions, so its sign is $1$. By the Lemma, a $p$-cycle can be written as a product of $p-1$ tranpositions, so its sign is $(-1)^{p-1}$.}
\itema If the cycle type of $\sigma$ is $m_1,m_2,\dots,m_t$, then what is the sign of $\sigma$?
\solution{Using the previous part, the sign is $(-1)^{(m_1 -1) + (m_2-1) + \cdots + (m_t-1)}$.} 
\end{enumerate}
...
\itemb By way of contradiction, suppose that there exists
\[ \tag{$\dagger$} (a_1 \ b_1) (a_2 \ b_2) \cdots (a_m \ b_m) = e \qquad \text{with $m$ odd.}\]
(Here $a_i \neq b_i$ but $a_i = a_j$ or $a_i=b_j$ is allowed.) Explain why, if an example of ($\dagger$) exists, then there is a ($\dagger$) with 
 \begin{itemize}
 \item the smallest value of $m$, among all ($\dagger$)'s
 \item among all ($\dagger$)'s where $m$ is minimal, the number $t$ of times that $a_1$ appears is minimal. 
 \end{itemize}
\itemb Show that $t=1$ is impossible,  and that\footnote{Hint: Use the identities $(cd)(ab) = (ab)(cd)$ and $(bc)(ab) = (ac)(bc)$.} if $t\geq 2$, one can find another expression with the same value of $m$ and $t$  and also $a_1=a_2$. Complete the proof.
\end{enumerate}
