...
On the other hand, in $G$, any expression involving $u$ and $v$ can be rewritten by replacing $v^2u$ by $uv$, so that any element can be written as $u^av^b$ for some integers $a$ and $b$. Since $u^7 = e$ and $v^9 = e$, any element in $G$ can then be written as
