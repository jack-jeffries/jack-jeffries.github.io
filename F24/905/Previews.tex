...
\subsection{Localization of modules \S5.2} \0
...
The \textbf{localization} $W^{-1}M$ is the $W^{-1}R$-module\footnote{If $0\in W$, then $W^{-1}R=0$ is not a ring.} with
\begin{itemize}
...
\noindent \textsc{Lemma:} Let $R$ be a ring, $W$ a multiplicatively closed set. Let $M$ be a finitely presented\footnote{This means that $M$ admits a finite generating set for which the module of relations is also finitely generated.}
