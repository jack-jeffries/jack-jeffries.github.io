...
\item Show that \[\frac{S}{\p(\X) S + \q(\Y)S + (X_1-Y_1,\dots,X_n-Y_n)}\cong \frac{R}{\p+\q}.\] Deduce\footnote{This is the punchline! {\color{meretale} Gift: you can use without proof that $\p(\X) S + \q(\Y)S$ is prime. This is not obvious---and may fail if $K$ is not algebraically closed---but can be shown along similar lines to (a).}} that if $\p+\q$ is proper, then  $\hgt(\p+\q) \leq \hgt(\p) + \hgt(\q)$.
