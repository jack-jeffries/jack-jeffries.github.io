...
Let $S$ be a subset of $\R$. Let $f: S \to \R$ be a function, and $a$ and $L$ be real numbers. We say that \textbf{the limit of $f$ as $x$ approaches $a$ is $L$}\index{limit} provided:
...
\end{cases}
$$
is not continuous at $x=3$. Since the domain of $f$ is all of $\R$, the negation of the definition of ``continuous at $3$'' is:
\begin{quote}
...
However, the function $g(x)$ with domain $\R$ defined by
\[ f(x) = \begin{cases}
2x - 7 & \text{ if $x \geq 3$ and } \\
2 -x  & \text{ if $x < 3$} \\
\end{cases}\]
is continuous at $x=3$. Indeed, let $\e>0$. Take $\de=\e/2$. Then if $|x-3|<\de$, $f(x)$ is defined since the domain is all real numbers, and
\[ | f(x) - f(3) | = \begin{cases} |2x - 7 - (-1)| & \text{ if $x \geq 3$,} \\
| 2-x - (-1)| & \text{ if $x < 3$}.
\end{cases}
= \begin{cases} |2x -6| & \text{ if $x \geq 3$,} \\
| 3-x| & \text{ if $x < 3$}.
\end{cases}
= \begin{cases} 2 |x -3| & \text{ if $x \geq 3$,} \\
| x-3| & \text{ if $x < 3$} .
\end{cases}\]
In either case, this is at most $2|x-3| < 2\de = \e$.

This shows that $f$ is continuous at $x=3$.
\end{ex}
