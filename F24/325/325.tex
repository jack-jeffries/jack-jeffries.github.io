...
\usepackage[top=0.9in, bottom=0.9in, left=1.2in, right=1.2in]{geometry}
\xyoption{all}
\xyoption{arc}
\usepackage{hyperref}
...
You might not have thought about it before, but an expression of the form $\frac{m}{n}$ is really an ``equivalence class'': 
...
\noindent\textsc{Theorem~30.3. (Intermediate Value Theorem):}\index{Intermediate Value Theorem}  Let $a<b$ and $f(x)$ be a function that is continuous on the closed interval $[a,b]$. If $y$ is any real number between $f(a)$ and $f(b)$, then there is some $c\in [a,b]$ such that $f(c)=y$. More precisely, if $f(a) \leq y \leq f(b)$ or $f(b) \leq y \leq f(a)$, then there is some $c\in [a,b]$ such that $f(c)=y$.
\end{framed}
...
\begin{framed} 
 ...
 \noindent \textsc{Definition 34.1:}  Let $f$ be a function and $r$ be a real number. We say that $f$ is \textbf{differentiable at $r$}\index{differentiable} if $f$ is defined at $r$ and the limit
\[ \lim_{x\to r} \frac{ f(x) - f(r) }{x-r}\]
exists. In this case, we call the limit \textbf{the derivative of $f$ at $r$}\index{derivative} and write $f'(r)$\index{$f'(r)$} for this limit. 
...
\begin{framed} 
 ...
 \noindent \textsc{Theorem 36.1 (Chain Rule):} \index{Chain rule}  
...
\begin{framed}
False: Take $f(x)=(x-7) - 2 \cdot 10^7(x-7)^2$. Then $f'(7) = 1$, but $f(7+10^{-7}) =10^{-7} - 2 10^7* 10^{-14} = -10^{-7}<0 = f(7)$.
\end{framed}
...
\begin{framed} 
 ...
 \noindent \textsc{Theorem 39.1 (Rolle's Theorem):}\index{Rolle's Theorem}  Let $f$ be continuous on the closed interval $[a,b]$ and differentiable at every point of $(a,b)$. If $f(a) = f(b)$, then there exists a $c \in
  (a,b)$ such that $f'(c) = 0$.
  
  
  \
  
 \noindent \textsc{Theorem 39.2 (Mean Value Theorem):}\index{Mean Value Theorem} Let $f$ be a function that is continuous on the closed interval $[a,b]$ and differentiable on $(a,b)$. Then there exists some $c\in (a,b)$ such that
...
\begin{itemize}
\item $f$ is \textbf{increasing}\index{increasing} on $S$ if for any $a,b\in S$ with $a<b$ we have $f(a) \leq f(b)$;
\item $f$ is \textbf{decreasing}\index{decreasing} on $S$ if for any $a,b\in S$ with $a<b$ we have $f(a) \geq f(b)$;
\item $f$ is \textbf{constant}\index{constant} on $S$ if for any $a,b\in S$ with $a<b$ we have $f(a) = f(b)$;
\item  $f$ is \textbf{strictly increasing}\index{strictly increasing} on $S$ if for any $a,b\in S$ with $a<b$ we have $f(a) < f(b)$;
\item $f$ is \textbf{strictly decreasing}\index{strictly decreasing} on $S$ if for any $a,b\in S$ with $a<b$ we have $f(a) > f(b)$.
\end{itemize}
