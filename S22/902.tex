...
\section{Minimal primes and support}


\begin{definition}
	The primes that contain $I$ and are minimal with the property of containing $I$ are called the {\bf minimal primes}\index{minimal prime} of $I$. That is, the minimal primes of $I$ are the minimal elements of $V(I)$. We write $\Min(I)$\index{$\Min(I)$} for this set. 
\end{definition}
...
As a special case, the nilpotent elements of a ring $R$ are exactly the elements in every minimal prime of $R$, or equivalently, in every minimal prime of the ideal $(0)$. The radical of $(0)$ is often called the {\bf nilradical} of $R$, denoted $\mathcal{N}(R)$.\index{nilradical}\index{$\mathcal{N}(R)$}
...
\begin{proposition}
	Let $R$ be a ring, and $X=\Spec(R)$.
	 If $R$ is Noetherian, the topology on $X$ can by recovered from the poset structure by the rule
		$$Y \subseteq X \text{is closed} \Longleftrightarrow  Y=\{ \q \in X \ | \ \p_i \subseteq \q \ \text{for some} \ i\} \textrm{ for some } \p_1,\dots,\p_n\in X.$$

\end{proposition}

\begin{proof}
f $Y$ is closed, we have $Y=V(I)= V(\sqrt{I}) = V(\p_1\cap \cdots \cap \p_n) = V(\p_1) \cup \cdots \cup V(\p_n)$. For the converse, we can work backwards.
\end{proof}
...
\begin{definition}
	If $M$ is an $R$-module, the {\bf support}\index{support} of $M$\index{$\Supp(M)$} is 
...
\begin{example}
	...
	On one hand, we claim that $\Supp(M) = \{(x)\}$. Indeed, any element of $M$ is killed by a large power of $x$, so $W^{-1}M=0$ whenever $x \in W$, so $\Supp(M) \subseteq \{ (x)\}$. We will soon see that the support of a nonzero module is nonempty, and thus $\Supp(M) = \{(x)\}$.
...
\begin{proof}
	Localization is exact, by ?, so for any $\p$,  
	$$\xymatrix{0 \ar[r] & L_\p \ar[r] & M_\p \ar[r] & N_\p \ar[r] & 0}$$ 
	is exact. If $\p\in \Supp(L) \cup \Supp(N)$, then $L_\p$ or $N_\p$ is nonzero, so $M_\p$ must be nonzero as well. On the other hand, if $\p \notin \Supp(L) \cup \Supp(N)$, then $L_\p=N_\p=0$, so $M_\p=0$.
\end{proof}

\begin{remark}
	As a corollary, $\Supp(L) \subseteq \Supp (M)$ for any submodule $L$ of $M$.
...
\begin{proof}
\item The implications $\Rightarrow$ are clear. To show the last implies the first, we show the contrapositive. If $m\neq 0$, consider $Rm \subseteq M$. By ?, there is a maximal ideal in $\Supp(Rm)$, and by ? applied to the inclusion $Rm \subseteq M$, this maximal ideal is in $\Supp(M)$ as well. \qedhere
\end{proof}

So we conclude that $\Supp(M) \neq \varnothing$ for any $R$-module $M \neq 0$.
