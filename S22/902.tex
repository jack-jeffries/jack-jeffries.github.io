...
\begin{remark} The ring $\gr_I(R)$ is $\NN$-graded with zeroth degree piece $R/I$ and is generated as an $R/I$ algebra by the elements in degree one; namely if $I=(f_1,f_2,\dots)$, then $\gr_I(R) = R/I [ \overline{f_1 t} ,\overline{f_2 t}, \dots]$. In particular, if $R$ is Noetherian, then $\gr_I(R)$ is a finitely generated standard graded $R/I$-algebra.
\end{remark}

\begin{definition}
	Let $I$ be an ideal in a ring $R$ and $M$ an $R$-module. A filtration on $M$ is \emph{$I$-stable}\index{$I$-stable} if it is consistent with the standard $I$-adic filtration on $R$ and there is some $N$ such that $I M_n=M_{n+1}$ for all $n\geq N$. 
\end{definition}

\begin{ex} The filtration $M_n = I^n M$ is $I$-stable; this is called the \DEF{standard $I$-filtration} \emph{on $M$}.
\end{ex}
...
\[ N_n := M_0 \oplus M_1 t \oplus \cdots \oplus M_n t^n \oplus  I M_{n+1} t^{n+1} \oplus  I^2 M_{n+2} t^{n+2} \oplus \cdots.\]
...
The proof of (1) implies (3) is similar to (1) implies (2).
\end{proof}
...
\[I (M_n \cap N) \subseteq I M_n \cap I N \subseteq M_{n+1} \cap IN \subseteq M_{n+1} \cap N,\]
 so we can consider the Rees module $\mathcal{R}(N_{\bullet}) \subseteq \mathcal{R}(M_{\bullet})$. By the previous lemma, $\mathcal{R}(M_{\bullet})$ is finitely genreated and since $\mathcal{R}(I)$ is Noetherian, so is $\mathcal{R}(N_{\bullet})$, and hence by the lemma, $N_{\bullet}$ is $I$-stable.
	\end{proof}
...
\end{thm}
\begin{proof} Note that it is not clear that $\dim(\gr_{R,\m})=\dim(R)$. We need to show that $\deg(\tilde{H}_R(t)) = \dim(R)$.
