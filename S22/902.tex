...
Likewise, polynomial rings over $\Z$ are regular.

\begin{prop} Any maximal ideal $\m$ of $\Z[x_1,\dots,x_n]$ can be generated by $n+1$ elements. In particular, $\Z[x_1,\dots,x_n]$ is regular.
\end{prop}
\begin{proof}
From a homework problem, any maximal ideal $\m$ contains some prime integer $p$. Then the quotient map $\Z[x_1,\dots,x_n] \to \Z[x_1,\dots,x_n]/\m$ factors through $\F_p[x_1,\dots,x_n]$, and the kernel of the latter surjection can be generated by $n$ elements by the previous proposition; we obtain a set of $n+1$ generators for the maximal ideal. 
We have \[\htt(\m) = \dim(\Z[x_1,\dots,x_n]_\m) = \dim(\F_p[x_1,\dots,x_n]_{\overline{\m}}) +1 = n +1,\]
since the image $\overline{\m}$ of $\m$ is a maximal ideal in $\F_p[x_1,\dots,x_n]$. Thus $\Z[x_1,\dots,x_n]_\m$ is regular.
\end{proof}
...
\begin{exer}
	If $(R,\m)$ is a local ring, and $f_1,\dots,f_t$ is a regular sequence, then $f_{\sigma(1)},\dots,f_{\sigma(t)}$ is a regular sequence for any $\sigma\in \Sigma_t$.
...
\begin{theorem} Let $(R,\m,k)$ be a local ring and $M$ be a finitely generated $R$-module. If $f_1,\dots,f_t$ is a maximal regular sequence on $M$ in the sense that there is no $f_{t+1}$ such that $f_1,\dots,f_t,f_{t+1}$ is a regular sequence on $M$, then $t=\min\{ n \ | \ \Ext^n_R(k,M)\neq 0\}$.
...
\begin{cor} If $(R,\m)$ is a local ring, $M$ is a finitely generated $R$-module, and $x_1,\dots,x_a$ is a regular sequence on $M$, then $\depth(M/(x_1,\dots,x_a)M) = \depth(M)-a$.
\end{cor}

\begin{proof} Extend $x_1,\dots,x_a$ to a maximal regular sequence $x_1,\dots,x_d$ on $M$, which has $d=\depth(M)$ by the theorem. Then $x_{a+1},\dots,x_d$ is a maximal regular sequence on $M/(x_1,\dots,x_a)M$, so the depth of this module is $d-a$.
\end{proof}
...
\begin{example}
	\begin{enumerate}
		...
		\item The depth of $K[x,y,z]_{(x,y,z)}/(x^2-yz)$ is two. $y,z$ is a regular sequence, and there can be no longer, for dimension reasons.
	\end{enumerate} 
\end{example}
...
\begin{proof}
	...
	Take $z$ such that $Rz\subseteq M$ is a maximal element in the set $\{ Ra \subseteq M \ | \ \p a =0\}$. We claim that $z\notin xM$. If not, $z=xz'$ for some $z'\in M$. Note that, for $p\in \p$, $pz=pxz'=0$, and $x$ a nonzerodivisor implies $pz'=0$, so $\p z'=0$, and $Rz \subsetneqq Rz'$, a contradiction. We note also that $\p+(x) \subseteq \ann(\overline{z})$. 
...
\begin{definition} A Noetherian local ring is \emph{Cohen-Macaulay}\index{Cohen-Macaulay} if $\depth(R)=\dim(R)$.
	A finitely generated module over a Noetherian local ring is a \emph{Cohen-Macaulay module} if $\depth(M)=\dim(M)$. It is a \emph{maximal Cohen-Macaulay module} if $\depth(M)=\dim(M)=\dim(R)$.
\end{definition}

One nice consequence of the Cohen-Macaulay property is unmixedness.

\begin{proposition} If $M$ is a Cohen-Macaulay $R$-module, then ${\depth(M)=\dim(R/\p)}$ for every ${\p\in \Ass(M)}$. In particular, if $R$ is a Cohen-Macaulay ring, then $R$ has no embedded primes, and ${\dim(R/\p)=\dim(R)}$ for each $\p\in \Min(R)$.
\end{proposition}
\begin{proof}
...
\begin{proposition} Let $f_1,\dots,f_t$ be a sequence of elements in a Noetherian local ring $(R,\m)$. For the conditions
	...
	\end{enumerate}
	we have (1)$\Rightarrow$(2)$\Rightarrow$(3)$\Rightarrow$(4) in general, and if $R$ is Cohen-Macaulay, (4)$\Rightarrow$(1), so all of the above are equivalent.
\end{proposition}
\begin{proof}
...
\begin{example}
	\begin{enumerate}
		...
		\item The ring $\displaystyle\frac{K[x,y]_{(x,y)}}{(x^2,xy)}$ is \emph{not} Cohen-Macaulay, since it has an associated prime of dimension zero, although the ring is dimension one. 
		\item The ring $\displaystyle\frac{K[x,y]_{(x,y,z)}}{(xy,xz)}$ is \emph{not} Cohen-Macaulay since it has minimal (a fortiori, associated) primes of different dimensions.
		\item $\displaystyle\frac{K[x,y]_{(x,y)}}{(xy)}$ is Cohen-Macaulay by example \#2 above. We can check by showing an SOP is a regular sequence. The element $x-y$ is a SOP, since the quotient is $K[x]/(x^2)$, which is zero-dimensional. This element is also a nonzerodivisor on the ring: any element in the ring can be represented by $f(x,y)= a + xb(x) + yc(y)$. $(x-y)f(x,y)= ax - ay + x^2 b(x) - y^2 c(y) = 0$ in $R$ implies $f(x,y)=0$. Thus, $R$ is CM.
		\item $\displaystyle{K[x^4,x^3y,xy^3,y^4]_{(x^4,x^3y,xy^3,y^4)}}$ is not CM. An SOP is $x^4,y^4$, but $y^4$ is a zerodivisor mod $(x^4)$, since $y^4 \cdot (x^3y)^2 = x^4 \cdot (xy^3)^2 \in (x^4)$, but $(x^3y)^2 \notin (x^4)$, since $x^2y^2\notin R$.
		\item $\displaystyle \frac{K[X_{2\times 3}]_{X}}{I_2(X)}$ is Cohen-Macaulay. A SOP is $x_{11}, x_{21}-x_{12},x_{31}-x_{22},x_{23}$; check that it is a regular sequence!
	\end{enumerate}
\end{example}
...
\end{lemma}
\begin{proof} Let $f_1,\dots,f_t$ be a maximal regular sequence of elements in $I$, and set $J=(f_1,\dots,f_t)\subseteq I$. Then every element of $I$ is a zerodivisor modulo $J$, so $I$ is contained in an associated prime $\p$ of $J$ by prime avoidance. But every associated prime of $R/J$ is minimal, and every minimal prime of $J$ has height at most $t$ by Krull height, so $\htt(I) \leq \htt(\p)\leq t$. On the other hand, $\htt(I) \geq \htt(J) = t$, using the proposition on regular sequences above.
\end{proof}
...
\begin{definition} A Noetherian local ring that may or may not be local is \emph{Cohen-Macaulay} if $R_{\m}$ is Cohen-Macaulay for all $\m\in \Max(R)$. Equivalently, $R$ is Cohen-Macaulay if $R_{\p}$ is Cohen-Macaulay for all $\p\in \Spec(R)$.
\end{definition}
...
\end{theorem}
\begin{proof}
	$R_{\q}$ is CM local of dimension $\mathrm{height}(\q)$. Pick $h=\height(\p)$ elements $r_1,\dots,r_h$ in $\p$. These form a regular sequence, so $R_{\q}/(r_1,\dots,r_h) R_{\q}$ is CM, and $\dim(R_{\q}/(r_1,\dots,r_h) R_{\q})=\dim(R_{\q} / \p R_{\q})$, since $\p R_{\q}\in \Ass(R_{\q}/(r_1,\dots,r_h) R_{\q})$. On the other hand, $\dim(R_{\q}/(r_1,\dots,r_h) R_{\q}) = \dim(R_{\q})-h$. The equality follows.
\end{proof}

\begin{theorem} Let $(R,\m)\subseteq (S,\mathfrak{n})$ be a module-finite local inclusion of Noetherian local rings, with $R$ regular. Then $S$ is Cohen-Macaulay if and only if $S$ is a free $R$-module.
\end{theorem}
\begin{proof}
...
%\begin{remark} By the Cohen structure theorem, every complete local ring $S$ is module-finite over a regular local ring.
%\end{remark}
