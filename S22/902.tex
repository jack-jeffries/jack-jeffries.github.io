...
\begin{definition}
	Let $M$ be an $R$-module. An element $r \in R$ is a {\bf zerodivisor}\index{zerodivisors} on $M$ if $rm = 0$ for some $m \in M$. We sometimes write the set of zerodivisors of $M$ as $\mathcal{Z}(M)$.\index{$\mathcal{Z}(M)$}
...
\begin{lemma}\label{associated primes nonzero}\label{zerodivisors associated primes}
	...
	\begin{enumerate}
		...
		\item $\bigcup\limits_{\p \in \Ass(M)} \p = \mathcal{Z}(M)$.
	\end{enumerate}
...
Associated primes also localize.
...
\begin{corollary}
	...
	Consequently,
	\begin{itemize}
		\item $\Ass_R(M)$ is finite.
		\item If $M$ is graded, $\Ass_R(M)$ is a finite set of homogeneous primes.
	\end{itemize} 
\end{corollary}
\begin{proof}
For each $i$, we have $\Ass(M_i) \subseteq \Ass(M_{{i-1}})\cup \Ass(M_i/M_{i-1}) = \Ass(M_{{i-1}}) \cup \{\p_i\}$ so that, inductively, $\Ass(M_i)\subseteq\{\p_1,\dots,\p_i\}$. The consequences follow from the previous theorem.
\end{proof}
