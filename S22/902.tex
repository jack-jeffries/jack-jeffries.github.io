...
of $M$ with $M_i/M_{i-1} \cong R/\p_i$, we have $\dim(M) = \max\{\dim(R/\p_i) \ | \ i=1,\dots,m\}.$
\end{lem}
\begin{proof}
...
\item If $f(t)$ is eventually equal to $p(t)$ and $F(t)$ is eventually equal to $P(t)$, then $\deg(P) = \deg(p)+1$, and the leading coefficient of $P$ over $\deg(P)!$  equals the leading coefficient of $p$ over $\deg(p)!$.
\end{enumerate}
\end{lemma}
\begin{proof}
 First, we note that if we set $c_n(t)=\binom{t}{n-1}$, which is a polynomial of degree $n$, the functions $\{c_0,\dots,c_j\}$ form a basis for the set of polynomials of degree at most $j$. Two basic binomial identities read as
 \[ c_n(t) - c_{n}(t-1) = c_{n-1}(t) \]
 \[ c_n(0) + c_n(1) + \cdots + c_n(t) = c_{n+1}(t).\]
...
If for $t\geq N$, $f(t)$ is equal to a polynomial $p(t)$ of degree $n$ and leading coefficient $a$, then write \[p(t) = n! a c_n(t) + b_{n-1} c_{n-1}(t) + \cdots + b_0 c_0(t).\]
 ...
 \[ \begin{aligned} f(t) &= F(t) - F(t-1) \\&= \left((n+1)! a c_{n+1}(t) + b_{n} c_{n}(t) + \cdots + b_0 c_0(t)\right) -  \left((n+1)! a c_{n+1}(t-1) + b_{n} c_{n}(t-1) + \cdots + b_0 c_0(t-1)\right) \\&=
  (n+1)! a c_n(t) +b_{n} c_{n-1 }(t) + \cdots + b_1 c_0(t)
 \end{aligned}\]
 which is a polynomial of degree $n$ and leading coefficient $\displaystyle \frac{(n+1)! a}{n!}$.
\end{proof}

\begin{corollary}
Let $K$ be a field, $R$ be a standard graded finitely generated $K$-algebra, and $M$ be a nonzero finitely generated $\ZZ$-graded $R$-module. Then the cumulative Hilbert function $\widetilde{H}_M(t)$ eventually agrees with a polynomial of degree $\dim(M)$, and $\dim(M)!$ times the leading coefficient of the polynomial is a positive integer, namely, the multiplicity of $M$.
\end{corollary}
...
\begin{definition}
	...
	An \emph{ideal filtration}\index{ideal filtration} is a ring $R$ equipped with a descending sequence of ideals 
	\[ R \supseteq I_1 \supseteq I_2 \supseteq \cdots \]
	such that $I_i I_j \subseteq I_{i+j}$.
	We may write $I_{\bullet}$\index{$I_{\bullet}$} to indicate the filtration.
	
	A \emph{module filtration compatible with $I_{\bullet}$}\index{module filtration} is an $R$-module $M$ with a filtration
	\[ M =M_0 \supseteq M_1 \supseteq M_2 \supseteq \cdots \]
	such that $I_i M_j \subseteq M_{j+{i}}$ for all $i,j\in \NN$.
\end{definition}

Observe that an ideal filtration or a module filtration is a special case of a filtered abelian group.
...
\begin{definition}
	...
	\[ \mathcal{R}(I_{\bullet}) := \bigoplus_{i\geq 0} I_i t^i \subseteq R[t]\]
	and a \emph{Rees module}\index{Rees module}\index{$\mathcal{R}(M_{\bullet})$}
	 	\[ \mathcal{R}(M_{\bullet}) := \bigoplus_{i\geq 0} M_i t^i \subseteq M[t].\]
\end{definition}

\begin{example} For a ring $R$ and ideal $I$, the key example of the above is the usual Rees ring $\mathcal{R}(I)=R[It]$\index{$\mathcal{R}(I)$} coming from the filtration given by powers of $I$.
\end{example}

The conditions $I_i I_j \subseteq I_{i+j}$ and $I_i M_j \subseteq M_{j+{i}}$ justify that $\mathcal{R}(I_{\bullet})$ is a ring and that $\mathcal{R}(M_{\bullet})$ is a $\mathcal{R}(I_{\bullet})$-module.
...
Let $I_{\bullet}$ be an ideal filtration on a ring $R$, and $M$ an $R$-module with a compatible filtration $M_{\bullet}$. Then there is an \emph{associated graded ring}\index{associated graded ring of a filtration}\index{$\gr(I_{\bullet})$}
	\[ \gr(I_{\bullet}) := \bigoplus_{i\geq 0} I_i/I_{i+1} t^i \]
	and an \emph{associate graded module}\index{associate graded module}\index{$\gr(M_{\bullet})$}
	 	\[ \gr(M_{\bullet}) := \bigoplus_{i\geq 0} M_i/M_{i+1} t^i.\]
\end{definition}

\begin{example} For a ring $R$ and ideal $I$, the key example of the above is the usual associated graded ring $\gr_I(R)$ \index{$\gr_I(R)$} coming from the filtration given by powers of $I$.
\end{example}

\begin{definition}
	Let $I$ be an ideal in a ring $R$ and $M$ an $R$-module. A filtration on $M$ is \emph{$I$-stable}\index{$I$-stable} if it is consistent with the standard $I$-adic filtration on $R$ and there is some $n$ such that $I M_m=M_{m+1}$ for all $m\geq n$. 
\end{definition}


\begin{lemma} Let $M$ be a finitely generated $R$-module. Let $M_{\bullet} = \{M_i\}$ be an $I$-consistent filtration of submodules of $M$. Consider the conditions
\begin{enumerate}
...
\begin{remark}
	Let $M_{\bullet}$ be a module with a filtration that is consistent with the standard $I$-adic filtration. Then $\mathcal{R}(M_{\bullet})$ is generated by elements of degree $\leq n$ for some $n$ if and only if $M_{\bullet}$ is $I$-stable.
\end{remark}


\begin{theorem}[Artin-Rees Lemma]
	Let $R$ be a Noetherian ring, $N\subseteq M$ finitely generated modules, and $I$ an ideal. If $\{M_i\}$ is an $I$-stable filtration of $M$, then $\{M_i \cap N\}$ is an $I$-stable filtration of $N$.
\end{theorem}
\begin{proof}
	Since $R$ is Noetherian, $I$ is a finitely generated ideal, and the Rees ring $\mathcal{R}(I_{\bullet})$ is Noetherian since it is generated as an $R$-algebra by generators of $I$ times $t$. Now, $\mathcal{R}(M_{\bullet})$ is a finitely generated $\mathcal{R}(I_{\bullet})$-module by the previous remark, since it is generated in bounded degrees, and in each degree, it is finitely generated. Now, $I(N \cap M_i)  \subseteq IN \cap I M_i \subseteq N \cap IM_{i+1}$, so $\widetilde{N}=\bigoplus (N \cap M_i) t^i \subseteq N[t]$ is an $\mathcal{R}(I_{\bullet})$-module. But, $\widetilde{N}$ is a submodule of $\mathcal{R}(M_{\bullet})$. By Noetherianity, it is finitely generated, so we are done.
\end{proof}
...
\begin{lem} Two stable filtrations are shift-equivalent.
\end{lem}
