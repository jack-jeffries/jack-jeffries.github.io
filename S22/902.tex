...
We take our inspriation from the fundamental setting of dimension theory: vector spaces. The notion of basis doesn't make sense for varieties (What does it mean to span? Where is zero?), but one relevant thing we do have for both vector spaces and for varieties is subobjects.
...
\begin{definition}
 A ring is {\em catenary}\index{catenary} if for every pair of primes $\q \supseteq \p$ in $R$, every saturated chain of primes $\p=\p_0 \subsetneq \p_1  \subsetneq \cdots \subsetneqq \p_n=\q $
	 has the same length.
...
It is difficult to come up with examples of rings that are not catenary, but they do exist. Nagata (1956) gave the first example of a Noetherian non-catenary ring in the paper {\em On the chain problem on prime ideals}.

\begin{defn}
A  ring $R$ is said to be equidimensional if it satisfies both the following conditions:
\begin{itemize}
\item All its maximal ideals have the same height
\item All its minimal prime ideals have the same dimension i.e. if  $P_1, P_2 \in\Min(R)$ then $\dim(R/P_1)=\dim(R/P_2)$
\end{itemize}
\end{defn}
...
%\end{proof}
 ...
 \begin{ex}
 Here is an example which shows that {\bf the inequality $\dim(R/I) + \mathrm{height}(I) \leq \dim(R)$ in part 6 of Proposition \ref{rk:dimprop} can fail to be an equality}.
 ...
 The ring $R$ in this example is a catenary domain.  Notice that there are maximal ideals of distinct heights in this ring, for example the ideal $\p$ given above is a prime of height 1 whereas and another maximal ideal $\m=(2,x)$ has height 2. Thus this ring is not equidimensional.
...
We give a related definition of dimension for modules, but first we remark on the geometric intuition behind this. In the same way as we visualize the ring $R$ by means of the topological space $\Spec(R)$ (or more concretely we visualize affine rings $R=K[x_1,\ldots,x_n]/I$ as the coordinate ring of the algebraic zet $Z(I)$), we visualize modules $M$ by means of their support, which is a subset of $\Spec(R)$.
...
\begin{defn}
Let $\phi:R\to S$ be a ring homomorphism and $\p\in \Spec(R)$ be a prime ideal. We define the \emph{fiber ring}\index{fiber ring}\index{\kappa_{\phi}(\p)} of $\phi$ over $\p$ as
\[ \kappa_{\varphi}(\p) = (R\smallsetminus \p)^{-1}(S/\p S),\]
where, by abuse of notation, we write $R\smallsetminus \p$ for the image of $R\smallsetminus \p$ in $S/\p S$ via $\phi$ and the quotient map $S\to S/\p S$  (and $\p S$ for $\phi(\p) S$ as usual).

As a special case, for the identity map, we simply write $\kappa(\p)$. That is, $\kappa(\p) = R_{\p} / \p R_\p$.
\end{defn}

The point of this definition is the following.

\begin{lemma} Let $\phi:R\to S$ be a ring homomorphism and $\p\in \Spec(R)$ be a prime ideal. The natural map $S \to \kappa_{\phi}(\p)$ induces a homeomorphism (in particular, an order-preserving bijection)
\[ \Spec( \kappa_{\phi}(\p) ) \cong \{ \q \in \Spec(S) \ | \ \phi^*(\q) = \p \},\]
where the right-hand side obtains the subspace topology from $\Spec(S)$.
\end{lemma}
\begin{proof}
First, we note that for any localization map or any quotient map, the induced map on Spec is a homeomorphism onto its image (exercise!). Write $S\xra{\alpha} S/\p S \xra{\beta} (R\smallsetminus \p)^{-1}(S/\p S)$. We have \[ \im(\beta^*)=\{ \overline{\q} \in \Spec(S/\p S) \ | \ \overline{\q} \cap \overline{\phi(R \smallsetminus \p)} = \varnothing\} .\]
Write $\overline{\phi}$ for the induced map from $R/\p \to S/\p S$. We have 
\[\overline{\q} \cap \overline{\phi(R \smallsetminus \p)}= \varnothing \ \Leftrightarrow \
\overline{\q} \cap \overline{\phi}(R/\p \smallsetminus 0_{R/\p})= \varnothing \ \Leftrightarrow \
 \overline{\phi}^{-1}(\overline{\q})\subseteq 0_{R/\p} \  \Leftrightarrow \
 \overline{\phi}^{-1}(\overline{\q})= 0_{R/\p}. \]
Under the map $\alpha$, such a prime $\overline{q}$ corresponds to a prime $\q$ in $S$ that contains $\p S$, so $\p \subseteq \phi^{-1}(\q)$; the condition above translates to $\phi^{-1}(\q) = \p$.
\end{proof}
...
\end{corollary}
\begin{proof} From the homework, we know that $I S \cap R = I$ for all ideals in this case.
\end{proof}
...
%Define local ring, local homom.

%\begin{lemma}
% Let $(R,\m) \to (S,\n)$ be a flat and local homomorphism.
%\begin{enumerate}
%\item If an $R$-module $M\neq 0$, then $M \otimes_R S\neq 0$.
%\item $R\to S$ is injective.
%\end{enumerate}
%\end{lemma}
%\begin{proof}
%\begin{enumerate}
%\item
%Let $m\in M$ be nonzero, so we have an injection $R/\ann_R(m) \cong Rm \subseteq M$, and $\ann_R(m) \subseteq \m$. By flatness, we have $S/\ann_R(m)S \cong S(1\otimes m) \subseteq S\otimes_R M$, and since $\ann_R(m)S \subseteq \m S \subseteq \n$, this is nonzero.
%\item Let $I=\ker(R \to S)$. for any ideal $I$ of $R$, we have $I\otimes_R S\cong IS$ by flatness (exercise!). But, if $I\neq 0$, then $0\neq I\otimes_R S = IS = 0$, a contradiction.\qedhere
%\end{enumerate}
%\end{proof}
...
\begin{corollary}
If $R \to S$ is integral, and $S$ is Noetherian, then for any $\p\in \Spec(R)$, only finitely many primes contract to $\p$. If, moverover, $S=\sum_{i=1}^t R s_i$ is generated as a module by $t$ elements, then for any prime of $R$, at most $t$ primes of $S$ map to $\p$.
\end{corollary}
\begin{proof}
...
\end{corollary}
\begin{proof}
	Given a chain of primes $\a_0 \subsetneqq \cdots \subsetneqq \a_n = \q$ in $\Spec(S)$, we can contract to $R$, and we get a chain of distinct primes in $\Spec(R)$, so the height of the latter is at least as big.
\end{proof}
...
\end{theorem}
\begin{proof} Consider the map $R/\p \to S/\q$. This is integral, as we observed above. It is also injective, so lying over applies. Thus, there is a prime $\a$ of $S/\q$ that contracts to the prime $\p'/\p$ in $\Spec(R/\p)$. We can write $\a=\q'/\q$ for some $\q'\in \Spec(S)$, and we must have that $\q'$ contracts to $\p'$.
\end{proof}
