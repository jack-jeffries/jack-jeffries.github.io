...
\begin{comment}
\begin{proof}
For (1), let $\fm$ be an element of the LHS, and consider $\pi: R \to R/\fm \cong K$. We then have that $\pi(x_i) = a_i$ for some $a_i\in K$, so $(x_1-a_1,\dots,x_n-a_n) \subseteq \ker(\pi)=\fm$, and this ideal is maximal, so equality holds. Then $\cZ (\fm) = \{ (a_1,\dots,a_n)\}$, so $\cZ$ gives a well-defined map from left to right. From the last theorem, we see that for any point, $\cI$ yields an ideal of this form so $\cI$ restricts to a well-defined map here. It is easy to see these are mutually inverse.

For (2), we note that there is a bijection between maximal ideals of $K[X]$ and maximal ideals of $K[x_1,\dots,x_n]$ that contain $\cI(X)$; moreover $\cI(X) \subseteq \fm$ if and only if $\cZ(\fm) \in \cZ(\cI(X))=X$. The statement follows.
\end{proof}
