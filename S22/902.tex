...
\begin{theorem}[Macaulay's unmixedness theorem]
	Let $R$ be Cohen-Macaulay. If $I=(f_1,\dots,f_r)$ has height $r$, then every associated prime of $I$ has height $r$, and hence is minimal. In particular, this holds true in a polynomial ring.
\end{theorem}
...
\begin{prop} Let either
 \begin{enumerate}
 \item $(R,\m,k) \subseteq (S,\mathfrak{n},\ell)$ be a module-finite inclusion of Noetherian local rings, with $R$ regular, or
...
\begin{proof}
	...
	We show the other direction by induction on the dimension of $R$ (= dimension of $S$). If $R$ has dimension zero, all its modules are free, and any module-finite $S$ has dimension zero and is Cohen-Macaulay. For the general case, assume that $x_1,\dots,x_d$ form a regular sequence on $S$, and fix a minimal generating set for $S$ as an $R$-module, $s_1,\dots,s_t$; we want to show that any $R$-linear relation on these is trivial. Note that by hypothesis the images of $s_1,\dots,s_t$ in $S/\m S$ are linearly independent over $k$.
	
	If we have a nontrivial relation $r_1 s_1 + \cdots + r_t s_t=0$, then there is a nontrivial relation where not all of the $r_i$'s lie in $(x_1)$. Indeed, given such a relation, we could cancel off $x_1$ from each $r_i$ and get another relation, since $x_1$ is a nonzerodivisor. By the Krull Intersection Theorem, if $r_i\neq 0$, there is some $N$ such that $r_i\notin \m^N$, so $r_i \notin (x_1)^N$, so we can cancel off $x_1$ finitely many times to get the relation we seek.
	
	On the other hand, the images of $s_1,\dots,s_t$ in $S/x_1 S$ form a minimal generating set for $S/x_1 S$ as a $R/x_1 R$-module, since their images are linearly independent in $(S/x_1 S)/(\m/x_1R)\cong S/\m S$ over $k$. Since $R/x_1 R$ is regular, and the images of $x_2,\dots,x_{d}$ form a regular sequence on $S/x_1 S$, the induction hypothesis yields that our relation is trivial in $R/ x_1 R$. That is, all of the $r_i$'s must lie in $(x_1)$. Thus, any such relation must be trivial. We conclude that $S$ is free over $R$.
	
	The proof in the graded case is the essentially the same.
\end{proof}
...
\end{proof}
 ...
 \begin{theorem} Let $K$ be an infinite field, $R=K[x_1,\dots,x_n]$ be a polynomial ring over $K$, and $G$ be a finite group with $|G|\in K^\times$ acting on by degree-preserving $K$-algebra automorphisms. Then $R^G$ is Cohen-Macaulay.
 \end{theorem}
 ...
 \begin{theorem}  Let $K$ be an infinite field, $R=K[x_1,\dots,x_n]$ be a polynomial ring over $K$, and $G$ be a finite group with $|G|\in K^\times$ acting on by degree-preserving $K$-algebra automorphisms. Then there exist sets of \DEF{primary invariants} $f_1,\dots,f_n\in R^G$ and \DEF{secondary invariants} $h_1,\dots,h_m\in R^G$ such that $f_1,\dots, f_n$ are algebraically independent and $R^G= \bigoplus_{i=1}^m K[f_1,\dots, f_n] h_i$. Namely, every invariant $f\in R^G$ has a unique expression of the form
 \[ f = a_1(f_1,\dots,f_n) h_1 + \cdots + a_m(f_1,\dots, f_n) h_m,\]
 for some (uniquely determined) tuple $a_1,\dots,a_m \in K[t_1,\dots,t_n]$.
 \end{theorem}
 
 \begin{ex} Let $K=\C$ and $G=C_3=\langle g \rangle$ act on $K[x,y]$ by $g \cdot x= \omega x$, $g\cdot y=\omega y$. Then 
 \[R^G= \C[x^3,x^2y,xy^2,y^3] \cong \frac{\C[a,b,c,d]}{(b^2-ac,c^2-bd,ad-bc)}.\]
 We can take a Noether normalization $\C[x^3,y^3]$ and a free basis $\{1,x^2y,xy^2\}$, so
 \[ R^G= \C[x^3,y^3] 1 \oplus  \C[x^3,y^3] x^2y \oplus  \C[x^3,y^3] xy^2.\]
 Concretely, for any invariant $f$, there is a unique triple of polynomials $f_1,f_2,f_3\in \C[t_1,t_2]$ such that 
 \[ f = f_1(x^3,y^3) + f_2(x^3,y^3) x^2 y + f_3(x^3,y^3) x y^2.\]
 \end{ex}
