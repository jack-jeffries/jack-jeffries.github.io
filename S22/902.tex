...
\begin{example}[Negative variables]\label{rootofunityinvariants}
	...
	Write $f= a_n x^n + a_{n-1} x^{n-1} + \cdots + a_0$.  We have $g(x^i)=(-x)^i = (-1)^i x^i$, so
	\[ g(f) = (-1)^n a_n x^n+ (-1)^{n-1} a_{n-1} x^{n-1} +  \cdots  + a_0, \]
	which differs from $f$ unless for each odd $i$, $a_i=0$. That is,
	\[ R^G = \{ f \in R \ | \ \text{every term of $f$ has even degree}\}.\]
	Any such $f$ is a polynomial in $x^2$, so we have
	\[ R^G = K[x^2].\]
	
	This computation readily generalizes to the case where $G=\{e,g\}$ acts on $K[x_1,\dots,x_n]$ by negating all of the variables. This yields the subring generated by all monomials of degree two.
		 \end{example}
		 
		 \begin{exer} Verify that the ring of invariants described above is indeed generated by all of the monomials of degree two.
		 \end{exer}
...
Suppose $X$ is a subvariety of $\A^m_K$ and $Y$ is a subvariety of $\A^n_K$. A {\em morphism of varieties}\index{morphism of varieites} or  {\em algebraic map}\index{algebraic map} or {\em regular map}\index{regular map} from $X$ to $Y$ is a set-theoretic function $G: X \to Y$ defined coordinatewise by polynomials $g_1, \cdots, g_n\in K[x_1, \dots, x_m]$, that is 
$$
G(a_1, \dots, a_m) = (g_1(a_1, \dots, a_m), \dots, g_n(a_1, \dots, a_m)) \text{ for all }\va\in X.
$$
...
Two algebraic subsets $X$ and $Y$ are isomorphic if there are algebraic maps $G: X \to Y$ and $H: Y \to
X$ such that $G \circ H = \id_Y$ and $H \circ G = \id_X$, in which
case each of $G$ an $H$ is referred to as an {\em isomorphism} of
algebraic sets.  
\end{defn}
...
and define $G: X \to \A^1_K$ by $G(a,b) = a$. Then $G$ is an algebraic map (indeed, it's given by a linear polynomial) and its image is $\A^1_K\setminus \{0\}$, which is {\em not} an algebraic subset of $\A^1_K$. So,  the set-theoretic  image of a morphism of varieties  need
  not be a variety.
...
Define
$$
G: \A^1_K \to Y \quad G(t) = (t^2, t^3).
$$
$G$ is an algebraic map from $\A^1_K$ to $Y$
since the component functions are polynomial functions of $t$ and $(t^3)^2 - (t^2)^3 = 0$ for all $t$.

Note that $G$ is a bijection of sets. However, it is not an isomorphism. Indeed, if it were, we would have some map $H$ such that $H\circ G = \mathrm{id}_{\A^1_K}$. This $H$ would be given by a polynomial $h$ in two variables such that $h(t^2,t^3) = t$. It is easy to see that no such $h$ exists.
...
Then an algebraic map $G:X\to \A^1_K$ is simply a polynomial $G=f\in K[x_1,\ldots, x_n]$. Two such polynomials $f, g$ give the same map $G$ if they agree on $X$, that is if $f-g\in \cI(X)$.
...
\begin{prop}
Let $K$ be infinite. Given $\phi: X \to Y$, $\ker(\phi^*) = \cI(\im \phi) K[Y]$.
\end{prop}
\begin{proof}
$(\subseteq):$ Let $f(y_1,\dots,y_n) \in K[Y]$ be an element of $\ker(\phi^*)$ and $(b_1,\dots,b_n) = \phi(a_1,\dots,a_m) \in \im \phi$. Then $\phi^*(f) =0$ implies $(f\circ \phi)$ is the zero function on $X$, so $f(b_1,\dots,b_n) = f\circ \phi (a_1,\dots,a_m) =  0$. 


$(\supseteq):$ Let $f\in K[Y]$ be such that $f\in \cI(\im \phi) K[Y]$. Then $f(b_1,\dots,b_n) = 0$ for all $(b_1,\dots,b_n)\in \im \phi$. This means that $f \circ \phi (a_1,\dots,a_n)=0$ for all $(a_1,\dots,a_m)\in X$. Thus, $f\circ \phi $ is zero in $K[X]$, which means that $f\in \ker(\phi^*)$.
\end{proof}
...
\item If $X=X_1\cup \cdots X_n$ with each $X_i$ irreducible, and suppose that $X_i \not\subseteq \bigcup_{j\neq i} X_j$; i.e., the union $X=X_1\cup \cdots X_n$ is \DEF{irredundant}. Then $\{X_1,\dots,X_n\}$ is the collection of irreducible components of $X$.
...
\begin{proof}
\begin{enumerate}
\item It suffices to show that any point is in an irreducible component. Let $x\in X$, and consider the collection of irreducible subsets of $X$ that contain $x$. This is nonempty, since $\{x\}$ is in the collection.  Consider the poset of irreducible subsets of $X$ containing $x$ under inclusion. By the exercise above, the union of any chain in this poset is again in the poset, so Zorn's lemma applies. Such a subset must be a maximal irreducible subset, since any larger subset also contains $x$.
...
\item Consider the collection of closed subsets of $X$ that are not finite unions of irreducibles. If this collection is nonempty (in particular, if $X$ is not a finite union of irreducibles), it has a minimal element by DCC; call it $Z$. In particular, $Z$ is reducible, so write $Z=Z'\cup Z''$ with $Z', Z''$ closed proper subsets of $Z$. Since they are smaller than $Z$, they are finite unions of irreducibles. Putting them together expresses $Z$ as a finite union of irreducibles.
\end{enumerate}
\end{proof}
