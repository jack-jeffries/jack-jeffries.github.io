...
\begin{ex} Note that the first inequality can fail in general. Let $\displaystyle R=\frac{K[x,y,z]}{(xy,xz,x^2-x)}$. This ring has dimension two: $A=K[y,z]$ is a Noether normalization, as $x^2-x=0$ makes $x$ integral over $A$, and no nonzero polynomial in $y,z$ withing $K[x,y,z]$ can belong to the ideal $(x) \supseteq (xy,xz,x^2-x)$. However, $\displaystyle R/(x-1) \cong \frac{K[x,y,z]}{(y,z,x-1)}\cong K$ has dimension zero.
\end{ex}
