...
\begin{proof}
	...
	If $d>0$, consider $x \in \m \smallsetminus \left( \m^2 \cup \bigcup_{\mathfrak{p} \in \Min R} \mathfrak{p} \right)$; such $x$ exists by prime avoidance. By NAK, $\m/xR$ is generated by $d-1$ elements, so that $\dim R/xR \leqslant d-1$ by Krull's height theorem. Then $\dim R/xR = d-1$, and $R/xR$ is regular. We want to show that $0$ is prime. By the induction hypothesis, $R/xR$ is a domain, and thus $xR$ is a prime ideal in $R$ that is not minimal. Take a prime ideal $0 \subseteq \mathfrak{p} \subseteq xR$. If $y \in \mathfrak{p}$, we can write $y =ax$ for some $a \in R$, and if $x \notin \mathfrak{p}$, then $a \in \mathfrak{p}$. Thus $x \mathfrak{p} = \mathfrak{p}$, which by NAK implies that $\mathfrak{p} = 0$. Thus $R$ is a domain.
\end{proof}
...
\begin{example} Some DVRs are $K[x]_{(x)}$, $K[x,y]_{(y-x^2)}$, $\ZZ_{(p)}$, and $\frac{K[x,y,u,v]_{(x,y)}}{(xu-yv)}$. For the last, note that $y=x(u/v)$.
\end{example}
...
\begin{proof}
	...
	Take $z$ such that $Rz\subseteq M$ is a maximal element in the set $\{ Ra \subseteq M \ | \ \p (Ra) =0\}$. We claim that $\overline{z}\in M/(x)M$ is nonzero. If not, $z=xz'$ for some $z'\in M$. Note that, for $p\in \p$, $pz=pxz'=0$, and $x$ a nonzerodivisor implies $pz'=0$, so $\p z'=0$, and $Rz \subsetneqq Rz'$, a contradiction. We note also that $\p+(x) \subseteq \ann(\overline{z})$. 
	
	Since $\p+(x)$ annihilates a nonzero element of $M/xM$, there is an associated prime $\q$ of $M/xM$ containing this ideal. In particular $\q \supsetneq \p$.	Now, using the inductive hypothesis, $\depth(M)-1 = \depth(M/xM)= \leq \dim(R/\q) \leq \dim(R/\p) -1$, and the theorem follows.
\end{proof}
...
\begin{remark} Any regular ring is Cohen-Macaulay. 
\end{remark}
