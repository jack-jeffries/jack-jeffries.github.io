...
\begin{proof}
 ...
 We can apply the Artin-Tate Lemma to $K\subseteq K' \subseteq L$ to see that $K'$ is algebra-finite over $K$. In particular, there are $f_i, g_i$ in the polynomial ring $K[x_1,\dots,x_c]$ such that $K'=K[\frac{f_1}{g_1},\dots,\frac{f_c}{g_c}]$. This implies that any element of $K'$ can be written as a fraction with denominator $(g_1\cdots g_c)^n$ for some $n$.  The element $\frac{1}{g_1\cdots g_c +1}\in K'$ cannot be written this way; if so, we would have 
	...
	for some $v$ with $g_1\cdots g_c \nmid v$ (since the polynomial ring is a UFD). But, the equation $g_1\cdots g_c v + v = (g_1\cdots g_c)^n$ contradicts this.
...
We show that if $J \subset K[x_1, \dots, x_n]$ is a proper ideal, then $Z(J) \ne \emptyset$. Since $I$ is proper, it is contained in some maximal ideal $\fm$. By Zariski's Lemma, $K[x_1,\dots,x_n]/\fm\cong K$,  so we have $\cZ(\fm) \neq \varnothing$.
\end{proof}
...
$f$ vanishes at every point of $\cZ(J)$ if and only if there is some $n\in \N$ such that $f^n\in I$.
...
The equations in $\sqrt{J}$ vanish on $\cZ(J)$, so $\sqrt{J}\subseteq \cI(\cZ(J))$.
	...
	For the converse, suppose that $f(\vx)$ vanishes on $\cZ(J)$. Considering the system 
\[
...
\begin{corollary}\label{ord-pres-bij}
	Let $K$ be an algebraically closed field and $R=K[x_1,\dots,x_d]$ a polynomial ring. The associations $\cZ$ and $\cI$ induce order-reversing bijections
\[ \xymatrix@R=1em{ \{ \text{subvarieties of $\A^n_K$}\} \ar@{<->}[r] & \{ \text{radical ideals of $K[x_1,\dots, x_n]$} \} \\  \{ \text{irreducible subvarieties of $\A^n_K$}\} \ar@{<->}[r] & \{ \text{prime ideals of $K[x_1,\dots, x_n]$} \} \\
 \{ \text{points of $\A^n_K$}\} \ar@{<->}[r] & \{ \text{maximal ideals of $K[x_1,\dots, x_n]$} \}} \]
	 In particular, given ideals $I$ and $J$, we have $\mathcal{Z}(I) = \mathcal{Z}(J)$ if and only if $\sqrt{I}=\sqrt{J}$.
	 
	 Likewise, we obtain analogous bijections for any affine variety $X$ in place of $\A^n_K$ and $K[X]$ in place of $K[x_1,\dots,x_n]$.
\end{corollary}
...
By the Nullstellensatz, $\cI(X) = \sqrt(I)$. The ideal is not radical: 
note that 
\[ x^9 yz \equiv (x^4)(x^5)yz \equiv y^4 z^4 \equiv y^9 \equiv x^{12} \equiv x^3 y^3 z^3 \quad \mod I\]
and
\[ x^6 y^2 z^2 \equiv x^2 y^5 z^2 \equiv x^2 z^6 \equiv x^{12}  \quad \mod I\]
...
\[ \xymatrix@R=2mm{ \Max(S) \ar[rrrrr]\ar[ddd] &&& & & \Max(R) \ar[ddd]\\
& \fm  \ar@{|->}[rrr] &&& (\phi^*)^{-1}(\fm) & \\
\\
X \ar[rrrrr] &&&&& Y}\]
...
\end{lemma}
\begin{proof}
	Consider the family of ideals $\{ J \ | \ J \supseteq I, J \cap W=\varnothing\}$. This is nonempty, since it contains $I$, and has some maximal element $\A$ by a basic application of Zorn's Lemma. We claim $\A$ is prime. Suppose $f,g\notin \A$. By maximality, $\A+(f)$ and $\A+(g)$ both have nonempty intersection with $W$, so there exist $r_1f+a_1$, $r_2 g+a_2 \in W$, with $a_1,a_2\in \A$. If $fg\in \A$, then $(r_1f+a_1)(r_2g+a_2)=r_1 r_2 fg + r_1 f a_2 + r_2 g a_1 + a_1 a_2 \in W \cap \A$, a contradiction.
\end{proof}
