...
\begin{corollary}
	Let $R$ be a finitely generated algebrad.
	\begin{itemize}
		\item $R$ is catenary.
\end{itemize}
...
\[\xymatrix@R=1mm{ 0  \ar[r] & J   \ar[r] & K[a,d] \ar[r] & K[a,b,c,d] / I }\]
is exact. Then, since $\overline{K}$ is a flat $K$-module, tensoring yields an exact sequence
\[\xymatrix@R=1mm{ 0  \ar[r] & J \otimes_K \overline{K}   \ar[r] & \overline{K}[a,d] \ar[r] & \overline{K}[a,b,c,d] / I ,}\]
...
\end{theorem}
\begin{proof}
	First, we claim that $I R_{\m_i}$ is $\m_i R_{\m_i}$-primary. Indeed, note that $(R/I)_{\m_i} = R_{\m_i} / I R_{\m_i}$ has a unique maximal ideal $\m_i R_{\m_i}$. Thus, if $x,y\in R_{\m_i}$ are such that $xy\in I R_{\m_i}$, and $x\notin \m_i R_{\m_i}$, then $x$ is a unit modulo $I_{\m_i}$, so $y\in I R_{\m_i}$. Then, the contraction of a primary ideal is primary (prove it!) so $\q_i = I R_{\m_i} \cap R$ is $\m_i$-primary, and $I \subseteq \q_1 \cap \cdots \cap \q_t$. On the other hand, equality of these modules is a local property; if $\p \notin V(I)$, then both sides are the unit ideal in $R_{\p}$, otherwise, in $R_{\m_i}$ they are proper but equal. Thus, the primary decomposition.
...
Let $I=(f_1,\dots,f_n)$ be an ideal, $\p$ be a minimal prime of $I$, and $\p_0 \subsetneq \p_1 \subsetneqq \cdots \subsetneqq \p_{h}=\p$ be a saturated chain of length $h$ ending at $\p$. If $f_1\in \p_1$, then we can apply the induction hypothesis to the ring $\overline{R}=R/(f_1)$ and the ideal $(f_2,\dots,f_n)\overline{R}$: the chain $\p_1 \overline{R} \subsetneqq \cdots \subsetneqq \p_{h} \overline{R}$ has length at most $h-1=n-1$, and we will be done. We use the previous lemma to replace our given chain with a chain of the same length that satisfies this hypothesis.
...
\begin{example}
	\begin{enumerate}
		...
		\item It is possible to have associated primes of height greater than the number of generators. For a cheap example, in $R=K[x,y]/(x^2,xy)$, the ideal generated by zero elements (the zero ideal) has an associated prime of height two, namely $(x,y)$.
		\item For the same phenomenon, but in a nice polynomial ring, $I=(x^3, y^3, x^2 u + xy v + y^2 w) \subset R= K[u,v,w,x,y]$. Note that $(u,v,w,x,y) = (I : x^2 y^2)$, so $I$ has an associated prime of height 5.
		\item Noetherian is necessary. Let $R=K[x,xy,xy^2,\dots] \subseteq K[x,y]$. Note that $(x)$ is not prime: for $a>0$, $xy^a \notin (x)$, since $y^a\notin R$, but $(xy^a)^2 = x \cdot x y^{2a}\in (x)$. Thus, $\m=(x,xy,xy^2,\dots) \subseteq \sqrt{(x)}$, and since $\m$ is a maximal ideal, we have equality, so $\Min{(x)}=\{\m\}$. However, the ideal $\p=(xy,xy^2, xy^3,\dots) = (y) K[x,y] \cap R$ is prime, and the chain $(0) \subsetneq \p \subsetneq \m$ shows that $\htt(\m)>1$.
	\end{enumerate}
\end{example}
