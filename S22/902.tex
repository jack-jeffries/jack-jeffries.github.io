...
\begin{definition}[Symbolic power]
	If $\p$ is a prime ideal in a ring $R$, the {\bf $n$th symbolic power}\index{symbolic power}\index{$\p^{(n)}$} of $\p$ is $\p^{(n)} := \p^n R_{\p} \cap R$.
\end{definition}
...
We will now spend a while discussing the notion of dimension of a ring and dimension of a variety. To motivate the definition, let's first think in terms of varieties. One way to characterize the dimension of a vector space $V$ is the the largest number $d$ such that there is a proper chain of subspaces
...
\noindent A chain of primes as above is \emph{saturated}\index{saturated chain of primes} if for each $i$, there is no $\q\in \Spec(R)$ with ${\p_i \subsetneqq \q \subsetneqq \p_{i+1}}$. One can equivalently define $\dim(R)$ as the supremum of the lengths of saturated chains of primes of $R$.
...
\begin{example}
	...
	\item A ring is zero-dimensional if and only if every minimal prime of $(0)$ is maximal.
...
\begin{rem}
We will show eventually that $\dim(K[x_1,\dots,x_d])=d$. One can easily deduce the case $d=1$, i.e. $\dim(K[x])=1$  form above, since we know that this ring is a PID.
\end{rem}
...
\item
{\bf there are Noetherian rings of infinite dimension}.  This was shown in a famous example due to Nagata presented below. 
\end{itemize}
\end{rem}
...
\label{ex:Nagata}
	Let $R=K[x_1,x_2,\dots]$. $R$ is clearly infinite-dimensional and not Noetherian. Let
	\[ W=R\smallsetminus ((x_1) \cup (x_2,x_3) \cup (x_4,x_5,x_6) \cdots )\]
	and $S=W^{-1}R$. This ring has primes of arbitrarily large height, given by the images of those primes we cut out from $W$. Thus, it has infinite dimension by Proposition \ref{rk:dimprop} part 4. The work is to show that this ring is Noetherian. We omit this argument here.
\end{example}
...
\item A prime has height zero if and only if it is a minimal prime of the 0 ideal.
...
Let's analyze our example of a copy of $\A_K^2$ union a copy of $\A_K^1$ algebraically.


\begin{ex}
The  ring $R=\frac{k[x,y,z]}{(xz,yz)}$ has dimension 2. 


We will show this using the machinery developed above.
First notice that in $k[x,y,z]$ we have the primary decomposition
$(xz,yz)=(x,y)\cap(z)$. This leads in $R$ to the primary decomposition
$(\bar 0)=(\bar x,\bar y)\cap(\bar z)$.
Thus $\Min(0)=\{(x,y),(z)\}$. According to Proposition \ref{rk:dimprop} (3), we have 
\begin{eqnarray*}
\dim(R) &=& \max\left\{\dim \frac{R}{(\bar x,\bar y)}, \dim \frac{R}{(\bar z)}\right \}\\
&=& \max\left\{\dim K[z], \dim K[x,y] \right \}\\
&=&\max\{1,2\}=2.
\end{eqnarray*}
...
\begin{rem}
The previous example illustrates the principle that the dimension of an algebraic set is the maximum of the dimensions of its irreducible component. Indeed, Proposition \ref{rk:dimprop} (3) applied to an affine ring $R=K[x_1,\ldots, x_n]/I$ gives that 
\[\dim(R)=\max\{\dim (R/\p_i) \mid \p_i\in \Min(I)\}.\]
\end{rem}
...
\begin{defn}[Support]
The {\em support}\index{support} of an $R$-module $M$ is the set
\[\Supp_R(M)=\{\p\in \Spec(R) \mid M_\p\neq  0\}. \]
\end{defn}

\begin{ex}[Alternate definition for support]
Show that for a finitely generated  $R$-module $M$, $\Supp_R(M)=V(\ann_R(M))$. In particular, $\Supp_R(R/I)=V(I)$ for any ideal $I$ of $R$.
\end{ex}
