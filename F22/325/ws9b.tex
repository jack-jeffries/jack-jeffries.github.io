...
are \emph{not} the same function, since their domains are different.
...
\item Draw the graph of $\ds g(x)=\frac{x^2-1}{x-1}$. Play the $\e-\de$ game with this function, $a=1$ and $L=-3$. What happens?
...
\item Consider the function $\ds g(x)=\frac{x^2-1}{x-1}$. It is true that $\lim_{x\to 1}  g(x) = 2$.
\begin{enumerate}
...
\item Consider the function $\ds g(x)=\frac{x^2-1}{x-1}$. It is not true that $\lim_{x\to 1}  g(x) = -3$. I claim that for $\e=1$, there is no choice of $\delta>0$ that ``works'' to make the rest of the definition true. Verify this.
