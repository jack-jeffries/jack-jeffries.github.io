...
&= c_1 (3y''_1 + 2ty'_1 + t^7 y_1) + c_2 (3y''_2 + 2ty'_2 + t^7 y_2) = 0,
\end{align*}
where the last equality is because $y_1$ and $y_2$ are solutions; this means that $y$ is a solution!
\end{ex}

For about the same reason, this works for any linear homogeneous ODE. Namely, we have:

\index{Principle of superposition and linear dependence}\index{Principle of superposition}
\begin{thm}[Principle of superposition] For any linear homogeneous ODE, given any solutions $y_1,y_2,\dots,y_n$, and any constants $c_1,c_2, \dots,c_n$, the function $y=c_1 y_1 + \cdots c_n y_n$ is also a solution to the same equation.
\end{thm}
...
\begin{defn} If $y_1,y_2,\dots,y_n$ are functions, we say that a function of the form $y=c_1 y_1 + \cdots c_n y_n$ for some constants $c_1,c_2, \dots,c_n$ is a \emph{linear combination}\index{linear combination} or \emph{superposition}\index{superposition} of the functions $y_1,y_2,\dots,y_n$.\end{defn}

Let's consider an example.
\begin{ex} Consider the ODE
\[ y'' - y=0\]
(equivalently, $y'' = y$).
Certainly $y_1= e^x$ is a solution, since it's derivative is itself.
...
\begin{defn} We say that a set of functions $\{y_1,\dots y_n\}$ is \emph{linearly independent}\index{linearly independent} (i.e., no redundancy) if whenever some linear combination takes us the zero function
