...
\[ y' = x+ y.\]
Find particular solutions with the initial conditions
\begin{enumerate}
\item $y(0)=0$,
\item $y(0)=-1$,
\item $y(0)=-2$,
\end{enumerate}
and characterize their behavior as $x\to \infty$.
...
\[ y = -(x+1) + C e^x.\]
For $y(0)=0$, we get
\[ 0 = -1  + C \rsa C=1 \rsa y= -(x+1) + e^x,\]
which goes to $+\infty$ quickly.
For $y(0)=-1$, we get
\[ -1 = -1  + C \rsa C=0 \rsa y= -(x+1) ,\]
which goes to $-\infty$ slowly.
For $y(0)=-2$, we get
\[ -2= -1  + C \rsa C=-1 \rsa y= -(x+1) - e^x,\]
which goes to $-\infty$ quickly.
\end{framed}
...
We now have great methods to solve certain first order ODEs, namely separable and linear ones. But this doesn't encompass all of them. There is a way so solve any IVP for first order ODE of the form
