...
is a PDE. A solution of this PDE would be a function $u(x,t)$ that depends two independent variables $x$ and $t$.
...
\end{enumerate}
\item There is a solution to $y'' - y' = 2y$ of the form $y=a e^{2x} + b e^{-x}$ for some real numbers $a,b$. Find it!
...
\[\begin{cases} y' = y^{2/3} \\ 
y(x_0) = y_0 \end{cases}.\]
Here $f(x,y) = y^{2/3}$ and $\frac{\partial f}{\partial y}(x,y) = 2/3 y^{-1/3}$. $f$ is continuous everywhere, but $f'$ is only continuous where $y\neq 0$.
Thus, if $y_0\neq 0$, then there is a unique solution.

However, if $y_0=0$, the theorem say nothing. We looked at this example earlier and saw that the solutions were not unique for $(x_0,y_0) = (0,0)$.
\end{ex}
...
\item To set up a differential equation, we want to relate the dependent variable with its rate of change. How much does the percentage of counterfeit bills change in a given day, on average? To figure this out, you might want to ask yourself first how many counterfeit bills are collected in a given day, on average.
...
Let's do an experiment to test our model. Each coin you've been given represents a million bills. Each face up coin is legal and each face down coin is counterfeit. Every day, take two random coins; replace the counterfeits with legal ones, and leave the legal ones alone.
...
\begin{enumerate}
\item Create a new differential equation and initial value problem to model this situation. You might find it helpful to write a closed formula for the number of total bills in circulation at a given time first.
