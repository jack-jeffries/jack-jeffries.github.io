...
If we write it like
\[ a_2 y'' + a_1 y' = -a_0 y,\]
this equation says that combining the second derivative and first derivative give some constant multiple of $y$. This suggests a class of functions to guess: for the exponential functions $y=e^{mx}$, the derivative is a constant multiple of the original, and hence the second (and all the higher ones) too. Let's try to find a solution of the form $y=e^{mx}$; if we plug in we get
\[ y' = m e^{mx}, \quad y'' = m^2 e^{mx}\]
\[\rsa 0 = a_2 m^2 e^{mx}  +a_1 m e^{mx} + a_0 e^{mx} = (a_2 m^2 + a_1 m + a_0) e^{mx}.\]

Since $e^{mx}\neq 0$, we have a solution if and only if $m$ is a solution (in $m$) of 
\[a_2 m^2 + a_1 m + a_0 =0.\] This is called the \emph{auxiliary equation}\index{auxiliary equation} of our ODE.
...
More generally,
\begin{fact} If $m_1\neq m_2$, then $e^{m_1 x}$ and $e^{m_2 x}$ are linearly independent. Even better, if $m_1, m_2,\dots, m_t$ are distinct real numbers, then the functions $\{ e^{m_1 x}, e^{m_2 x},\dots,e^{m_t x}\}$ are linearly independent.
\end{fact}
...
Not every quadratic polynomial distinct real roots. Two things can go wrong: repeated roots (like $m^2 - 4m + 4 = 0$ with double root $m=2$) and complex roots (like $m^2+ 1= 0$ with roots $m=\pm i$).
...
\[ m^2 - 4m + 4=0\]
has $m=2$ as a double root. We at least get $y_1=e^{2x}$ as a solution. How can we get another? Reduction of order!
...
(In a general reduction of order example, we would now set ${v=u'}, {v' = u''}$, but this equation is pretty simple.)
...
\begin{fact} If $m_1$ is a double root of the auxiliary equation, then $e^{m_1 x}$ and $x e^{m_1 x}$ are two linearly independent solutions.
\end{fact}
...
Now, if our auxiliary equation has an imaginary root like $m=bi$ (or a complex root $m=a+bi$), we might want to say $e^{ bix}$ is a solution, but we are only interested in real functions.
\begin{comment}
\end{comment}
