...
starting from $x=-1$ we can go to the left forever and to the right up until $x=7$ without getting into trouble, so this IVP has a unique solution on $(-\infty , 7)$.
\end{ex}
...
\begin{thm}[Principle of superposition for homogeneous ODEs] For any linear homogeneous ODE, given any solutions $y_1,y_2,\dots,y_n$, and any constants $c_1,c_2, \dots,c_n$, the function $y=c_1 y_1 + \cdots + c_n y_n$ is also a solution to the same equation.
\end{thm}
