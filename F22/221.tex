...
\begin{ex} We discussed earlier a module of population growth wherein the rate of growth of a population of proportional to the population itself. This led to the model
\[ \frac{dP}{dt} = k P\]
...
\begin{ex} A simple model for the spread of the flu throughout a population assumes that the rate at which the disease spreads is jointly proportional to the number of people infected and the number of people not infected: if $I$ is the number of total infected people and $T$ is the total population (which we assume is constant), we have
\[ \frac{dI}{dt} = k I (T-I),\]
so this is a logistic model.
Supposing the flu spreads among a population of $4000$ people\dots
\end{ex}
...
\[ \frac{P}{P_c - P}  =   \pm e^C e^{k P_c t } = C e^{k P_c t } \]
Rearranging and solving gives
\[ P = \frac{C e^{k P_c t }}{1+ C e^{k P_c t }}.\]
