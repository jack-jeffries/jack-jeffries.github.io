...
\begin{thm}[Existence and uniqueness theorem for linear ODEs, Version 1]
Given a linear ODE
\[ a_n(x) y^{(n)} + a_{n-1}(x) y^{(n-1)} + \cdots + a_1(x) y' + a_0(x) y = g(x)\]
...
We now want a tool to detect linear (in)dependence of functions. Here is a clever idea: if $c_1 f + c_2 g=0$ (as functions) then, taking derivatives,
 \[ c_1 f' + c_2 g' = 0\] also. Then\dots
