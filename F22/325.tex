...
\item Explain why every rational number $q\in \Q$ occurs in $\{ w_n\}_{n=1}^\infty$ infinitely many times.
...
Let $\{a_n\}_{n=1}^\infty$ be a sequence.
 Assume $\{a_n\}_{n=1}^\infty$ is bounded above and that the set of values $\{a_n \ | \ n\in \N\}$ does not have a maximum value. Then the sequence has an increasing subsequence.
\end{lem}
...
\begin{proof}
Assume $\{a_n\}_{n=1}^\infty$ is bounded above and that the set of values $\{a_n \ | \ n\in \N\}$ does not have a maximum value.
...
then the set of values of the sequence has a maximum.
\end{quote}
\end{rem}
...
decreasing subsequence by constructing the indices $n_1<n_2< \cdots$ of such a subsequence recursively. 
 ...
 By Lemma~\ref{lem214b} (see the remark following it), since $\{a_n\}_{n=1}^\infty$ is bounded
...
For any $k$, given $n_k$, the subsequence $a_{n_k +1} , a_{n_k+2},a_{n_k+3},\ldots$ is also bounded above and has no increasing subsequence. Thus, it must have a maximum value by Lemma~\ref{lem214b}; choose $n_{k+1}$ such that $a_{n_{k+1}} = \max\{ a_{n_k +1} , a_{n_k+2},a_{n_k+3}, \dots\}$. This gives a recursive definition for $n_k$.

By construction, we have $n_{k+1} > n_k$ for all $k$. Note that $a_{n_{k}}$ is the maximum of a set that contains $a_{n_{k+1}}$ (since it is later in the sequence). It follows that $a_{n_k} \geq a_{n_{k+1}}$. That is, we have constructed a decreasing subsequence of the original sequence.
\end{proof}
