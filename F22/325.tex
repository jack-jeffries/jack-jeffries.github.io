...
We have shown that $f_1$ is differentiable at any $x\in \R$ and $f_1'(x) = 1$, and $f_2$ is differentiable at $x=0$ with $f_2'(0)=0$. Thus $f$ is differentiable at $0$ and $f'(0)=1$. However, $f$ is not increasing on any interval containing $0$. To see it, let $\d>0$, and (using density of rational numbers) pick a rational number $a$ with $0<a<\d$. Then $a^2>0$ so $a^2+a>a$. By density of irrational numbers, there is some irrational number $b$ with $a<b<\min\{a^2+a,\d\}$. Then $a<b$ (and both are in $(-\d,\d)$) but $f(a)=a^2+a>b=f(b)$, so $f$ is not increasing on $(-\d,\d)$.
\end{ex}
