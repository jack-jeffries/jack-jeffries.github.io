...
\begin{framed}
Take $h(x) = \frac{ f(x) - f(r) }{x - r}$.  Let $\e=f'(r) = \lim_{x\to r} h(x)$. Then there is some $\d>0$ such that $|h(x) - f'(r)| > \e$ when $x\in (r-\d,r+\d)$. But $|h(x) - f'(r)| < f'(r)$ implies $h(x)>0$. If $h(x)>0$ and $x>r$ then $x-r>0$ so $f(x)-f(r) = h(x) (x-r)>0$. Thus, if $x\in (r,r+\d)$, then $f(x)>f(r)$. If $x<r$, then $x-r<0$ so $f(x)-f(r) = h(x) (x-r)<0$. Thus, if $x\in (r-\d,r)$, then $f(x)<f(r)$.
\end{framed}
