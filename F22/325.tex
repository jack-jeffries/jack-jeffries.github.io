...
We can also ``start'' our induction at any other integer $m$ and prove a statement for all integers greater than or equal to that number $m$. (To justify it from the statement of PMI we have, let $P'_k = P_{k-m-1}$ and prove $P'_k$ by induction.)


\begin{ex} Let's show that $2^n < n!$ for all natural numbers $n>3$. To show it is true for all $n\geq 4$, first we check the statement for $n=4$:
\[ 2^4 = 16 < 24 = 4!.\]
We also check the inductive step: let $k$ be a natural number greater than $3$ and assume that $2^k < k !$. 

Then \[ 2^{k+1} = 2 \cdot 2^k < 2 \cdot k! < (k+1) \cdot k! = (k+1)!.\]
Thus, by induction, the statement is true for any natural number ${n>3}$.\qed
\end{ex}
...
Let us use a recursive sequence to prove a proposition that we will use later.

\begin{prop}
For any real number $r$, there exists a strictly increasing sequence of rational numbers that converges to $r$.
\end{prop}
\begin{proof}
We will construct a sequence of rational numbers $\{q_n\}_{n=1}^\infty$ such that $r-\frac{1}{n} < q_n < r$ for every $n$ that is strictly increasing, and then show that this sequence converges to $r$. By Density of Rational Numbers, there exists a rational number $q_1$ such that $r-1 < q < r$. Given $q_1,\dots,q_n$, take $q_{n+1}$ to be a rational number such that $\max\{r-\frac{1}{n+1}, q_n\} < q_{n+1} < r$ again using Density of Rational Numbers. Since $q_n \leq \max\{r-\frac{1}{n+1}, q_n\} < q_{n+1}$ for every $n$, the sequence we obtain is strictly increasing. Since $r-\frac{1}{n+1}  \leq \max\{r-\frac{1}{n+1}, q_n\} < q_{n+1}$ for every~$n$ (equivalently, $r-\frac{1}{n} < q_n < r$ for all $n$), and $\{ r - \frac{1}{n}\}_{n=1}^\infty$ converges to $r$, by the Squeeze Theorem, the sequence $\{q_n\}_{n=1}^\infty$ converges to $r$.
\end{proof}


Induction and recursion go together well. Let us apply induction to understand a recursively defined sequence.

\begin{ex} Define a sequence recursively by the rules $a_1 = 2$ and $a_{n} = \frac{a_{n-1}}{2} + \frac{1}{a_{n-1}}$ for $n>1$. So we have $a_1 = 2$, $a_2 = \frac{1}{2} + \frac{1}{1} = \frac{3}{2}$, etc. 

Let's show that $a_n>0$ for all $n$. We proceed by induction on $n$. First we note that $a_1 = 2 >0$. Then, assuming $a_k>0$, we have $a_{k+1} = \frac{a_k}{2} + \frac{1}{a_k} >0$ since both terms are positive. Thus, $a_n>0$ for all $n$ by induction.\qed
\end{ex}
