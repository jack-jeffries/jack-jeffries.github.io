...
\[ P = E_{ii} E_{i} = \begin{bmatrix} [2] & 0 \\ x & [1] \end{bmatrix}, \ \text{and} \ Q=E_{iii}.\]
\end{proof}
\item Using your transformations from (a), find bases $B, C$ for $R^2$ such that $[t_A]_B^C = A'$.
\begin{proof}
We first reinterpret each of our elementary matrices as a change of basis matrix for an elementary operation. Let $D=\{d_1,d_2\}$ be a basis for $R^2$.
\begin{enumerate}[(i)]
\item  $E_i$ is the change of basis matrix $[\mathrm{id}_{R^2}]_{D'}^D$ where $D'=\{[2] d_1, d_2\}$.
\item $E_{ii}$ is the change of basis matrix $[\mathrm{id}_{R^2}]_{D'}^D$ where $D'=\{d_1, d_2\}$.
\item Add $x+[1]$ times column $1$ to column $2$. \qedhere
\end{enumerate}
\end{enumerate}
