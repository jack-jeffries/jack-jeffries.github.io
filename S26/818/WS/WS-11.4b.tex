...
\solution{
\begin{enumerate}
\itema Let $f\in F$. Since $B$ is a basis, there is at least one expression $f= r_1 b_1 + \cdots +r_n b_n$ with $r_i\in R$ and $b_i \in B$ because $B$ generates $F$. Given another, by including some extra zero coefficients (to both expressions), we can assume the other uses the same elements of $B$, so take $f=r'_1 b_1 + \cdots + r'_n b_n$. Then after subtracting we get
