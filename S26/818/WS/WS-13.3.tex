...
\textsc{Structure Theorem for Finitely Generated Modules over PIDs (Invariant Factor Form):} Let $R$ be a PID. Let $M$ be a finitely generated $R$-module. Then there exist $r,t\geq 0$ and nonzero nonunit elements $a_1, \dots, a_t\in R$ such that
\begin{itemize}
...
\solution{From a Lemma in class, we know that $A$ and $PAQ=D$ present isomorphic modules. We can get rid of any columns of zeroes, and given any unit $d_{ii}$'s, we can scratch off that row and column. We end up with the}
...
\itemb Suppose that $n>0$. Let $\phi$ be an isomorphism from left to right, and ${m= (0, 0, \dots, 1+(d_n))}$ in the left-hand side. Show that $\mathrm{ann}_R(\phi(m)) = (d_n)$.
