...
Consider the collection of subsets $\{m + I^a M \ | \ m\in M, a\in \N\}$. We claim that this forms the basis of open sets for a topological space, i.e., that the intersection of two of these is contained in another. Indeed, if $(m+I^aM) \cap (n+ I^b M)$ is nonempty, suppose that $a\leq b$ and $t$ is in the intersection. Then $t-m \in I^a M$ and $t-n \in I^b M \subseteq I^a M$, so $n-m = (t-m)-(t-n)\in I^a M$, so $n\in m+ I^aM$, and hence $n+I^b M \subseteq n+I^a M = m+I^a M$, and thus $(m+I^aM) \cap (n+ I^b M)= m+I^aM$.

\begin{defn} Let $R$ be a ring, $I$ an ideal, and $M$ an $R$-module. The \emph{$I$-adic topology}\index{$I$-adic topology} on $M$ is the topology with open basis $\{m + I^a M \ | \ m\in M, a\in \N\}$; that is, the topology whose open sets are arbitrary unions of sets of the form $m + I^a M$.
\end{defn}
...
\item If $S$ is another ring, $J$ ideal of $S$, and $N$ an $S$-module, a function $f:M\to N$ is continuous with respect to the two topologies if for any $e\in \N$, there is some $d\in \N$ such that $m \equiv m' \mod I^e M$ implies $f(m) \equiv f(m') \mod I^d N$; that is, $f(m+I^e M) \subseteq f(m) + I^dN$. If $f$ is additive, the last condition is equivalent to $f(I^e M ) \subseteq I^dN$.
\end{itemize}
\end{prop}
...
Given a ring $R$, and ideal $I$, the set of Cauchy sequences with pointwise addition and multiplication forms a ring: it is straightforward to see that the sum and product of Cauchy sequences is Cauchy. There is a homomorphism from $R$ to this ring that sends an element to the associated constant sequence. The set of sequences that converge to zero forms an ideal in this ring.
...
Note that the map from $R$ to $\hat{R}^I$ is continuous (with respect to the $I$ and $I\hat{R}^I$ topologies), that $R$ is dense in $\hat{R}^I$ in the $I\hat{R}^I$-topology, and that $\hat{R}^I$ is $I\hat{R}^I$-adically complete.
...
\end{defn}
 ...
 \begin{prop} Let $R$ and $S$ be rings, with respective ideals $I$ and $J$, and $\phi:R\to S$ a homomorphism with $\phi(I)\subseteq J$.
 
  If $S$ is $J$-adically complete, then there is a unique continuous  ring homomorphism $\phi':\hat{R}^I \to S$ extending $\phi$.
 
 If $R$ is $I$-adiaclly complete and $S$ is $J$-adically complete, then there is a unique continuous  ring homomorphism $\hat{\phi}:\hat{R}^I \to \hat{S}^J$ extending $\phi$.
 \end{prop}
 
 Ex: power series is completion of poly ring
 
 Completion of Noeth local ring is quotient of power series, so Noeth.
