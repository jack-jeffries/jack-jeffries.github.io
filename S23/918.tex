...
\begin{cor} Let $K$ be a perfect field and $R=K[x_1,\dots,x_n]/I$. Let $I=\fp_1,\dots,\fp_t$ be the minimal primes of $R$, with heights $h_1,\dots,h_t$ and preimages $\fq_1,\dots,\fq_t$. Let $I= Q_1 \cap \cdots \cap Q_t \cap Q_{t+1} \cap \cdots \cap Q_s$ be a minimal primary decomposition, with $Q_i$ corresponding to the preimage of $\fp_i$ for $i=1,\dots,t$ and the rest embedded. Then 
\[ \mathrm{Sing}(R) = \bigcup_{i<j} V( \fp_i + \fp_j) \cup \bigcup_{k=1}^t V(\fp_k + I_{h_k}(J^T(\text{gens of} \ Q_k)))= V( \prod_{i<j} ( \fp_i + \fp_j) \cdot \prod_{k=1}^t (\fp_k + I_{h_k}(J^T(\text{gens of} \ Q_k))) ). \]
In particular, the singular locus is closed.
\end{cor}
\begin{proof} Let $\fp\in \Spec(R)$ with preimage $\fq\in \Spec(S)$.

Suppose that $R_{\fp}$ is regular. Then $R_\fp \cong (S/I)_{\fq}$ is $S_{\fq}$ mod the primary components contained in $\fq$; since this is a domain, this intersection must be just a single prime, so $\fp$ contains only one of the $\fp_i$'s and $R_\fp \cong (S/Q_i)_{\fq}$ for some $i$. Then $\fp$ must not contain the $h_i$ minors of the Jacobian of a generating set of $Q_i$, by the case where all components have the same height. This shows the containment ($\supseteq$). Conversely, if $\fp$ is not in the right hand side, then $\fp$ must contain only one minimal prime $\fp_i$, and it cannot contain the $h_i$ minors of the corresponding Jacobian, so, using the same isomorphism, $R_{\fp}$ is regular.
\end{proof}
...
Let $K$ be a field and $R=K[x_1,\dots,x_n]$. Let $K_0\subseteq K$ be a subfield. Let $T= \{k_i\}$ be a transcendence basis for $K/K_0$ if characteristic 0 or a $p$-basis for $K/K_0$ if characteristic $p$. Then we can take $\Omega_{R|K_0}$ to be free on the basis $dx_1,\dots,dx_n, \{d k_j\}$.
 
Recall the following derivations of $K$ over $K_0$.
...
\begin{cor} A composition of two formally smooth maps is smooth. Likewise for the other properties.
\end{cor}
