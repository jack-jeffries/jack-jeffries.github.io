...
\begin{defn} Let $K$ be a field. We say that a $K$-algebra $R$ is \emph{geometrically regular}
\index{geometrically regular} if for any finite field extension $L$, we have $L\otimes_K R$ is regular.
\end{defn}

\begin{lem} If $K$ is perfect and $R$ is a regular ring essentially algebra finite over $K$, then $R$ is geometrically regular over $K$.
\end{lem}
\begin{proof} Let $L$ be a finite extension of $K$. By hypothesis, $L$ is finite separable over $K$, so we can write $L=K[\theta] / (f(\theta))$ with $f$ separable; then $L\otimes_K R = R[\theta]/(f(\theta))$. 
