...
\[ \Der_{R|A}(M) \to \Hom_A(I/I^2,M)\]
induced by restriction.
\end{prop}

Viewing $\Hom_A(I/I^2,M)$ as an $R$-module via the action on $M$, this is $R$-linear.
...
\[ \Der_{R|\C}(R/\fm) \xra{\res} \Hom_{\C}(\fm/\fm^2,R/\fm) =(\fm/\fm^2)^*, \]
...
We actually don't need the very very strong hypothesis of polynomial ring in the last example. 

%\begin{prop}
%Let $\phi:R\to S$ and $\pi:S\to R$ be ring homomorphisms such that $\pi \circ \phi = 1_R$. Let $J=\ker(\pi)$ and asuume that $J^2=0$. Then
%\[ \Der_{S|R}(R)\cong \Hom_{R}(J,R).\]
%\end{prop}
%\begin{proof}

\begin{ex} Let $R$ be any finitely generated $\C$-algebra, and  $\fm$ be a maximal ideal.
Let's compute $\Der_{R|K}(R/\fm)$. 

We $R=K[x_1,\dots,x_n]/I$, and $\fm=(x_1-a_1,\dots,x_n-a_n)$ for some $a_i\in \C$; set $\widetilde{x_i} = x_i-a_i$; these generate $R$ as a $\C$-algebra (since we can get the $x$'s back from them). Thus, the map
\[  \Der_{R|A}(\C) \xra{\res} \Hom_{\C}(\fm/\fm^2,R/\fm)=(\fm/\fm^2)^* \]
is injective: the values on the elements on $\widetilde{x_i}$ uniquely determine the derivation.

In fact, this map is surjective as well. To see it, take a $\C$-basis for $\fm/\fm^2$; without loss of generality we can take $\widetilde{x_1},\dots,\widetilde{x_m}$. then $1,\widetilde{x_1},\dots,\widetilde{x_m}$ is a basis for $R/\fm^2$, and $\widetilde{x_i} \widetilde{x_j}=0$ for all $i,j$. For any element of $R$ we can uniquely write 
\[f= \lambda_0 + \sum_{i=1}^m \lambda_i \widetilde{x_i} + \xi \]
with $\xi\in \fm^2$.
Define the map 
\[\partial_i( \lambda_0 + \sum_j \lambda_j v_j + \xi ) = \lambda_i;\]
this is definitely $\C$-linear, and
\[\begin{aligned} \partial_i( (\lambda_0 &+ \sum_j \lambda_j v_j + \xi ) (\mu_0 + \sum_j \mu_j v_j + \xi' )) = \partial_i ( \lambda_0 \mu + \sum_j ( \lambda_0 \mu_j + \mu_0 \lambda_j) v_j + \xi'') \\& = \lambda_0 \mu_i + \mu_0 \lambda_i\\&= (\lambda_0 + \sum_j \lambda_j v_j+\xi) \cdot \partial_i( (\mu_0 + \sum_j \mu_j v_j+ \xi' ))+ (\mu_0 + \sum_j \mu_j v_j+\xi') \cdot \partial_i( (\lambda_0 + \sum_j \lambda_j v_j+\xi).\end{aligned}\]
...
so the linear part of $f$ is given by the linear equation $\nabla(f_i)|_{x=a} \cdot \widetilde{x}=0$. Then we take $T_{a}(X)$ to be the linear subspace of $T_a(\C^n)$ cut out by the linear equations $\nabla(f_1)|_{x=a},\dots,\nabla(f_m)|_{x=a}$. In particular, $T_a(\C^n)$ is the kernel of the \emph{Jacobian matrix}\index{Jacobian matrix}
...
Then  the images of the $\widetilde{x_j}$ form a vector space for $\fn/\fn^2$; we have essentially defined $T_a(\C^n)$ as the $\C$-vector space with coordinates $\widetilde{x_j}$; i.e., the dual space to $\fn/\fn^2$. So $T_a(\C^n)\cong (\fn/\fn^2)^*$.

By some standard isomorphism theorems, we can identify $\fm/\fm^2 \cong \fn / (\fn^2+I)$: namely,
\[ \frac{\fm}{\fm^2} \cong \frac{\fn/I}{(\fn^2 + I)/I} \cong \frac{\fn}{\fn^2+I}.\]
So 
\[ \dim_{\C}(\fm/\fm^2) = n - \dim_{\C}(  \frac{\fn}{\fn^2+I} ) = n- \dim_{\C}( \mathrm{im}\ I \to \frac{\fn}{\fn^2}).\]
Given an element $f\in I$, as above we write
\[f = \sum_j \frac{d}{dx_j}|_{x=a}(f_i) \widetilde{x_j} + \mathrm{higher\  order\  terms} \]
so the image of $f$ is in $\fn/\fn^2$ is $\nabla_{x=a}(f_i)  \underline{\widetilde{x}}$. Thus, the dimension of the image of $I$ is the rank of the transpose of the Jacobian matrix. By rank nullity, the dimension of $\fm/\fm^2$ is the same as the dimension of the kernel of the Jacobian matrix, which is just the dimension of $T_a(X)$.
\end{proof}

We say that a variety $X$ is \emph{nonsingular} at $a$ if $\dim_\C T_a(X) = \dim_a X$, and \emph{singular}\index{singular}\index{nonsingular} otherwise (in which case ``$>$'' happens).
...
Motivated by the geometric case, for a local ring $(R,\fm,k)$ we define $\fm/\fm^2$ to be the \emph{cotangent space}\index{cotangent space} and $\Hom_k(\fm/\fm^2,k)$ to be the \emph{tangent space}\index{tangent space} of $R$. We say that $R$ is a \emph{regular local ring}\index{regular ring} $\dim(R) = \dim_k(\fm/\fm^2)$.
