...
\begin{cor} Let $K$ be a perfect field and $R=K[x_1,\dots,x_n]/I$. Suppose that the height of each minimal prime of $I$ is $h$. Then, the singular locus of $R$ is the ideal of $h\times h$ minors of the column Jacobian matrix of~$f$.
\end{cor}
\begin{proof}
...
\begin{ex}
\begin{enumerate}
\item Let $K=\C$ and $R=K[x,y,z]/(x^2-x,xyz)$. Then the height of the ideal $I$ is $1$, since $x$ is a minimal prime, but $I_1$ of the Jacobian contains $2x-1$, which generates the unit ideal in the quotient. But, localizing at $(x,y,z)$ we get $K[x,y,z]_{(x,y,z)}/(xyz)$, which is not regular.
\item Let $K=\F_p$ and $R=K[x]/(x^p)$. Then $\Omega_{R|K}$ is free of rank $1$, which is not $1-1=0$, and $R$ is not regular.
\end{enumerate}
\end{ex}
...
If the characteristic of $K$ is zero, then every element of $\fa R_\fa$ is niltpotent in $R_\fa$. Let $f^t=0$ but $f^{t-1}\neq 0$. Applying 
...
Let $K$ be a field and $R=K[x_1,\dots,x_n]$. Universal differential.
