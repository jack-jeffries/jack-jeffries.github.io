...
\end{lem}
\begin{proof}
For the first statement, given a $t\times t$ minor of $AB$, we can replace $A$ by the rows occurring in the given minor and $B$ by the columns occurring in that minor. The statement then follows from Cauchy-Binet.  The second is straightforward.
\end{proof}
...
\begin{lem}
If $I \subseteq \fa$, then one has $J^T(I) = M J^T(\fa)$ for some matrix $M$.
\end{lem}

\begin{lem} Let $K$ be a perfect field and $S=K[x_1,\dots,x_n]$ be a polynomial ring. Let $I=(f_1,\dots,f_t) \subseteq S$ be an ideal and $\fq \supseteq I$ a prime ideal. If $\htt(I_\fq) = h$, then $I_{h+1}(J^T(f_1,\dots,f_t)) = 0$ in $S_\fq$.
\end{lem}
\begin{proof} Let $I \subseteq \fa\subseteq \fq$ with $\fa$ of height $h$. Then $S_\fa$ is regular, so there exist $g_1,\dots,g_h\in S$ with $\fa S_\fa = (g_1,\dots,g_h) S_\fa$. Now,  since $I S_{\fa} \subseteq \fa S_{\fa}$ by the lemma there is some matrix $M$ such that $J^T(f_1,\dots,f_t) S_{\fa} = M  J^T(g_1,\dots,g_h) S_\fa$. But then $I_{h+1}(J^T(f_1,\dots,f_t)) S_{\fa} =0$. Since $S_{\fq}$ is a domain, we must have $I_{h+1}(J^T(f_1,\dots,f_t)) S_{\fq} =0$ as well.
\end{proof}
...
\item The rank of the Jacobian matrix for $I$ in $\kappa(\fp)$ is $\htt I_\fq$.
\item $\Omega_{R_\fp|K}$ is free of rank $n-\htt I_\fq$.
\end{itemize}
\end{prop}
\begin{proof} Set $h=\htt I_\fq$. 

We have maps
\[ \frac{I_\fq}{\fq I_\fq} \xra{\eta} \frac{\fq S_{\fq}}{\fq^2 S_{\fq}} \xra{d} \kappa(\fq) \otimes_K \Omega_{S|K}\]
with the second injective. Observe that the cokernel of  $\frac{I S_\fq}{\fq I S_\fq} \xra{\eta} \frac{\fq S_{\fq}}{\fq^2 S_{\fq}}$ is $\frac{\fp R_\fp}{\fp^2 R_{\fp}}$, so $\dim(\frac{\fp R_\fp}{\fp^2 R_{\fp}}) = \dim(\frac{\fq S_{\fq}}{\fq^2 S_{\fq}}) - \rk(\eta) = \dim(S_\fq) - \rk(\eta)$, and $\dim(R_\fp) = \dim(S_\fq) - \htt(I_\fq)$, so $R_\fp$ is regular if and only if $\rk(\eta) = \htt(I_\fq)$. But since $d$ is injective, the rank of $\eta$ equals that of $d \circ \eta$, which is the rank of the Jacobian matrix taken in $\kappa(\fp)$.

On the other hand, recall that $\Omega_{R_\fp|K}$ is the cokernel of the Jacobian considered as a map of $R_\fp$ modules. By the lemma, the $I_{h+1}$ of the presenting matrix is zero in $S_\fq$, hence in $R_\fp$. We then have $I_h$ of this matrix is the unit ideal if and only it is not contained in the maximal ideal; i.e., if and only if it is nonzero considered in the residue field. But this is equivalent to free of rank $n-h$.
\end{proof}
