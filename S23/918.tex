...
Moreover, one can take formal combinations of the elements $\frac{d}{dt_0}$.
\end{rem}
...
\end{prop}
 ...
 \begin{prop} Let $R$ and $S$ be rings, with respective ideals $I$ and $J$, and $\phi:R\to S$ a homomorphism with $\phi(I)\subseteq J$. There is a unique continuous ring homomorphism $\hat{\phi}:\hat{R}^I \to \hat{S}^J$ extending $\phi$. Likewise, if $\alpha: M\to N$ is $R$-linear, there is a unique continuous $\hat{R}^I$-linear map $\hat{\alpha}:\hat{M}^I \to \hat{N}^I$ extending $\alpha$.
 \end{prop}
 \begin{proof}
 Since $\phi$ is continuous, a continuous homomorphism maps a Cauchy sequence to a Cauchy sequence, and a sequence converging to zero to one converging to zero, so induces a well-defined continuous map on the completions $\hat{\phi}$ which sends $\hat{\phi}(\lim r_n) = \lim \phi(r_n)$. For uniqueness, if $\hat{\phi}$ is continuous and extends $\phi$, the latter formula must hold. Likewise for modules.
 \end{proof}
