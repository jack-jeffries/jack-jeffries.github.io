...
\begin{lem} Let $K$ be a perfect field and $S=K[x_1,\dots,x_n]$ be a polynomial ring. Let $I=(f_1,\dots,f_t) \subseteq S$ be an ideal and $\fa \supseteq I$ a prime ideal. If $\htt(\fa) = h$, then $I_{h+1}(J^T(f_1,\dots,f_t)) = 0$ in $S_{\fa}/\fa S_{\fa}$.
\end{lem}
...
\item Let $K=\F(t)$ and $R=K[x]/(x^p-t)$. Then $R$ is regular, but the Jacobian criterion would say the singular locus is all of $\Spec(R)$.
