...
\begin{defn} A \emph{Lie algebra}\index{Lie algebra} over a ring $A$ is an $R$-module $M$ equipped with an operation $[ - , - ]: M\times M \to M$ such that, for all $l,m,n\in M$ and $a\in A$:
\begin{itemize}
\item $[l+m,n] = [l,n]+[m,n]$,
\item $[rm,n] = r[m,n]$,
\item $[m,m] =0$,
...
\begin{ex} If $N$ is an $A$-module, then $E=\End_A(N)$ with bracket $[\alpha,\beta]:= \alpha\circ \beta - \beta\circ \alpha$ is a Lie algebra over $A$. 
