...
Now \[R_{\fp_i R} = S_{\fp_i} = \{ \frac{f}{g} \in \mathrm{frac}(S) \ | \ f\in S, g\in S \ \text{with some nonzero term not involving $x_i$}\}.\]
We have $L_i:=K(\{ x_j \ | \ j \neq i\}) = \mathrm{frac}( K[x_j^2,x_j^3 \ | \ j\neq i])$.
We claim that $L_i[x_i^2,x_i^3]_{(x_i^2,x_i^3)}\cong S_{\fp_i}$. Indeed, from above we can identify $L_i[x_i^2,x_i^3]$ and hence $L_i[x_i^2,x_i^3]_{(x_i^2,x_i^3)}$ with subsets of $\mathrm{frac}(S)$. Any element of $L_i[x_i^2,x_i^3]$ can be written as a fraction with denominator not involving $x_i$; any element of $L_i[x_i^2,x_i^3]\smallsetminus (x_i^2,x_i^3)L_i[x_i^2,x_i^3]$ can be written as a fraction with denominator not involving $x_i$ and numerator with some nonzero term not involving $x_i$. Putting together, an element of the localization can be written in in $S_{\fp_i}$.
