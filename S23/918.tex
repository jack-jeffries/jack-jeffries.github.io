...
\begin{proof}
	...
	Now, pick $A_i,B_i\in R[x]$ with $\overline{A_i}=a_i, \deg(A_i)=\deg(a_i)$ (likewise with $B$'s), and set $G_{n+1}=G_n + \sum_i U_i B_i$, $H_{n+1}=H_n + \sum_i U_i A_i$. These polynomials then satisfy the same hypotheses (modulo $\fm^{n+1}$). Indeed:
...
\begin{equation}\tag{$\clubsuit$} R^{p^e}[T] \cap \fm \subseteq \fm^{p^e} \text{ (power of $\fm$ in $R$)} .\end{equation} To verify it, write $u\in \fm$ as an $R^{p^e}$-linear combination of $T^{[<p^e]}$. Taking images of this linear combination modulo $\fm$ gives a $k^{p^e}$-linear combination of  $\Theta^{[<p^e]}$ that is zero, so by the lemma, each coefficient is zero in $k^{p^e}$, and hence in the original combination, each coefficient is in $\fm \cap R^{p^e} \subseteq \fm^{p^e}$ (maximal ideal of  $R^{p^e}$). Thus \[ K \cap \fm = \bigcap_e (R^{p^e}[T] \cap \fm) \subseteq \bigcap_e \fm^{p^e} \text{ (powers of $\fm$ in $R$) }= 0.\] Thus, $K$ injects into $k$.
...
\begin{thm}[Cohen Structure Theorem]\index{Cohen structure theorem} Let $(R,\fm,k)$ be a local ring containing a field. Then $R\cong k\llbracket x_1,\dots,x_n\rrbracket / I$ for some ideal $I$.
\end{thm}
\begin{proof}
...
\item Column jacobian is injective in $\kappa(\fp)$.
\end{itemize}
\end{thm}
...
\end{lem}
\begin{proof}
We have seen the equivalence of the first two earlier. If $S/I$ is regular, then the rank of $\eta$ equals the minimal number of generators of $I$, and each minimal generator decreases the dimension by one, so the height equals the minimal number of generators, thus rank of $\eta$ equals height of $I$.  Conversely, if the rank of $\eta$ equals the height of $I$, take a set of minimal generators whose images form a basis of the image of $\eta$ and let $J$ be the ideal they generate. Then $\dim(S/J)$ is regular, hence a domain, and $\dim(S/J)=\dim(S) - \height(I) = \dim(S/I)$, so $I/J=0$, and hence $S/I$ is regular.
\end{proof}

\begin{lem}
Let $(R,\fm)$ be a local ring. Given an $m\times n$ matrix $A$ of rank at most $t$, the cokernel of $A$ is free of rank $m-t$ if and only if some $t\times t$ minor of $A$ is a unit.
\end{lem}
