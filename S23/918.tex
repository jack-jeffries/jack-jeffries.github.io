...
We now encode many of the key properties of derivations in some left-exact sequences. Recall that a sequence of maps of $R$-modules is  a \emph{left exact sequence}\index{left exact sequence} if it is of the form
\[ 0\to L \xra{\alpha} M \xra{\beta} N\]
with $\alpha$ injective and $\ker(\beta)=\im(\alpha)$.
...
\[ 0 \to \Der_{R/I|A}(M) \xra{\pi^*} \Der_{R|A}(M) \xra{\res} \Hom_A(I/I^2,M). \]
\end{prop}
\begin{proof}
