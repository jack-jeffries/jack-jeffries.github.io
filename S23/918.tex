...
Note that if $A\to R$ is surjective with kernel $I$, then $\Gamma_{R|A} = I/I^2$, since the target of the map $d$ above is zero.
...
\begin{exer} Show that $W^{-1}\Gamma_{R|A}\cong \Gamma_{W^{-1}R|A}$. Hint: First show that if $S=A[X]$ is a polynomial ring, then $W^{-1}S$ is flat over $A$ and $\Gamma_{W^{-1}S|A}=0$.
\end{exer}
...
\begin{itemize}
\item $P$ is locally free: for every maximal ideal $\fm$ (equivalently, every prime ideal $\fp$), $P_\fm$ is a free $R_\fm$-module (resp., $P_{\fp}$ is a free $R_{\fp}$-module).
\end{itemize}
