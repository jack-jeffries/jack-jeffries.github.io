...
\begin{rem} Let $R$ be a ring. A subset  $X\subseteq \Spec(R)$ is \emph{specialization closed}\index{specialization closed} if for any $\fp\in X$ and $\fq \supseteq \fp$, $\fq\in X$. A closed subset of $\Spec(R)$ is necessarily specialization closed, but the converse is false. For example, in $\Spec(\Z)$, the set of maximal ideals is specialization closed but not closed: any closed subset $V(I) = V((n))$ if not containing $(0)$ must have $n\neq 0$, and $n$ has finitely many prime divisors, so $V((n))$ is finite.
...
\May{11}
 ...
 The target of a $p$-derivation is an algebra and not a module, so we can't make sense of a universal target module akin to the module of differentials. However, if $p=0$ in the target, then the product rule simplifies in a useful way. 
...
To make this line up, let $M$ be an $R/pR$ module, and let ${F_{R/pR}}$ be the Frobenius map on $R/pR \to R/pR$. Let ${F_{R/pR}}_*(M)$ be the $R/pR$-module obtained by restriction of scalars through $F$: this is identical to $M$ as a set, but an element of $r$ sends $m$ to $r^p m$ instead. The function $\alpha:R/pR \to M$  satisfies 
\begin{itemize}
 \item $\alpha(x+y) = \alpha(x) + \alpha(y)$
 \item $\alpha(xy) = x^p \alpha(y) + y^p\alpha(x)$
 \end{itemize}
 if and only if the same function  $\alpha:R/pR \to {F_{R/pR}}_*(M)$ is a derivation: the first condition is additivity, and writing $\star$ for the ${F_{R/pR}}_*(M)$ action, the second translates to
 \[ \alpha(xy) = x^p \alpha(y) + y^p\alpha(x) = x \star \alpha(y) + y \star \alpha(x).\]
 
 
Furthermore, if $\alpha:R\to M$ with $pM=0$ satisfies the perivation axioms with $\alpha(p)=0$, then $\alpha$ is additive and hence $\alpha(pR)\subseteq pM=0$, so $\alpha$ factors through $R/pR$, and the map $R \to R/pR \to {F_{R/pR}}_*(M)$ is a derivation; conversely if $R \to {F_{R/pR}}_*(M)$ is a derivation, it is additive so it factors through $pR$ and we again get a perivation with $\alpha(p)=0$.
Thus, a perivation with $\alpha(p)=0$ is the same thing as a derivation $\alpha: R\to {F_{R/pR}}_*(M)$. We call such a map a \emph{derivation of Frobenius}\index{derivation of Frobenius}.

The ``over $(A,\delta)$'' condition for a derivation of Frobenius considered as a perivation is equivalent to $\alpha(a) = 0$ for all $a\in A$, which is equivalent to $A$-linear.
\end{rem}

\begin{ex} Let $A$ be a ring in which $p$ is not a unit (so $A/pA\neq 0$) and $R=A[x]$. Then the map $\partial:R\to R/pR$,  $\partial(r) = (\frac{dr}{dx})^p$ is a derivation of Frobenius: we have
 \[ \partial(rs) = \big(\frac{d (rs)}{dx} \big)^p = \big( r \frac{ds}{dx} + s \frac{dr}{dx} \big)^p = r^p \partial(s) + s^p \partial(r).\]
 \end{ex}
...
\begin{rem} If $p$ is a unit in $R$, then $\widetilde{\Omega}_{R} = 0$ (and likewise over and $(A,\delta)$), since any $R$-module with $pM=0$ is the zero module.
\end{rem}

This construction almost coincides with the Kahler differential construction in positive characteristic.

\begin{rem} Let $R\xra{\phi} S$ be a homomorphism of rings. We recall for any $R$-module $M$ and $S$-module $N$ that there is an isomorphism
\[\xymatrix@R=0pc{  \Hom_{S}(\phi^*(M) , N) \ar[r]^{\eta^*}_{\cong} & \Hom_{R}(M, \phi_*(N)) \\ \theta \ar@{|->}[r] & \theta \circ \eta}\]
where $\eta$ is the map $M \to \phi^*(M)$.

Now, if $R$ has characteristic $p>0$, consider the map $R \xra{F_R} R$. For any $R$-modules $M$ and $N$ we get an isomorphism
\[\xymatrix@R=0pc{  \Hom_{R}(F_R^*(M) , N) \ar[r]^{\eta^*}_{\cong} & \Hom_{R}(M, {F_R}_*(N)) \\ \theta \ar@{|->}[r] & \theta \circ \eta}\]
where $\eta$ is the map $M \to {F_R}^*(M)$.
\end{rem}


\begin{prop} Let $(A,\delta)$ be a $\delta$-ring. Let $R$ be an $A$-algebra with $pR=0$. Then $\widetilde{\Omega}_{R|(A,\delta)}=F_R^*( \Omega_{R|A})$ with universal perivation $\tilde{d} = \eta \circ d$.
\end{prop}
\begin{proof} We know that any perivation is an $A$-linear derivation of Frobenius.
Consider the diagram
\[ \xymatrix{ & \Omega_{R|A} \ar[r]^-{\eta} \ar@{-->}[dd]^{\beta} & F^*_R(\Omega_{R|A}) \ar@{-->}[dd]^{\theta} \\ R \ar[ur]^{d_{R|A}} \ar[dr]_{\alpha} \ar[drr]^{\alpha} && \\ & {F_R}_*(M) \ar@{=}[r] & M  }\]
Since $\alpha$ with target ${F_R}_*(M)$ is a derivation, there is a unique $R$-linear $\beta$ making the diagram commute. Then by extension of scalars, there is a unique  $R$-linear $\theta$ making the diagram commute. Thus $\eta d_{R|A}$ satisfies the universal property.
\end{proof}
...
\end{thm}
\begin{proof} This is a sum of perivations, hence a perivation. Given $\alpha$, we must have $\theta(\tilde{d}x_i) = \alpha(x_i)$ and $\theta(\tilde{d}p) = \alpha(p)$, and since $\tilde{d}p, \tilde{d}x_1,\dots,\tilde{d}x_n$ are a free basis, there is a unique map $\beta$ satisfying this, so if $\theta$ exists, we must have $\theta=\beta$.  For this map, we have $\alpha$ and $\beta \circ \tilde{d}$ are two perivations that agree on $p$ and the $x_i$'s. But it follows from the sum and product rule that a perivation is determined by its values on this set, so $\alpha=\beta \circ \tilde{d}$; i.e., $\theta=\beta$ makes the diagram commute, showing the universal property.

The unique iso statement follows exactly as in the case of differentials.
\end{proof}
...
\begin{thm}[Second fundamental sequence]
Let $(A,\delta)$ be a $\delta$-ring, and $R\xra{\phi} S=R/I$ be a map of $A$-algebras. Then there is an exact sequence
\[ F_{S/pS}(I/(I^2,pI)) \xra{\tilde{d}} S\otimes_R \widetilde{\Omega}_{R|(A,\delta)} \to \widetilde{\Omega}_{S|(A,\delta)} \to 0.\]
Furthermore, if $A/pA$ is a perfect field and $I$ is a maximal ideal containing $p$, the first map is injective.
\end{thm}


\begin{defn} Let $(A,\delta)$ be a $\delta$-ring and $S=A[x_1,\dots,x_n]$. Let $\Delta$ be a $p$-derivation on $S$ extending $\delta$. For a sequence of elements $f_1,\dots,f_t\in S$, we define the \emph{mixed (column) Jacobian matrix}\index{mixed Jacobian matrix} to be
\[ \widetilde{J}^T(f_1,\dots,f_t) = \begin{bmatrix} \Delta(f_1) & \cdots & \Delta(f_t) \\ (\frac{df_1}{dx_1})^p & \cdots & (\frac{df_t}{dx_1})^p \\ \vdots & \ddots & \vdots \\ (\frac{df_1}{dx_n})^p & \cdots & (\frac{df_t}{dx_n})^p \end{bmatrix}.\]
\end{defn}

\begin{thm}
 Let $(A,\delta)$ be a $\delta$-ring and $S=A[x_1,\dots,x_n]$. Let $\Delta$ be a $p$-derivation on $S$ extending $\delta$. Let $I=(f_1,\dots,f_t)$ and $R=S/I$. Let  $\fq$  be a prime containing $I$, and let $\fp=\fq R$. Then $\widetilde{\Omega}_{R_{\fp}|(A,\delta)}$ is the $R_\fp/ p R_{\fp}$-module with presentation matrix $\widetilde{J}^T(f_1,\dots,f_t)$.
 \end{thm}
 \begin{proof}[Sketch:] Follows from the second fundamental sequence + universal perivation modules localize.
 \end{proof}
 
 \begin{thm}
 Let $S=\Z_{(p)}[x_1,\dots,x_n]$. Let $R=S/I$, with $I=(f_1,\dots,f_t)$, let $\fp$ be a prime ideal of $R$ containing $p$, and $\fq$ the preimage of $\fp$ in $S$. The following are equivalent:
 \begin{enumerate}
 \item $R_\fp$ is regular
 \item $\widetilde{\Omega}_{R_{\fp}}$ is free of rank $h:= n-\htt(I S_\fq)$
 \item $\fp \supseteq I_h(\widetilde{J}^T(f_1,\dots,f_t))$.
 \end{enumerate}
 \end{thm}

 \begin{ex} Let $R=\Z[\sqrt[3]{n}]$ with $n$ squarefree (and not equal to $0,1$). Let us determine which of these are regular using the theorem.
 We have $R=\Z[x]/(x^3-n)$, using that $n^3\notin \Z$. Then for $\fa$ prime in $\Z[x]$ containing $p$, $\Z \smallsetminus (p) \subseteq \Z[x]\smallsetminus \fa$, so $R_{\fp} \cong \Z_{(p)}[x]_{\fq}/(x^3-n)$ where $\fq$ is the expansion of $\fa$.
 
 Now, the set of singular primes containing $p$ is $V_R(p, \delta(x^3-n), 3x^2)$ (from the $1\times 1$ minors of the mixed Jacobian), where $\delta$ is a $p$-derivation on $\Z_{(p)}[x]$. Let's simplify this ideal up to radical.
 
 We compute
 \[ \delta(x^3 - n) = \frac{x^{3p} - n - (x^3 - n)^p }{p} = \delta(-n) - C_p(x^3,-n)\]
 Since $x^3\equiv n$ in $R$, we have
 \[  \delta(x^3 - n) = \delta(-n) - C_p(n,-n). \]
 If $p\neq 2$, then $\delta(-n) = -\delta(n)$ and $C_p(n,-n) = 0$, so $\delta(x^3 - n) = -\delta(n)$; if $p=2$, then $\delta(-n) = -\delta(n) + n^2$ and $C_p(n,-n) = -n^2$, so $\delta(-n) = -\delta(n)+2n^2 \equiv -\delta(n)$ mod $p=2$.
 So the set of singular primes containing $p$ is $V_R(p, \delta(n), 3x^2)$.
 
 If $p\neq 3$, then this is $V_R(p, \delta(n), x^2) = V_R(p, \delta(n), x) = V_R(p, \delta(n), n, x)$, using the defining equation. If $p\nmid n$, then $(p,n)=1$ and this is empty. If $p\mid n$, by squarefree hypothesis, $p^2 \nmid n$, but by last time's computation, $p\nmid \delta(n)$, so $(p,\delta(n))=1$, and this is empty again.
 
 If $p=3$, then this is $V_R(3, \delta(n))$. If $3\mid \delta(n)$, then this is just $V_R(3) = \{ (3,1+ \sqrt[3]{n})\}$; if $3\nmid \delta(n)$, then $(3,\delta(n)) = 1$ and this is empty.
 
 Observe that $\delta(9k) = 9^3 \delta(k) + k^3\delta(9) + 3 \delta(k)\delta(9)$ is a multiple of $3$, since $\delta(9)$ is, and then $\delta(a+9k) = \delta(a) + \delta(9k) + C_3(a,9k)$ is congruent to $\delta(a)$ modulo $3$. We then compute for $i=0,\dots,8$,
 
 \[ \begin{tabular}{c|cccccccccc} $n$ & $-4$&$ -3$ & $-2$ &$-1$ & 0 & 1 & 2 & 3 & 4 \\ \hline
 $\delta(n)$ &
    20  & 8  & 2& 0    &   0 & 0 & $-2$ & $-8$ & $-20$\end{tabular}\]
 
Thus, $\delta(n)$ is only a multiple of $3$ if $n\equiv \pm 1$ (or $0$) modulo $9$.
 
 In particular, $\Z[\sqrt[3]{35}]$ and $\Z[\sqrt[3]{37}]$ are not regular, whereas $\Z[\sqrt[3]{39}]$, $\Z[\sqrt[3]{41}]$, and $\Z[\sqrt[3]{43}]$ are.
 \end{ex}
...
\begin{rem}
In the Theorem above, we can replace the condition ``$J^2=0$'' with ``$J$ is nilpotent'' (there exists $n$ such that $I^n=0$). To see it, write $T\to T/I^{n-1} \to T/I^{n-2} \to \cdots \to T/I^2 \to T/I$ and note that in each step one kills a square zero ideal in the source. Then consider the following diagram:
