...
If $K\to S_{\fq}$ is smooth, then $d$ is a split injection, so $I S_{\fq} / I^2 S_{\fq}$ is free, and hence the map $q$ is an isomorphism. Then $J(f)^T$ is a split injection, so by the lemma, the column Jacobian must be injective modulo $\fq S_{\fq}$, yielding the last condition.

Conversely, if the map $J(f)^T$ is injective modulo $\fq S_{\fq}$, it is split injective by the lemma, forcing $q$ to be an isomorphism, and then forcing $d$ to be a split injection, and hence forcing $R_\fq$ to be essentially smooth over~$K$.\end{proof}
