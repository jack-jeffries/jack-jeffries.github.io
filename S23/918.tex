...
\end{lem}
\begin{proof}
In either case, we have a well-defined function returning an object of the same type. The formulas are the same in both cases, just allowing infinite formal sums for power series, so we'll deal with both simultaneously. We'll 
...
\[\begin{aligned}
 ...
 \\&= \frac{d}{dx}  (\sum_{k=0} \sum_{i+j=k} (a_ib_j) x^k ) = \frac{d}{dx} (rs).\end{aligned}\]
 \end{proof}
...
\[ \frac{d}{dx}|_{x=x_0}\ (fg) =  f(0) (\frac{d}{dx}|_{x=x_0} \ g) +  g(0) (\frac{d}{dx}|_{x=x_0} \ f) = f \cdot  (\frac{d}{dx}|_{x=x_0}\ g) + g \cdot (\frac{d}{dx}|_{x=x_0}\ f).\]
\end{ex}
