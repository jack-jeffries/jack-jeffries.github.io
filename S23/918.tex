...
\end{prop}
\begin{proof}
Unfortunately, we do not have a snappy universal property to help us here, so we proceed directly. Suppose we are given polynomial rings $S=A[X]$ and $S'=A[Y]$ with surjections $\pi:S\to R$ and $\pi':S'\to R$. Then there is a surjection $\pi'':A[X,Y]\to R$ given by sending $\pi''(x_i)=\pi(x_i)$ and $\pi''(y_i)=\pi'(y_i)$. 
\[\xymatrix{  S'' \ar[dr]^-{\pi''} & S' \ar[l]_-{\rotatebox{180}{$\subset$}} \ar[d]^-{\pi'} \\ S \ar[r]^-{\pi}\ar[u]^-{\rotatebox{90}{$\subset$}} & R}\]
If we show that $\pi$ and $\pi''$ yield isomorphic $\Gamma$ modules, then by symmetry, $\pi$ and $\pi'$ do as well.

For each $y_i\in Y$, we can choose some $g_i\in A[X]$ with $\pi(g_i)=\pi'(y_i)$. Let $I=\ker(\pi)$; we'll sometimes write $I(x)$ for $I$ to indicate this consists of polynomials in the $X$ variables. Note that $\ker(\pi'')=I(x) S'' +(\{ y_i - g_i \})$. We can take a change of variables in $S''=A[X,Y]$ replacing $y_i$ by $y_i-g_i$; then without loss of generality, we can assume that $J=\ker(\pi'') = I(x)S''+(Y)$. 

The inclusion maps $I\subseteq J$ and $I^2\subseteq J^2$ induce a map $I/I^2 \to J/J^2$. The kernel of this is $\frac{I\cap J^2}{I^2}$. Since $J^2\cap I = (I(x)^2,I(x) (Y), (Y)^2)S'' \cap I \subseteq (I(x)^2,I(x) (Y), (Y)^2)A[X,Y] \cap A[X] = I^2$, this map is injective. The cokernel is $\frac{(I(x),Y)S''}{(I(x),(Y))^2 S'' + I(x)}\cong \frac{(I(x),Y)S''}{(I(x),(Y)^2) S''}$. We claim that this is a free $R$-module with basis $\{[y_i]\}$. Indeed, it is generated by these as an $S''$-module, and given an $R$-linear relation on these classes, this pulls back to an $S''$-linear relation $\sum_j p_j(x,y) y_j\in (I(x),(Y)^2)A[X,Y]$: the terms in $p_j$ without any $y$'s must also be in $I$, so each $p_j(x,y)\in (I(x),(Y))S'$, and hence is the trivial relation over $R$.
...
We can then take a change of coordinates and replace $y_w-w^{-1}$ by $y_w$.

Then, the rest of the argument proceeds as in the previous one.
\end{proof}
...
\begin{lem} A regular local ring $(R,\fm,k)$ is a domain.
\end{lem}
\begin{proof}
	By induction on $d = \dim R$, where $R$ is a regular local ring. If $d=0$, then $R$ must be a field, and thus a domain.
	
	If $d>0$, consider $x \in \fm \setminus \left( \fm^2 \cup \bigcup_{\mathfrak{p} \in \mathrm{Min} R} \mathfrak{p} \right)$. (Here we are using the strong version of prime avoidance where we can avoid one arbitrary ideal and a finite number of prime ideals.) By NAK, $\fm/xR$ is generated by $d-1$ elements, so that $\dim R/xR \leqslant d-1$ by Krull's Height Theorem. Then $\dim R/xR = d-1$. By Induction Hypothesis, $R/xR$ is a domain, and thus $xR$ is a prime ideal in $R$, which is not minimal. Given a prime ideal $0 \subseteq \mathfrak{p} \subseteq xR$, if $y \in \mathfrak{p}$, we can write $y =ax$ for some $a \in R$, and if $x \notin \mathfrak{p}$, then $a \in \mathfrak{p}$. Thus $x \mathfrak{p} = \mathfrak{p}$, which by NAK implies that $\mathfrak{p} = 0$. Thus $R$ is a domain.
\end{proof}
...
\end{lem}
\begin{proof}
First, we note that $\fp R[x]$ is prime, since $R[x]/\fp R[x] \cong (R/\fp)[x]$.

Now, we show every other prime is of the form above. Every prime $\fq$ with $\fq \cap R=\fp$ contains $\fp R[x]$, and corresponds to a prime in $(R/\fp)[x]$ that contracts to $0$ in $R/\fp$. Moreover, by the correspondence of primes in localizations, such a prime is the contraction of a prime in $(R/\fp)_{\fp}[x]$. Since $(R/\fp)_{\fp}$ is a field, such a prime is principal $(f(x))$, and the contraction to $(R/\fp)[x]$ is the elements with a nonzero $R/\fp$-multiple in $(f(x))$. The, taking preimages in $R[x]$ gives the description above.

The height of $\fp[x]$ is at least that of $\fp$, since a chain
 \[ \fr_0 \subsetneqq \fr_1 \subsetneqq \fr_2 \subsetneqq \cdots \fr_h= \fp\]
 yields a chain
  \[ \fr_0[x] \subsetneqq \fr_1[x] \subsetneqq \fr_2[x] \subsetneqq \cdots \fr_h[x]= \fp[x].\]
  Consequently, a prime of the second type has height at least $\htt(\fp)+1$.

Finally, we compute the heights. We have $\htt(\fq)=\htt( \fq R_{\fp}[x])$ since any prime contained in $\fq$ does not meet $\fp$. Now, $R_{\fp}$ has a system of parameters $(z_1,\dots,z_h)$ with $\fp R_\fp$ nilpotent modulo $(z_1,\dots,z_h)$. But then $\fp R_\fp[x]$ is nilpotent modulo $(z_1,\dots,z_h)R_\fp[x]$ as well (since a polynomial with nilpotent coefficients is nilpotent). Thus, $\fp R_\fp[x]$ is a minimal prime of $(z_1,\dots,z_h)R_\fp[x]$, so $\htt(\fp R_\fp[x]) \leq h = \htt(\fp R_{\fp}) = \htt(\fp)$, so equality holds in case (1). For a prime of case (2), we note that 
$\frac{R_\fp}{(z_1,\dots,z_h)}[x]$ has dimension one, since killing the ideal of nilpotents $\fp R_\fp$ yields a polynomial ring over a field, which has dimension one. Then, since $\frac{R[x]_{\fq}}{(z_1,\dots,z_h)}$ is a localization of $\frac{R_\fp}{(z_1,\dots,z_h)}[x]$, we have
...
\end{prop}
\begin{proof} This follows from the Auslander-Buchsbaum-Serre theorem in homological algebra, but we can argue directly. Let $\fq$ be prime in $R[x]$ and $\fp=\fq \cap R$. Let $h=\htt(\fp)$ and $e_1,\dots,e_h\in\fp$ be a basis for $\fp R_\fp/ \fp^2 R_\fp$, so for any $p\in \fp$, there is $p'\in \fp^2$, and $r_j\in R_{\fp}$ with $p = \sum_j r_j e_j + p'$ in $R_\fp$.
...
\begin{cor} If $K$ is a field, $R=K[x_1,\dots,x_n]$ is a polynomial ring, and $\fp\subset R$ is prime, then $R_\fp$ is regular.
\end{cor}


\begin{lem}
Let $(S,\fn,k)$ be a regular local ring and $I$ be an ideal. Then the quotient ring $S/I$ is regular if and only if the images of a set of minimal generators of $I$ in $\fn/\fn^2$ is linearly independent, i.e., the map $I/\fn I \to \fn/\fn^2$ induced by inclusion is injective.
\end{lem}
\begin{proof} Let $d=\dim(S)$.
First, we observe that if $f\in \fn/\fn^2$, then $S/(f)$ is regular: indeed, as $S$ is a domain, we have $\dim(S/(f)) < d$ and $\dim(S/(f))\geq d-1$, so $\dim(S/(f))=d-1$; on the other hand, the image of $f$ is nonzero in $\fn/\fn^2$, so $\frac{\fn(S/(f))}{\fn^2 (S/(f))} \cong \frac{\fn}{\fn^2+(f)}$ has dimension $d-1$.

Now, if $f_1,\dots,f_t$ be a minimal generating set for $I$ and their images are linearly independent in $\fn/\fn^2$, by induction on $t$ we get that $S/I$ is regular. Conversely, if the images of the $f$'s are not linearly independent in $\fn/\fn^2$ suppose that the images of $f_1,\dots,f_a$ form a basis for the image; subtracting off copies of these, we may suppose the rest are in $\fn^2$. Then $S/(f_1,\dots,f_a)$ is a regular local ring, and $S/I \cong S'/J$ for $S'=S/(f_1,\dots,f_a)$ some ideal $J$ in the maximal ideal $\fn S'$. But then $\dim(S'/J)<\dim(S')$ and $\dim(\fn (S/I)/ \fn^2 (S/I)) = \dim (\fn S' / \fn^2 S') = \dim(S')$, so $S/I\cong S'/J$ is not regular.
\end{proof}

\subsection{Smoothness vs regularity}

\begin{lem} Let $(R,\fm,k)$ be a local ring. Let $A$ be an $n \times m$ matrix with entries in $R$. Then the map $R^m \xra{A} R^n$ is a split injection if and only if $m\leq n$ and some $m \times m$ minor of $A$ is a unit.
\end{lem}
\begin{proof}
$(\Leftarrow):$ Suppose that some $m \times m$ minor of $A$ is a unit; without loss of generality, say the minor involving the first $a$ rows. Let $A'$ be this submatrix. Then $B' = \det(A')^{-1} \mathrm{adj}(A')$ satisfies $B' A' = I$. Then set $B=[B' | 0]$ and we have $BA=I$.
$(\Rightarrow):$ We apply the Cauchy-Binet formula: for an $m\times n$ matrix $C$ and an $n\times m$ matrix $D$ and a subset $I\subseteq [n]$ of size $m$, write $C_{\bullet,I}$ to be the submatrix with columns indexed by $I$ and $D_{I,\bullet}$ to be the submatrix with rows indexed by $I$. Then $\det(CD) = \sum_{I\subseteq [n], |I|=m} \det(C_{\bullet,I}) \det(D_{I,\bullet})$. In particular, $\det(CD)$ is a linear combination of the $m\times m$ minors of $D$. So, if all the minors are in $\fm$, then there can be no left inverse.
\end{proof}

\begin{exer} Prove the Cauchy-Binet formula.
\end{exer}
...
Conversly, if the map $J(f)^T$ is injective modulo $\fq S_{\fq}$, it is split injective, forcing $q$ to be an isomorphism, and then forcing $d$ to be a split injection, and hence forcing $R_\fq$ to be essentially smooth other $K$.\end{proof}
