...
If $R\to S$ is {formally smooth}, $\Omega_{S|R}$ is projective and $\Gamma_{S|R}=0$, so $S\otimes_R \Omega_{R|A} \xra{S\otimes d_{\phi|A}} \Omega_{S|A}$ is a split injection, and $\Hom_{S}(\Omega_{S|A},M)\cong \Der_{S|A}(M) \xra{\phi^*} \Der_{R|A}(M)\cong \Hom_S(S\otimes_R \Omega_{R|A},M)$ is a surjective for all $M$.
...
\end{lem}
\begin{proof}
Let $a_1,\dots,a_n$ be the rows of $A$ and $b_1,\dots,b_n$ be the columns of $B$. We can think of $\det(AB)$ as a function $D(a_1,\dots,a_n,b_1,\dots,b_n)$ that is $R$-linear in each argument. Likewise, the function $\sum_{I\subseteq [m], |I|=n} \det(A_{\bullet,I}) \det(B_{I,\bullet})$ is $D'(a_1,\dots,a_n,b_1,\dots,b_n)$ linear in each argument. Thus, if the formula holds whenever each $a_i$ and each $b_i$ is a standard basis vector in $R^m$, the formula holds in general by linearity. Moreover, if any standard vector repeats in the $a$'s or the $b$'s, then both sides are zero. Thus, we reduce to the case where each row of $A$ has exactly one $1$ and all other entries $0$, and each column of $B$ has exactly one $1$ and all other entries $0$. Both expressions also stay the same if we permute the columns of $A$ and rows of $B$ simultaneously, so we can assume that $A$ is an identity block followed by zeroes. Then the product of $AB$ is the first $n$ rows of $B$, which corresponds to the only nonzero term in the second sum.
\end{proof}
...
$(\Rightarrow):$ We apply the Cauchy-Binet formula: $\det(BA)$ is a linear combination of the $m\times m$ minors of $A$. So, if all the minors are in $\fm$, then there can be no left inverse.
\end{proof}
