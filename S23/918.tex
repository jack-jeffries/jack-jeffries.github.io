...
\begin{cor} Let $K$ be a perfect field and $R=K[x_1,\dots,x_n]/I$. Let $\fp_1,\dots,\fp_t$ be the minimal primes of $R$, with heights $h_1,\dots,h_t$. Then 
\[ \mathrm{Sing}(R) = \bigcup_{i<j} V( \fp_i + \fp_j) \cup \bigcup_{k} V(I_{h_k}(J^T(\text{gens of} \ \fp_k)))= V( \prod_{i<j} ( \fp_i + \fp_j) \cdot \prod_{k}I_{h_k}(J^T(\text{gens of} \ \fp_k)) ). \]
In particular, the singular locus is closed.
\end{cor}
...
\end{thm}
\begin{proof} By the previous theorem, $R_{\fp}$ is regular if and only if $\Omega_{R_{\fp}|K}$ is free of rank $n-\htt I S_\fq$, where $\fq$ is the preimage of $\fp$ in the polynomial ring. Removing the minimal components of $R$ that are not contained in $\fp$ (i.e., replacing $R$ by  $\dim(\widetilde{R}^\fp)$), we can assume that $\dim(R)=n-\htt I S_\fq$. Thus, we just need to show that if $\Omega_{R_{\fp}|K}$ is free, then its rank is $\dim(R)$.
Let $\fa$ be a minimal prime of $R$ with $\dim(R) = \dim(R/\fa)$. From the second fundamental sequence, we have
\[ \frac{\fa R_{\fa}}{\fa^2 R_{\fa}} \xra{d} \kappa(\fa) \otimes_{R_\fp} \Omega_{R_\fp|K} \to \Omega_{\kappa(\fa)|K} \to 0.\]
Since $\kappa(\fa)$ is the fraction field of $R/\fa$, which is a finitely generated domain over the field $K$, we have $\dim(R/\fa) = \mathrm{trdeg}_K(\kappa(\fa))$; by the lemma, $p\deg_K(\kappa(\fa))=\dim(R/\fa)=\dim(R)$ if $K$ has positive characteristic. Either way, $\Omega_{\kappa(\fa)}$ has rank $\dim(R)$. Thus, it suffices to show that the map $d$ in the sequence above is the zero map.
...
However, if we can differentiate by $t$, then the $1\times 1$ minors of the Jacobian matrix give the expected result.
\end{ex}


Let $K$ be a field and $R=K[x_1,\dots,x_n]$. Let $K_0\subseteq K$ be a subfield, so we have inclusions $K_0 \xra{i} K \xra{j} R$. Recall that $R$ is smooth over $K$, so from the Jacobi-Zariski sequence,
\[ 0 \to R \otimes_{K} \Omega_{K|K_0} \xra{R\otimes d_{j|K_0}} \Omega_{R|K_0} \xra{d_{R|i}} \Omega_{R|K} \to 0\]
is exact, and split-exact by projectivity of the right. We can use this to compute the universal derivation of $R$ over $K_0$.
 Let $T= \{k_i\}$ be a transcendence basis for $K/K_0$ if characteristic 0 or a $p$-basis for $K/K_0$ if characteristic $p$. Then we can take $\Omega_{R|K_0}$ to be free on the basis $dx_1,\dots,dx_n, \{d k_j\}$.
 
  
 For $k\in K$, we can compute the universal derivation of $k$ by: in characteristic zero, taking derivatives with respect to the transcendence basis on the purely transcendental extension $K_0\{k_j\}$ and extending uniquely by separability or in characteristic $p$, taking the derivatives with respect to a $p$-basis as discussed earlier. These are the dual basis elements to $dk_j$. Let's write $\frac{d}{dk_j}$ for these derivations on $K$. Then each $\frac{d}{dk_j}$ extends to a $K_0$-linear derivation on $R$ by acting on each coefficient of $f$ considered as a polynomial. 
...
\begin{thm} Let $K$ be a field and $R=K[x_1,\dots,x_n]/I$. Suppose that the height of each minimal prime of $I$ is $h$. Let $K_0\subseteq K$ be a perfect field. Then, the singular locus of $R$ is the ideal of $h\times h$ minors of an extended column Jacobian matrix on a generating set for $I$.
\end{thm}

\begin{cor} Let $R$ be essentially of finite type over a field $K$. Then the singular locus of $R$ is closed.
 Let $K$ be a perfect field and $R=K[x_1,\dots,x_n]/I$. Let $\fp_1,\dots,\fp_t$ be the minimal primes of $R$, with heights $h_1,\dots,h_t$. Then 
\[ \mathrm{Sing}(R) = \bigcup_{i<j} V( \fp_i + \fp_j) \cup \bigcup_{k} V(I_{h_k}(\widetilde{J}^T(\text{gens of} \ \fp_k)))= V( \prod_{i<j} ( \fp_i + \fp_j) \cdot \prod_{k}I_{h_k}(\widetilde{J}^T(\text{gens of} \ \fp_k)) ), \]
where $\widetilde{J}^T$ denotes an extended Jacobian matrix.
In particular, the singular locus is closed.
\end{cor}
...
So, for every $g\in I$, $v(f) \cap v(g)$ is nonempty. That means that every $g\in I$ is in some $\fp_i$ for $iin v(f)$, so $I \subseteq \bigcup_{i\in v(f)} \fp_i$. By the usual version of prime avoidance, $I \subseteq \fp_i$ for some $i$.
\end{proof}
...
Now, $R_{\fp_i R}$ has maximal ideal minimally generated by $x_i^2, x_i^3$, but is one-dimensional, so is not regular. But $R_{(0)}$ is a field, so is regular. Thus, the singular locus consists of the set of the maximal ideals. This set has finitely minimal elements, so it is not a closed set of $\Spec(R)$ (which would have finitely many minimal elements by Noetherianity). Thus, the singular locus is not closed.
