...
\begin{cor} Let $R$ be the $A$-algebra with generators $x_{\lambda}$ and relations $f_{\gamma}$. Then $\Omega_{R|A}$ is the $R$-module with generators $dx_{\lambda}$ and relations $df_\gamma=\sum_\lambda \frac{df_\gamma}{dx_\lambda} dx_{\lambda}$. The universal derivation maps $g\in R$ to $\sum_{\lambda} \frac{dg}{dx_\lambda} dx_{\lambda}$.

In particular (if the set of generators and relations are finite), the presentation matrix of $\Omega_{R|A}$ is
\[ J(f_1,\dots,f_m)^T = \begin{bmatrix} \frac{d}{dx_1}|_{x=a} (f_1)& \cdots & \frac{d}{dx_1}|_{x=a} (f_m)\\ 
\vdots & \ddots & \vdots \\
 \frac{d}{dx_n}|_{x=a} (f_1) & \cdots & \frac{d}{dx_n}|_{x=a} (f_m) \end{bmatrix}.\]
\end{cor}
\begin{proof}
