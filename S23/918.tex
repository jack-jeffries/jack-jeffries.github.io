...
\[ \xymatrix@C=4em{ 0 \ar[r] & \Hom_{R}(\Omega_{R/I | A},M) \ar[r]^{(R/I \otimes d_{\pi|A})^*} \ar[d]^{d_{R/I |A}^*}_{\cong} & \Hom_{R}(\Omega_{R|A},M) \ar[r]^-{d_{R|A}^*} \ar[d]^{\overline{d_{R|A}}^*}_{\cong} & \Hom_{R}(I/I^2,M) \ar@{=}[d]  \\
...
\item The universal derivation $d_{R|A}$ maps $r\in R$ to $\sum_{\lambda} \frac{dr}{dx_i} dx_i$. 
...
\item For $R=K[x,y,z]/(x^n+y^n+z^n)$ with $n>1$ we have $\Omega_{R|K} = \frac{R dx \oplus R dy \oplus R dz}{(n x^{n-1} dx + n y^{n-1} dy + n z^{n-1} dz)}$. 

In particular, if $K$ has characteristic zero we get a free module of rank three quotiented out by a relation with entries in the maximal ideal $\fm=(x,y,z)$. If we invert the nonzero elements in $R$ in our relation we can divide by the coefficient of $dx$ and write it as a combination of the others, so $ )\Omega_{R|K})_{(0)}$ is a free module over the quotient field of rank two. That is $\Omega_{R|K}$ has rank two as an $R$-module. However, $\Omega_{R|K}$ is not free; in particular $(\Omega_{R|K})_\fm$ is not, since $(\Omega_{R|K})_\fm / \fm (\Omega_{R|K})_\fm$ is a three dimensional $K$-vector space on the basis $dx,dy,dz$. This all works as long as $n$ is not a multiple of the characteristic of $K$.

If $K$ has characteristic $p$ and $n=p$, then the relation is trivial, so $\Omega_{R|K}$ is free of rank three.

\item For $R=K[x,y,z]/(x^n+y^n+z^n-1)$, we have $\Omega_{R|K} = \frac{R dx \oplus R dy \oplus R dz}{(n x^{n-1} dx + n y^{n-1} dy + n z^{n-1} dz)}$. We claim that, if the characteristic of $K$ is zero, $\Omega_{R|K}$ is a locally free module of rank two. Since $(x,y,z)$ generate the unit ideal of $R$, there is no maximal ideal containing all of them, so any maximal ideal fails to contain at least one. Let $\fm$ be a maximal ideal; WLOG say $x\notin \fm$. Then by an argument similar to above, we get that $(\Omega_{R|K})_\fm$ is free of rank two. Depending on $K$ and $n$, this module is not necessarily (globally) free. For example, if $K=\R$ and $n=2$, this is impossible. Prove it! 
...
\begin{defn} We say that a ring homomorphism $R\xra{\phi} S$ is \emph{essentially algebra-finite} or \index{essentially of finite type} if $\phi$ factors as an algebra-finite map followed by a localization. Concretely, $S=W^{-1} (R[x_1,\dots,x_n]/I)$ for some ideal $I$ and multiplicatively closed set $W$.
\end{defn}
...
We recall that a ring homomorphism $R\xra{\phi} S$ is \emph{flat}\index{flat} if the corresponding extension of scalars functor $S\otimes_R -$ turns injective maps into injective maps.
...
with exact rows and the snake lemma gives the desired sequence. In the case $R\to S$ is flat, we have that the kernel of $S\otimes_R I/I^2 \to \bigoplus S dx_i$ is $S\otimes_R \Gamma_{R|A}$, and we get one more from the snake lemma.
\end{proof}
...
\begin{exer} Show that $W^{-1}\Gamma_{R|A}\cong \Gamma_{W^{-1}R|A}$.
\end{exer}
...
By exactness of Hom, $\Der_{S|A}(M) \xra{\phi^*} \Der_{R|A}(M)$ is bijective for all $M$ if and only if $S\otimes_R \Omega_{R|A} \xra{S\otimes d_{\phi|A}} \Omega_{S|A}$ is a split injection, which happens at least if $\Gamma_{S|R}=0$ and $\Omega_{S|R}$ is projective.
...
\begin{prop} Let $R\to S$ be a homomrophism of $A$-algebras and $M$ be an $S$-module.
\begin{enumerate}
\item If $\phi$ is formally unramified, then every derivation from $R$ to $M$ extends (via $\phi^*$) to at most one derivation from $S$ to $M$.
\item If $\phi$ is formally \'etale, then every derivation from $R$ to $M$ extends (via $\phi^*$) to exactly one derivation from $S$ to $M$.
\item If $\phi$ is formally smooth, then every derivation from $R$ to $M$ extends (via $\phi^*$) to at least one derivation from $S$ to $M$.
\end{enumerate}
\end{prop}
