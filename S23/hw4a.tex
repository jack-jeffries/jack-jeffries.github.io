...
\item Let $(R,\fm)$ be a local ring. Show that every derivation $\partial:R\to R$ extends to a unique derivation $\hat{\partial}: \hat{R} \to \hat{R}$.
...
\item Show that if $K$ has characteristic $p>0$, then $\Der_{R|K}(M) = \sum_i M\frac{d}{dx_i}$.
