...
\begin{ex}
Let's return to old-fashioned derivatives of $\C^\infty$ functions. Before we get derivatives of functions as functions, we start with the notion of derivative at a point, which should just be a number. Let's try to realize ``derivative at $x=x_0$'' for some real number $x_0$, which we'll write as $\frac{d}{dx}|_{x=x_0}$\index{derivative at $x=x_0$}\index{${\frac{d}{dx}}\mid_{x=x_0}$}, as a derivation on $\cC^{\infty}(\R)$. The target should be $\R$:
...
\begin{cor} Let $R$ be the $A$-algebra with generators $x_i$ and relations $f_j$, and $S$ be the $A$-algebra with generators $y_i$ and relations $g_j$.
\begin{enumerate}
\item $\Omega_{R|A}$ is the $R$-module with generators $dx_i$ and relations $df_j=\sum_i \frac{df_j}{dx_i} dx_i$. The universal derivation maps $g\in R$ to $\sum_{\lambda} \frac{dg}{dx_\lambda} dx_{\lambda}$.
\end{enumerate}
In particular (if the set of generators and relations are finite), the presentation matrix of $\Omega_{R|A}$ is the column Jacobian $J(f_1,\dots,f_m)^T$.
\end{cor}
\begin{proof}
